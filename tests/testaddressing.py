...
class ChatAddressingTest(unittest.TestCase):
    ...
    def make_group_event(
        self, *, sender_id="u1", message_id="m1", text="找你一下", self_id="bot01"
    ):
        event = Event(
            sender_id=sender_id,
    ...
    def test_group_reply_target_uses_at_when_directed_but_message_id_missing(self):
        ...
        event.message_obj = types.SimpleNamespace(message=event.message_items)
        runtime.note_structured_incoming_message(event)
        runtime.note_structured_incoming_message(
            self.make_group_event(sender_id="u2", message_id="m-new")
        )
    ...
    def test_group_reply_without_intervening_message_stays_plain(self):
        runtime = self.make_runtime()
        event = self.make_group_event(message_id="m2")
        event.message_items = [
            Reply(message_id="bot-old", target_message_sender_id="bot01")
        ]
        event.message_obj = types.SimpleNamespace(message=event.message_items)
        runtime.note_structured_incoming_message(event)
    ...
    def test_group_reply_to_older_bot_message_quotes_current_message(self):
        runtime = self.make_runtime()
        scope = "aiocqhttp:GroupMessage:10001"
        old_bot = runtime.note_structured_bot_message(scope, "较早的法律解释")
        runtime.note_structured_bot_message(scope, "刚刚发送的另一条回复")

        event = self.make_group_event(sender_id="u1", message_id="m-old-reply")
        event.is_at_or_wake_command = True
        event.message_items = [
            Reply(message_id=old_bot.message_id, target_message_sender_id="bot01"),
            At(user_id="bot01"),
        ]
        event.message_obj = types.SimpleNamespace(message=event.message_items)
        runtime.note_structured_incoming_message(event)
        event.set_result(event.chain_result([types.SimpleNamespace(text="我接着说")]))

        changed = runtime.apply_group_addressing_before_send(event)

        self.assertTrue(changed)
        self.assertIn("reply", component_kind(event.get_result().chain[0]))
        self.assertEqual(component_message_id(event.get_result().chain[0]), "m-old-reply")

    def test_group_reply_with_intervening_user_quotes_current_message(self):
        runtime = self.make_runtime()
        event = self.make_group_event(sender_id="u1", message_id="m2")
        event.message_items = [
            Reply(message_id="bot-old", target_message_sender_id="bot01")
        ]
        event.message_obj = types.SimpleNamespace(message=event.message_items)
        runtime.note_structured_incoming_message(event)
        runtime.note_structured_incoming_message(
            self.make_group_event(sender_id="u2", message_id="m3", text="我插一句")
        )
        event.set_result(event.chain_result([types.SimpleNamespace(text="我看到了")]))
    ...
    def test_group_at_with_intervening_user_quotes_current_message(self):
        runtime = self.make_runtime()
        event = self.make_group_event(sender_id="u1", message_id="m2")
        event.is_at_or_wake_command = True
        event.message_items = [At(user_id="bot01")]
        ...
        event.set_result(event.chain_result([types.SimpleNamespace(text="我接着答")]))
    ...
    def test_group_addressing_log_reports_direct_reply_and_interruption_count(self):
        runtime = self.make_runtime()
        direct = self.make_group_event(sender_id="u1", message_id="m-direct")
        direct.is_at_or_wake_command = True
        direct.message_items = [At(user_id="bot01")]
        direct.message_obj = types.SimpleNamespace(message=direct.message_items)
        runtime.note_structured_incoming_message(direct)
        direct.set_result(direct.chain_result([types.SimpleNamespace(text="直接答")]))

        with patch("core.runtime.addressing.logger.debug") as log_info:
            runtime.apply_group_addressing_before_send(direct)

        self.assertIn("引用=否", log_info.call_args.args[0])
        self.assertIn("来源=直接回应", log_info.call_args.args[0])
        self.assertIn("插话数=0", log_info.call_args.args[0])

        interrupted = self.make_group_event(sender_id="u1", message_id="m-interrupted")
        interrupted.is_at_or_wake_command = True
        interrupted.message_items = [At(user_id="bot01")]
        interrupted.message_obj = types.SimpleNamespace(message=interrupted.message_items)
        runtime.note_structured_incoming_message(interrupted)
        runtime.note_structured_incoming_message(
            self.make_group_event(sender_id="u2", message_id="m-new", text="我插一句")
        )
        interrupted.set_result(
            interrupted.chain_result([types.SimpleNamespace(text="引用答")])
        )

        with patch("core.runtime.addressing.logger.debug") as log_info:
            runtime.apply_group_addressing_before_send(interrupted)

        self.assertIn("引用=是", log_info.call_args.args[0])
        self.assertIn("来源=发送前出现插话", log_info.call_args.args[0])
        self.assertIn("插话数=1", log_info.call_args.args[0])

    def test_group_at_with_old_multi_user_history_stays_plain(self):
        runtime = self.make_runtime()
        runtime.note_structured_incoming_message(
            self.make_group_event(sender_id="u2", message_id="m-old-1")
        )
        runtime.note_structured_incoming_message(
            self.make_group_event(sender_id="u3", message_id="m-old-2")
        )
        event = self.make_group_event(sender_id="u1", message_id="m2")
        event.is_at_or_wake_command = True
        event.message_items = [At(user_id="bot01")]
    ...
    def test_group_at_same_sender_followup_is_not_interruption(self):
        runtime = self.make_runtime()
        event = self.make_group_event(sender_id="u1", message_id="m2")
        event.is_at_or_wake_command = True
        event.message_items = [At(user_id="bot01")]
        ...
        runtime.note_structured_incoming_message(event)
        runtime.note_structured_incoming_message(
            self.make_group_event(sender_id="u1", message_id="m3", text="再补一句")
        )
        event.set_result(event.chain_result([types.SimpleNamespace(text="知道啦")]))

        changed = runtime.apply_group_addressing_before_send(event)

        self.assertFalse(changed)

    def test_group_at_ignores_intervening_bot_message(self):
        runtime = self.make_runtime()
        event = self.make_group_event(sender_id="u1", message_id="m2")
        event.is_at_or_wake_command = True
        event.message_items = [At(user_id="bot01")]
        event.message_obj = types.SimpleNamespace(message=event.message_items)
        runtime.note_structured_incoming_message(event)
        runtime.note_structured_bot_message(
            event.unified_msg_origin,
            "刚才发出的另一段",
        )
        event.set_result(event.chain_result([types.SimpleNamespace(text="接着说")]))
    ...
    def test_group_interruption_quotes_first_natural_segment_only(self):
        runtime = self.make_runtime()
        for method_name in (
        ...
        previous = self.make_group_event(
            sender_id="u2", message_id="m1", text="我先说一句"
        )
        ...
        runtime.note_structured_incoming_message(event)
        runtime.note_structured_incoming_message(
            self.make_group_event(sender_id="u3", message_id="m3", text="我插一句")
        )
        segments = ["先这样。", "后面再说。"]
        event.set_result(
            event.chain_result([types.SimpleNamespace(text=text) for text in segments])
        )
    ...
    def test_group_proactive_reply_quotes_source_message(self):
        runtime = self.make_runtime()
        source = self.make_group_event(
            sender_id="u3", message_id="m-source", text="没人接话了吗"
        )
    ...
    def test_group_proactive_reply_records_explicit_source_message(self):
        ...
        self.assertTrue(sent)
        messages = runtime.structured_recent_history_messages(
            "aiocqhttp:GroupMessage:10001", limit=2
        )
    ...
    def test_private_reply_never_adds_group_addressing(self):
        runtime = self.make_runtime()
        event = Event(
            unified_msg_origin="aiocqhttp:FriendMessage:10001", message_id="m-private"
        )
