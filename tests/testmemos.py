...
class MemOSTest(unittest.IsolatedAsyncioTestCase):
    def test_format_hidden_context_limits_and_labels_items(self):
        ...
        items = service._parse_search_result(
            {
                "memory_detail_list": [
                    {"memory_value": "阿林喜欢周末看展", "update_time": 1780000000000}
                ],
        ...
        items = [
            *[
                type(
                    ...
                    {
                        "kind": "fact",
                        "content": f"第 {index} 条很长的外部事实记忆，包含很多不应该全部塞进聊天主提示的细节",
                        "score": 0.8,
                    },
                )()
    ...
    async def test_runtime_memos_hidden_context_times_out_quickly(self):
        ...
        runtime.config = settings
        runtime.memos = HostedMemOSService(settings.memos)
        runtime.memos.client = SlowMemOSClient(
            {"memory_detail_list": [{"memory_value": "不该阻塞本轮回复"}]}
        )
        runtime.contact_resolver = type(
            "Resolver",
            (),
    ...
    async def test_runtime_memos_hidden_context_reuses_short_cache(self):
        ...
        runtime.memos = HostedMemOSService(settings.memos)
        runtime.memos.client = FakeMemOSClient(
            {"memory_detail_list": [{"memory_value": "阿林喜欢夜聊"}]}
        )
        runtime.contact_resolver = type(
            "Resolver",
            (),
    ...
    async def test_runtime_memos_concurrent_requests_share_one_lookup(self):
        settings = LifeSettings.from_dict(
            {
                "memos_config": {
                    "enabled": True,
                    "api_key": "key",
                    "injection_timeout_seconds": 1,
                }
            }
        )
        runtime = DailyLifeRuntime.__new__(DailyLifeRuntime)
        ...
        class WaitingMemOSClient(FakeMemOSClient):
            async def search_memory(self, payload):
                self.search_payloads.append(payload)
                started.set()
                await release.wait()
                return type(
                    "Result", (), {"success": True, "data": self.data, "error": ""}
                )()

        runtime.memos.client = WaitingMemOSClient(
            {"memory_detail_list": [{"memory_value": "阿林喜欢夜聊"}]}
        )
        runtime.contact_resolver = type(
            "Resolver",
        ...
        first_task = asyncio.create_task(runtime.build_memos_hidden_context(event))
        await started.wait()
        second_task = asyncio.create_task(runtime.build_memos_hidden_context(event))
        ...
        self.assertIn("阿林喜欢夜聊", first)
        self.assertEqual(len(runtime.memos.client.search_payloads), 1)
        await runtime.close_memos_service()

    def test_memos_cache_key_normalizes_equivalent_query_without_merging_scope(self):
        first_meta = {
            "session_id": "private:10001",
            "sender_profile_id": "10001",
            "group_id": "",
        }
        other_meta = {**first_meta, "session_id": "private:20002"}

        first = DailyLifeRuntime._memos_context_cache_key(
            first_meta, "今晚\u3000  聊什么"
        )
        equivalent = DailyLifeRuntime._memos_context_cache_key(
            first_meta, "今晚 聊什么"
        )
        equivalent_case = DailyLifeRuntime._memos_context_cache_key(
            first_meta, "HELLO World"
        )
        lower_case = DailyLifeRuntime._memos_context_cache_key(
            first_meta, "hello world"
        )
        other_scope = DailyLifeRuntime._memos_context_cache_key(
            other_meta, "今晚 聊什么"
        )

        self.assertEqual(first, equivalent)
        self.assertEqual(equivalent_case, lower_case)
    ...
    async def test_group_memos_context_is_scoped_by_group_and_sender(self):
        ...
        runtime.contact_resolver = type(
            ...
            (),
            {
                ...
                "resolve_group_name": staticmethod(
                    lambda *args, **kwargs: async_return("看展小群")
                ),
            },
        )()
        event = Event(
    ...
    async def test_runtime_schedules_memos_sync_in_background(self):
        ...
        result = runtime.schedule_memos_selected_memory(
            payload, meta, user_message="周末去看展吗", summary=summary
        )
    ...
    async def test_sync_selected_items_uploads_compact_long_term_entries(self):
        ...
        self.assertIn(
            "行为反馈：用户希望必要时改用文字", payload["messages"][1]["content"]
        )
        self.assertFalse(payload["async_mode"])
        self.assertNotIn("source", payload)
    ...
    def test_runtime_builds_selected_items_from_saved_memory_payload(self):
        runtime = DailyLifeRuntime.__new__(DailyLifeRuntime)
        summary = type(
            "Summary", (), {"brief": "阿林提出周末看展", "long_summary": ""}
        )()
        payload = {
        ...
        items = runtime._memos_items_from_payload(
            payload, saved_summary=summary, saved_records=saved_records
        )
    ...
    async def test_sync_memo_uploads_confirmed_memo(self):
        ...
        saved = await service.sync_memo(
            meta=meta, memo="与【阿林】的约定：明天去吃火锅"
        )
