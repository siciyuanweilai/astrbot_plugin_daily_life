...
class GrokVideoServiceTest(unittest.IsolatedAsyncioTestCase):
    async def test_generate_video_uses_compatible_videos_endpoint(self):
        settings = LifeSettings.from_dict(
            {
    ...
    async def test_compatible_video_uploads_reference_image_as_json_data_url(self):
        settings = LifeSettings.from_dict(
            {
                "video_generation_config": {
                    "enabled": True,
        ...
        result = await service._generate_compatible(
            session,
            service._headers(),
            "撑伞走路",
            b"\x89PNG\r\n\x1a\nabc",
    ...
    async def test_poll_timeout_continues_until_next_success(self):
        ...
        result = await service._generate_compatible(session, service._headers(), "撑伞走路", None)
