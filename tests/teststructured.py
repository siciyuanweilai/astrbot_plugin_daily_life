...
class StructuredContextTest(unittest.TestCase):
    def make_group_event(
        self, *, sender_id, sender_name, message_id, text, items=None, card=""
    ):
        event = Event(
            sender_id=sender_id,
    ...
    def test_structured_messages_receive_monotonic_scope_sequence(self):
        runtime = Runtime()
        first_event = self.make_group_event(
            sender_id="u1",
            sender_name="昵称甲",
            message_id="m1",
            text="第一条",
        )
        second_event = self.make_group_event(
            sender_id="u2",
            sender_name="昵称乙",
            message_id="m2",
            text="第二条",
        ...
        first = runtime.note_structured_incoming_message(first_event)
        second = runtime.note_structured_incoming_message(second_event)
        bot = runtime.note_structured_bot_message(
            first_event.unified_msg_origin,
            "第三条",
        )

        self.assertEqual((first.sequence, second.sequence, bot.sequence), (1, 2, 3))
        self.assertEqual(
            getattr(first_event, runtime._STRUCTURED_EVENT_SEQUENCE_ATTR), 1
        )
        self.assertEqual(
            getattr(second_event, runtime._STRUCTURED_EVENT_SEQUENCE_ATTR), 2
        )
