...
class ModelContractTest(unittest.TestCase):
    ...
    def test_world_models_use_current_field_names(self):
        ...
        self.assertIsNone(
            ActionDecisionRecord.from_value({"decision_reason": "旧原因"})
        )
    ...
    def test_lifecycle_models_use_current_field_names(self):
        ...
        self.assertEqual(
            PreferenceRecord.from_value({"content": "偏好", "kind": "sleep"}).category,
            "general",
        )
        self.assertEqual(
            PreferenceRecord.from_value(
                {"content": "偏好", "reason": "旧证据"}
            ).evidence,
            "",
        )
        ...
        event = LifeEventRecord.from_value(
            {"title": "事件", "description": "旧详情", "impact": "旧影响"}
        )
    ...
    def test_experience_models_use_current_field_names(self):
        ...
        self.assertIsNone(
            MemoryEvidenceRecord.from_value(
                {"target_type": "relationship", "summary": "缺目标"}
            )
        )
        ...
        feedback = BehaviorFeedbackRecord.from_value(
            {"action": "observe", "result": "neutral"}
        )
        self.assertEqual(feedback.result, "neutral")

        term = LifeTermRecord.from_value(
            {"term": "蹲后续", "meaning": "继续围观同一话题"}
        )
        self.assertEqual(term.term, "蹲后续")

        intent = ExpressionIntentRecord.from_value(
            {"emotion": "慵懒治愈", "emotion_category": "neutral"}
        )
        ...
        raw_asset = EmojiAssetRecord.from_value(
            {"file_hash": "emoji-2", "emotions": ["happy"]}
        )
        ...
        old_category_asset = EmojiAssetRecord.from_value(
            {"file_hash": "emoji-3", "emotion_category": "category:happy"}
        )
        self.assertEqual(old_category_asset.emotions, [])

        old_single_asset = EmojiAssetRecord.from_value(
            {"file_hash": "emoji-4", "emotion": "happy"}
        )
        ...
        self.assertIsNone(
            MemoryBoundaryRecord.from_value({"source": "群A", "target": "群B"})
        )
    ...
    def test_experience_models_reject_legacy_alias_fields(self):
        ...
        self.assertIsNone(
            ExpressionIntentRecord.from_value({"expression_intent": "旧表达"})
        )
        self.assertIsNone(
            ExpressionIntentRecord.from_value({"emoji_style": "旧表情风格"})
        )
        self.assertIsNone(ExpressionIntentRecord.from_value({"action": "旧动作"}))
        self.assertEqual(
            BehaviorPatternRecord.from_value(
                {"scene": "场景", "pattern": "模式", "action": "reply"}
            ).suggested_action,
            "",
        )
        self.assertEqual(
            BehaviorPatternRecord.from_value(
                {"scene": "场景", "pattern": "模式", "reason": "旧依据"}
            ).evidence,
            "",
        )
        self.assertIsNone(
            BehaviorSceneRecord.from_value({"label": "旧场景", "signals": ["旧线索"]})
        )
        self.assertIsNone(
            BehaviorSceneRecord.from_value({"scene": "场景", "action": "reply"})
        )
        self.assertIsNone(
            BehaviorSceneRecord.from_value({"scene": "场景", "outcome": "旧结果"})
        )
        self.assertEqual(
            SessionMidSummaryRecord.from_value(
                {"session_id": "s1", "summary": "摘要", "people": ["旧参与者"]}
            ).participants,
            [],
        )
        self.assertEqual(
            SessionMidSummaryRecord.from_value(
                {"session_id": "s1", "summary": "摘要", "label": "旧标签"}
            ).scope_label,
            "",
        )
        self.assertEqual(
            LifeTermRecord.from_value(
                {"term": "梗", "meaning": "含义", "example": "旧例子"}
            ).examples,
            [],
        )
        self.assertEqual(
            ReplyEffectRecord.from_value({"scope": "s1", "result": "positive"}).outcome,
            "pending",
        )
        self.assertEqual(
            ReplyEffectRecord.from_value(
                {"scope": "s1", "feedback": "旧反馈"}
            ).evidence,
            "",
        )
        self.assertEqual(
            ReplyEffectRecord.from_value(
                {"scope": "s1", "message_id": "m-old"}
            ).target_message_id,
            "",
        )
        ...
        self.assertIsNone(
            MemoryCorrectionRecord.from_value(
                {"target_type": "relationship", "target_id": "u1", "content": "旧修正"}
            )
        )
        self.assertEqual(
            MemoryCorrectionRecord.from_value(
        ...
        self.assertEqual(
            PhysiologicalRhythmLogRecord.from_value(
                {"summary": "稳定态旧枚举", "lifecycle_kind": "stable"}
            ).lifecycle_kind,
            "transient",
        )
        self.assertFalse(
    ...
    def test_commitment_model_uses_current_field_names(self):
        ...
        commitment = CommitmentRecord.from_value(
            {"content": "约定", "date": "2026-06-18", "time": "10:00"}
        )
