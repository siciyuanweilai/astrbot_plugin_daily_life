...
class LifeSettingsTest(unittest.TestCase):
    ...
    def test_config_parsing_tolerates_bad_types(self):
        config = LifeSettings.from_dict(
            {
                    ...
                    "friend_reference_profiles": [
                        {
                            ...
                            "display_name": "示例好友",
                            "appearance_profile": "身形修长，肩腰比例自然",
                            "reference_images": [
                                {"path": f"D:/ref/friend-{index}.png"}
                ...
                "search_config": {
                    "enabled": "yes",
                    "provider": "grok-search-model",
                    "tavily_api_keys": ["key-a", "key-b"],
                    "max_results": "99",
                    "max_sources": "99",
                    "timeout_seconds": "1",
                    "total_timeout_seconds": "9999",
                    "fetch_timeout_seconds": "999",
                    "cache_ttl_seconds": "999999",
                    "cache_max_items": "1",
                    "deep_max_followups": "99",
                    "max_page_chars": "1",
                    "map_max_results": "999",
        ...
        self.assertEqual(config.history_max_count, 0)
        ...
        self.assertEqual(config.memory.embedding_provider, "embedding-model")
        ...
        self.assertEqual(
            config.memos.base_url, "https://memos.memtensor.cn/api/openmem/v1"
        )
        ...
        self.assertFalse(
            hasattr(config.image_generation, "character_appearance_profile")
        )
        self.assertEqual(len(config.image_generation.text_channels), 1)
        self.assertEqual(
            config.image_generation.text_channels[0].api_url, "https://relay-a.example"
        )
        ...
        self.assertEqual(
            config.image_generation.edit_channels[0].api_url, "https://relay-b.example"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            config.image_generation.character_reference_images[0]["name"],
            "正面参考.png",
        )
        self.assertEqual(len(config.image_generation.friend_reference_profiles), 1)
        friend_profile = config.image_generation.friend_reference_profiles[0]
        self.assertEqual(friend_profile["profile_id"], "profile:friend")
        self.assertEqual(friend_profile["display_name"], "示例好友")
        self.assertNotIn("appearance_profile", friend_profile)
        self.assertEqual(len(friend_profile["reference_images"]), 3)
        self.assertNotIn("appearance_note", friend_profile)
        ...
        self.assertEqual(config.search.provider, "grok-search-model")
        self.assertEqual(config.search.tavily_api_keys, ["key-a", "key-b"])
        self.assertEqual(config.search.max_results, 20)
        self.assertEqual(config.search.max_sources, 20)
        self.assertEqual(config.search.timeout_seconds, 5)
        self.assertEqual(config.search.total_timeout_seconds, 300)
        self.assertEqual(config.search.fetch_timeout_seconds, 180)
        self.assertEqual(config.search.cache_ttl_seconds, 86400)
        self.assertEqual(config.search.cache_max_items, 16)
        self.assertEqual(config.search.deep_max_followups, 3)
        self.assertEqual(config.search.max_page_chars, 2000)
        self.assertEqual(config.search.map_max_results, 500)
        self.assertTrue(config.search.inspiration_enabled)
        self.assertEqual(config.search.today_prompt, "今日 {keyword}")
        self.assertEqual(
            config.chat_style.casual_short_prompt, "自然短一点，认真事先给结论。"
        )
    ...
    def test_character_reference_images_are_fixed_to_six(self):
        config = LifeSettings.from_dict(
            {
                "image_generation_config": {
                    "character_reference_images": [
        ...
        self.assertEqual(len(config.image_generation.character_reference_images), 6)
        self.assertEqual(
            config.image_generation.character_reference_images[-1]["path"],
            "D:/ref/character-5.png",
        )
    ...
    def test_semantic_segment_count_defaults_and_clamps_to_ten(self):
        default = LifeSettings.from_dict({"chat_style_config": {}})
        clamped = LifeSettings.from_dict(
            {"chat_style_config": {"semantic_max_segments": 99}}
        )

        self.assertEqual(default.chat_style.semantic_max_segments, 10)
        self.assertEqual(clamped.chat_style.semantic_max_segments, 10)
    ...
    def test_chat_style_delay_range_is_normalized(self):
        config = LifeSettings.from_dict(
            {"chat_style_config": {"segment_delay_range": "2.2,1.2"}}
        )

        self.assertEqual(config.chat_style.segment_delay_range, "1.2,2.2")
        self.assertEqual(config.chat_style.segment_min_delay_seconds, 1.2)
        self.assertEqual(config.chat_style.segment_max_delay_seconds, 2.2)

        disabled = LifeSettings.from_dict(
            {"chat_style_config": {"segment_delay_range": "1.2,0"}}
        )
        self.assertEqual(disabled.chat_style.segment_min_delay_seconds, 0)
        self.assertEqual(disabled.chat_style.segment_max_delay_seconds, 0)

        removed_fields = LifeSettings.from_dict(
            {
                "chat_style_config": {
                    "segment_delay_range": "invalid",
                    "segment_min_delay_seconds": 0.1,
                    "segment_max_delay_seconds": 0.2,
                }
            }
        )
        self.assertEqual(removed_fields.chat_style.segment_delay_range, "1.5,3.5")
        self.assertEqual(removed_fields.chat_style.segment_min_delay_seconds, 1.5)
        self.assertEqual(removed_fields.chat_style.segment_max_delay_seconds, 3.5)

    def test_chat_style_master_switch_has_no_legacy_field(self):
        enabled = LifeSettings.from_dict({"chat_style_config": {}})
        disabled = LifeSettings.from_dict({"chat_style_config": {"enabled": False}})
        missing = LifeSettings.from_dict({})
        removed_key = "eemantic" + "_enabled"

        self.assertTrue(enabled.chat_style.enabled)
        self.assertFalse(disabled.chat_style.enabled)
        self.assertTrue(missing.chat_style.enabled)
        self.assertFalse(hasattr(enabled.chat_style, removed_key))

    def test_chat_style_punctuation_cleanup_settings(self):
        config = LifeSettings.from_dict(
            {
                "chat_style_config": {
                    "punctuation_cleanup_enabled": "false",
                    "punctuation_cleanup_chars": "，。",
                }
            }
        )
    ...
    def test_chat_style_semantic_segmentation_hints(self):
        ...
        items = schema["chat_style_config"]["items"]
    ...
    def test_conf_schema_no_longer_exposes_catalog_workshop(self):
        ...
        self.assertEqual(
            schema["relationship_aliases"]["description"], "本地用户称呼映射"
        )
        self.assertIn(
            "聊天记忆、邀约、备忘录、参考私聊、MemOS 和面板展示",
            schema["relationship_aliases"]["hint"],
        )
        self.assertNotIn(
            "自定义邀约用户称呼", schema["relationship_aliases"]["description"]
        )
        ...
        for section in ("outfit_config", "invite_config", "vision_config"):
            self.assertEqual(
                schema[section]["items"]["provider"]["_special"], "select_provider"
            )
        embedding_provider = schema["memory_config"]["items"]["embedding_provider"]
        self.assertEqual(embedding_provider["_special"], "select_provider_embedding")
        self.assertEqual(embedding_provider["default"], "")
        self.assertIn("当前默认向量模型", embedding_provider["hint"])
        self.assertNotIn("semantic_segment_config", schema)
        semantic_provider_hint = schema["chat_style_config"]["items"][
            "semantic_provider"
        ]["hint"]
        self.assertIn("留空则使用当前默认模型", semantic_provider_hint)
        self.assertEqual(
            schema["chat_style_config"]["items"]["semantic_provider"]["default"],
            "",
        )
        ...
        self.assertEqual(
            chat_style_items["punctuation_cleanup_enabled"]["default"], True
        )
        self.assertEqual(
            chat_style_items["punctuation_cleanup_chars"]["default"],
            LifeSettings.from_dict({}).chat_style.punctuation_cleanup_chars,
        )
        ...
        self.assertIn("使用当前默认模型", sight_items["frame_provider"]["hint"])
        ...
        self.assertIn(
            "填 0 表示不限制大小", sight_items["video_download_max_mb"]["hint"]
        )
        self.assertEqual(sight_items["video_download_timeout_seconds"]["default"], 240)
        self.assertEqual(
            sight_items["video_download_timeout_seconds"]["slider"]["min"], 30
        )
        self.assertEqual(
            sight_items["video_download_timeout_seconds"]["slider"]["max"], 600
        )
        self.assertEqual(
            sight_items["video_download_timeout_seconds"]["slider"]["step"], 30
        )
        self.assertIn(
            "只影响画面抽帧", sight_items["video_download_timeout_seconds"]["hint"]
        )
        ...
        self.assertEqual(
            sight_items["audio_transcript_mode"]["options"], ["local", "bcut"]
        )
        self.assertEqual(
        ...
        self.assertIn(
            "默认优先使用本地ASR", sight_items["audio_transcript_mode"]["hint"]
        )
        ...
        self.assertIn(
            "generated/images", storage_items["generated_media_keep_days"]["hint"]
        )
        self.assertEqual(storage_items["reverse_cache_keep_days"]["default"], 7)
        self.assertIn(
            "图片反推原图缓存", storage_items["reverse_cache_keep_days"]["hint"]
        )
        response_gate_items = schema["response_gate_config"]["items"]
        self.assertNotIn("enabled", response_gate_items)
        self.assertTrue(response_gate_items["group_enabled"]["default"])
        self.assertTrue(response_gate_items["private_enabled"]["default"])
        ...
        proactive_items = schema["proactive_config"]["items"]
        ...
        delay_index = chat_style_keys.index("segment_delay_range")
        self.assertEqual(
            chat_style_keys[delay_index + 1 : delay_index + 3],
            ["punctuation_cleanup_enabled", "punctuation_cleanup_chars"],
        )
        ...
        self.assertEqual(
            chat_style_items["semantic_max_segments"]["description"],
            "语义分段数量",
        )
        self.assertEqual(chat_style_items["semantic_max_segments"]["default"], 10)
        self.assertEqual(chat_style_items["semantic_max_segments"]["slider"]["max"], 10)
        self.assertTrue(chat_style_items["enabled"]["default"])
        self.assertIn("四项参考长度", chat_style_items["enabled"]["hint"])
        for key in (
        ...
        self.assertNotIn("eemantic" + "_enabled", chat_style_items)
        self.assertEqual(chat_style_items["casual_max_chars"]["slider"]["min"], 10)
        self.assertEqual(
            chat_style_items["group_casual_max_chars"]["slider"]["min"], 10
        )
        self.assertEqual(
            chat_style_items["private_casual_max_chars"]["slider"]["min"], 10
        )
        ...
        self.assertEqual(
            chat_style_items["group_casual_max_chars"]["slider"]["max"], 30
        )
        self.assertEqual(
            chat_style_items["private_casual_max_chars"]["slider"]["max"], 30
        )
        ...
        self.assertIn(
            "不要写具体场景示例", chat_style_items["casual_short_prompt"]["hint"]
        )
        ...
        self.assertEqual(
            schema["memos_config"]["items"]["base_url"]["default"],
        ...
        self.assertFalse(
            schema["memos_config"]["items"]["sync_selected_memory"]["default"]
        )
        self.assertTrue(schema["memos_config"]["items"]["sync_corrections"]["default"])
        self.assertEqual(
            schema["memos_config"]["items"]["timeout_seconds"]["default"], 15
        )
        self.assertEqual(
            schema["memos_config"]["items"]["timeout_seconds"]["slider"]["max"], 60
        )
        self.assertEqual(
            schema["memos_config"]["items"]["injection_timeout_seconds"]["default"], 0.8
        )
        self.assertIn(
            "聊天回复前的注入等待由下方单独控制",
            schema["memos_config"]["items"]["timeout_seconds"]["hint"],
        )
        self.assertIn("MemOS 托管服务", schema["memos_config"]["hint"])
        self.assertEqual(schema["storage_config"]["description"], "数据管理")
        self.assertFalse(
            schema["image_generation_config"]["items"]["enabled"]["default"]
        )
        self.assertFalse(
            schema["video_generation_config"]["items"]["enabled"]["default"]
        )
        self.assertFalse(
            schema["voice_generation_config"]["items"]["enabled"]["default"]
        )
        image_items = schema["image_generation_config"]["items"]
        ...
        self.assertEqual(
            image_items["prompt_rewrite_provider"]["_special"], "select_provider"
        )
        self.assertIn("安全拒绝", image_items["prompt_rewrite_provider"]["hint"])
        self.assertEqual(
            image_items["photo_suite_planning_timeout_seconds"]["default"], 45
        )
        self.assertEqual(
            image_items["photo_suite_planning_timeout_seconds"]["slider"],
            {"min": 10, "max": 120, "step": 5},
        )
        self.assertIn(
            "当前默认模型",
            image_items["photo_suite_planning_timeout_seconds"]["hint"],
        )
        ...
        self.assertEqual(
            channel_items["model"]["default"], "gemini-3-pro-image-preview"
        )
        ...
        self.assertLess(
            list(channel_items).index("resolution"),
            list(channel_items).index("aspect_ratio"),
        )
        self.assertLess(
            list(channel_items).index("aspect_ratio"),
            list(channel_items).index("timeout_seconds"),
        )
        openai_channel_items = image_items["edit_channels"]["templates"]["openai"][
            "items"
        ]
        ...
        self.assertNotIn(
            "character_reference_image", schema["image_generation_config"]["items"]
        )
        self.assertIn(
            "character_reference_images", schema["image_generation_config"]["items"]
        )
        self.assertEqual(
            schema["image_generation_config"]["items"]["character_reference_images"][
                "_special"
            ],
            "character_reference_gallery",
        )
        self.assertIn(
            "character_reference_policy", schema["image_generation_config"]["items"]
        )
        self.assertNotIn("character_appearance_profile", image_items)
        self.assertIn("friend_reference_profiles", image_items)
        self.assertEqual(
            image_items["friend_reference_profiles"]["_special"],
            "friend_reference_profiles",
        )
        ...
        self.assertLess(
            list(image_items).index("character_reference_images"),
            list(image_items).index("photo_suite_planning_timeout_seconds"),
        )
        self.assertLess(
            list(image_items).index("photo_suite_planning_timeout_seconds"),
            list(image_items).index("friend_reference_profiles"),
        )
        self.assertLess(
            list(image_items).index("friend_reference_profiles"),
            list(image_items).index("text_channels"),
        )
        self.assertLess(
            list(image_items).index("text_channels"),
            list(image_items).index("edit_channels"),
        )
        self.assertEqual(
            schema["image_generation_config"]["items"]["character_reference_policy"][
                "description"
            ],
            "角色形象参考",
        )
        ...
        self.assertTrue(
            schema["voice_generation_config"]["items"]["smart_switch_enabled"][
                "default"
            ]
        )
        self.assertIn(
            "普通聊天文字发送前智能切换语音",
            schema["voice_generation_config"]["items"]["smart_switch_enabled"]["hint"],
        )
        self.assertNotIn(
            "模型判断",
            schema["voice_generation_config"]["items"]["smart_switch_enabled"]["hint"],
        ...
        self.assertIn(
            "smart_switch_probability", schema["voice_generation_config"]["items"]
        )
        self.assertEqual(
            schema["voice_generation_config"]["items"]["smart_switch_probability"][
                "default"
            ],
            35,
        )
        ...
        self.assertEqual(
            emoji_items["collect_chat_emojis"]["description"], "自动收集表情"
        )
        self.assertEqual(emoji_items["max_ready"]["description"], "表情数量上限")
        self.assertEqual(
            emoji_items["max_size_mb"]["description"], "表情大小上限（MB）"
        )
        self.assertEqual(
        ...
        self.assertIn(
            "不代表一次发送这么多", emoji_items["send_candidate_limit"]["hint"]
        )
        self.assertEqual(
        ...
        self.assertIn(
            "待识别、待审核、识别失败、已拒绝、文件缺失",
            emoji_items["inactive_record_keep_days"]["hint"],
        )
        ...
        self.assertEqual(
            schema["state_config"]["items"]["enabled"]["description"], "启用实时状态"
        )
        ...
        self.assertEqual(
            schema["state_config"]["items"]["quiet_hours"]["description"], "静默时段"
        )
        self.assertEqual(
            schema["state_config"]["items"]["quiet_hours"]["default"], "00:00-06:30"
        )
        self.assertIn(
            "00:00-06:30", schema["state_config"]["items"]["quiet_hours"]["hint"]
        )
        ...
        search_items = schema["search_config"]["items"]
        self.assertNotIn("web_inspiration_config", schema)
        self.assertEqual(search_items["provider"]["_special"], "select_provider")
        self.assertNotIn("replace_builtin", search_items)
        self.assertNotIn("tavily_api_key", search_items)
        self.assertIn("tavily_api_keys", search_items)
        self.assertNotIn("deep_max_rounds", search_items)
        self.assertIn("deep_max_followups", search_items)
        self.assertEqual(search_items["deep_max_followups"]["default"], 2)
        self.assertEqual(search_items["deep_max_followups"]["slider"]["min"], 0)
        self.assertEqual(search_items["deep_max_followups"]["slider"]["max"], 3)
        self.assertIn("total_timeout_seconds", search_items)
        self.assertEqual(search_items["total_timeout_seconds"]["default"], 75)
        self.assertIn("today_prompt", search_items)
        self.assertIn("Tavily", schema["search_config"]["hint"])
        self.assertIn("Grok", schema["search_config"]["hint"])
        self.assertEqual(
            search_items["today_prompt"]["default"], DEFAULT_WEB_TODAY_PROMPT
        )
        self.assertIn("季节天气", search_items["today_prompt"]["default"])
        self.assertIn("出行或居家状态", search_items["today_prompt"]["default"])
        for variable in ("keyword", "category", "date", "persona", "today"):
            self.assertIn(f"{{{variable}}}", search_items["today_prompt"]["default"])
            self.assertIn(f"{{{variable}}}", search_items["today_prompt"]["hint"])
        self.assertEqual(
            search_items["today_prompt"]["hint"],
            "控制日程联网灵感搜索，支持 {keyword}、{category}、{date}、{persona}、{today} 自动替换。",
        )
    ...
    def test_search_does_not_read_removed_deep_rounds_setting(self):
        config = LifeSettings.from_dict({"search_config": {"deep_max_rounds": 3}})

        self.assertEqual(config.search.deep_max_followups, 2)
        self.assertFalse(hasattr(config.search, "deep_max_rounds"))
    ...
    def test_default_chat_prompt_avoids_unfounded_gender_guessing(self):
        ...
        self.assertIn(
            "优先使用最新且最具体的参考对象线索或用户自述", config.chat_prompt
        )
        self.assertIn(
            "昵称、头像、平台、语气、表情或刻板印象不能单独作为依据", config.chat_prompt
        )
    ...
    def test_new_memory_ui_text_has_no_encoding_corruption(self):
        ...
        dashboard_config = (
            PLUGIN_ROOT / "pages" / "dashboard" / "ui" / "settings.js"
        ).read_text(encoding="utf-8")
        runtime_chain = (
            PLUGIN_ROOT / "core" / "runtime" / "layer" / "chain.py"
        ).read_text(encoding="utf-8")

        self.assertTrue(
            all("?" * 3 not in value and chr(0xFFFD) not in value for value in texts)
        )
    ...
    def test_release_version_is_consistent(self):
        ...
        self.assertIn("数据库结构保持 v13", release_128)
