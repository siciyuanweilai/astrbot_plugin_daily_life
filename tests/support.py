...
def _install_stubs():
    ...
    class _At:
        ...
        def __init__(self, user_id="", target_user_id="", qq="", **kwargs):
            ...
            self.target_user_nickname = str(kwargs.get("target_user_nickname") or kwargs.get("name") or "")
    ...
    class _Reply:
        ...
        def __init__(self, message_id="", target_message_id="", id="", **kwargs):
            ...
            self.target_message_sender_id = str(kwargs.get("target_message_sender_id") or kwargs.get("sender_id") or "")
            self.target_message_sender_nickname = str(
                kwargs.get("target_message_sender_nickname") or kwargs.get("sender_name") or ""
            )
            self.target_message_sender_cardname = str(kwargs.get("target_message_sender_cardname") or "")
            self.target_message_content = str(kwargs.get("target_message_content") or kwargs.get("content") or "")
    ...
    class _ProviderRequest:
        def __init__(self, **kwargs):
            ...
            self.extra_user_content_parts = list(kwargs.get("extra_user_content_parts") or [])
    ...
    class _HtmlRenderer:
        ...
        async def render_t2i(self, text, use_network=True, return_url=False, template_name=None):
            self.calls.append(
                {
                    "text": text,
...
class DataManager:
    ...
    async def save_emotion_arc(self, arc):
        item = arc if isinstance(arc, EmotionArcRecord) else EmotionArcRecord.from_value(arc)
        if not item:
            return None
    ...
    def _decayed_emotion_arc(self, item):
        ...
        copied.intensity = max(0, min(round(item.intensity * min(1.0, remaining / span)), item.intensity))
        return copied
    ...
    async def get_emotion_arcs(self, limit=20, *, scope="", date="", active_only=True, include_global=True):
        ...
        if active_only:
            values = [item for item in values if item.status == "active" and item.intensity > 0]
    ...
    def _compact_single_active_rhythm_logs(self):
        values = [
            ...
            if item.status == "active" and item.lifecycle_kind in {"short_term", "sustained"}
        ]
        values.sort(key=lambda item: (item.date, item.updated_at, item.id), reverse=True)
        seen = set()
        compacted = 0
        for item in values:
    ...
    async def save_physiological_rhythm_log(self, record):
        item = record if isinstance(record, PhysiologicalRhythmLogRecord) else PhysiologicalRhythmLogRecord.from_value(record)
        if not item:
            return None
        now_text = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        if not item.id:
        ...
        if item.lifecycle_kind == "transient" and (
            item.body_intensity >= 35 or item.social_battery <= 40 or item.recovery_actions or item.optional_cycle_enabled
        ):
            item.lifecycle_kind = "short_term"
        item.weight = {"transient": 0.6, "short_term": 1.0, "sustained": 1.5}.get(item.lifecycle_kind, 0.6)
    ...
    async def get_physiological_rhythm_logs(self, limit=20, *, date="", active_only=True):
    ...
    async def get_physiological_rhythm_trend(self, days=7, *, limit=20):
        ...
        if not values:
            result = {"days": days, "logs": [], "summary": "", "updated_at": "", "trend_cache_key": cache_key}
        ...
        high_body_days = len({item.date for item in values if item.body_intensity >= 35})
        low_social_days = len({item.date for item in values if item.social_battery <= 40})
        parts = [f"近{days}天平均身体负荷 {avg_body:g}/100", f"平均社交电量 {avg_social:g}/100"]
        if high_body_days:
        ...
        result = {
            ...
            "cycle_labels": [],
            "summary": "；".join(parts),
            "updated_at": max((item.updated_at for item in values if item.updated_at), default=""),
    ...
    async def save_life_decision(self, decision):
        item = decision if isinstance(decision, LifeDecisionRecord) else LifeDecisionRecord.from_value(decision)
        if not item:
            return None
        if not item.id:
        ...
        query = " ".join(
            part for part in (item.decision, item.reason, item.evidence, item.outcome, item.subject) if part
        )
        if query:
    ...
    async def upsert_emoji_asset(self, asset):
        ...
        if existing:
            ...
            if existing.status in {"failed", "disabled", "rejected"} and item.status in {"pending", "reviewing"}:
    ...
    def _prune_video_insights(self, ttl_seconds=7200, max_items=60):
        ...
        if len(kept) > max_items:
            ordered = sorted(kept.items(), key=lambda pair: float(pair[1].get("updated_at") or 0), reverse=True)
    ...
    async def get_recent_video_insights(self, scope="", *, limit=3, ttl_seconds=7200, max_items=60):
        self._prune_video_insights(ttl_seconds, max_items)
        scope = self._text(scope)
        values = list(self.video_insights.values())
        if scope:
            values = [item for item in values if self._text((item.get("clip") or {}).get("scope")) == scope]
    ...
    async def upsert_long_term_memory(self, memory):
        item = memory if isinstance(memory, LongTermMemoryRecord) else LongTermMemoryRecord.from_value(memory)
        if not item:
            return None
        existing = next(
            (
    ...
    async def _maintain_long_term_memory_quality(self, item):
        if self._memory_lifecycle_kind(item.category) == "short_term" and not item.expires_at:
            item.expires_at = (datetime.datetime.now().date() + datetime.timedelta(days=7)).strftime("%Y-%m-%d")
            item.weight = max(float(item.weight or 1.0), 1.2)
        elif self._memory_lifecycle_kind(item.category) in {"long_term", "correction"}:
            item.weight = max(float(item.weight or 1.0), 2.0 if self._memory_lifecycle_kind(item.category) == "correction" else 1.5)
    ...
    async def _upsert_memory_entity(self, item):
        entity_type = "preference" if str(item.category).startswith("preference") else {
            "relationship": "person",
            "place": "place",
            "event": "event",
            "episode": "event",
        ...
        name = (item.title or str(item.content).replace("，", " ").replace("。", " ").split()[0])[:80]
        existing = next(
            (
                current
                for current in self.memory_entities.values()
                if current.scope == item.scope and current.entity_type == entity_type and current.name == name
            ),
            None,
        )
        if existing:
    ...
    async def _upsert_memory_cluster(self, item):
        ...
        existing = next(
            (
                ...
                if current.scope == item.scope and current.category == category and current.title == title
            ),
            None,
        )
        if existing:
    ...
    async def _record_memory_conflicts(self, item):
        if self._memory_lifecycle_kind(item.category) not in {"short_term", "correction"}:
            return []
        candidates = [
            other for other in self.long_term_memories.values()
            if other.id != item.id
            and other.status == "active"
            and other.scope == item.scope
            and (str(other.category).startswith("preference:") or other.category in {"relationship", "place"})
        ][:3]
        conflicts = []
        for other in candidates:
            conflict = MemoryConflictRecord(
                ...
                conflict_type="override" if self._memory_lifecycle_kind(item.category) == "correction" else "temporary_tension",
    ...
    def _long_term_category_match(self, item, categories):
        clean = {str(category or "").strip() for category in (categories or []) if str(category or "").strip()}
        return not clean or item.category in clean

    async def search_long_term_memories(self, query, scopes=None, categories=None, limit=5):
        normalized = (
            str(query or "")
            .replace("，", " ")
            .replace("。", " ")
        ...
        for item in self.long_term_memories.values():
            if item.status != "active" or not self._long_term_scope_match(item, scopes) or not self._long_term_category_match(item, categories):
                continue
            haystack = f"{item.title} {item.category} {item.content}"
            score = sum(1 for token in tokens if token in haystack)
            if score:
                cloned = LongTermMemoryRecord.from_value({**item.as_dict(), "score": float(score)})
                values.append(cloned)
        values.sort(key=lambda item: (item.score, item.weight, item.updated_at, item.id), reverse=True)
        return values[:limit] if limit > 0 else values

    async def list_recent_long_term_memories(self, scopes=None, categories=None, limit=8):
        values = [
            item
            for item in self.long_term_memories.values()
            if item.status == "active"
        ...
        values.sort(key=lambda item: (item.weight, item.updated_at, item.id), reverse=True)
        return values[:limit] if limit > 0 else values

    async def get_memory_episode_clusters(self, scopes=None, limit=8):
        values = [
            item for item in self.memory_episode_clusters.values()
            if item.status == "active" and self._long_term_scope_match(item, scopes)
        ]
        values.sort(key=lambda item: (item.weight, item.last_date, item.id), reverse=True)
        return values[:limit] if limit > 0 else values

    async def get_memory_entities(self, scopes=None, entity_type="", limit=12):
        values = [
            item for item in self.memory_entities.values()
            if item.status == "active"
            and self._long_term_scope_match(item, scopes)
            and (not entity_type or item.entity_type == entity_type)
        ]
        values.sort(key=lambda item: (item.mention_count, item.confidence, item.last_seen, item.id), reverse=True)
        return values[:limit] if limit > 0 else values

    async def get_memory_conflicts(self, scopes=None, status="open", limit=8):
        values = [
            item for item in self.memory_conflicts.values()
            if self._long_term_scope_match(item, scopes) and (not status or item.status == status)
        ]
        values.sort(key=lambda item: item.id, reverse=True)
        return values[:limit] if limit > 0 else values

    async def link_decision_memories(self, decision_id, memories, influence="", weight=1.0):
        links = []
        for memory in memories:
            memory_id = memory if isinstance(memory, int) else int(getattr(memory, "id", 0) or (memory or {}).get("id") or 0)
            if not memory_id:
                continue
            existing = next(
                (
                    current for current in self.memory_decision_links.values()
                    if current.decision_id == int(decision_id) and current.memory_id == int(memory_id)
                ),
                None,
            )
            if existing:
    ...
    async def get_memory_decision_links(self, decision_id, limit=8):
        values = [item for item in self.memory_decision_links.values() if item.decision_id == int(decision_id)]
    ...
    async def get_life_health_report(self, policy=None):
        ...
        checks = [
            ...
            {"key": "emotion_arcs", "label": "情绪脉络", "ok": bool(self.emotion_arcs), "count": len(self.emotion_arcs)},
            {"key": "physiological_rhythm_logs", "label": "生理节律", "ok": bool(self.physiological_rhythm_logs), "count": len(self.physiological_rhythm_logs)},
            {"key": "terms", "label": "语言", "ok": bool(self.life_terms), "count": len(self.life_terms)},
    ...
    async def cleanup_storage_category(self, category_key, keep_days=None):
        ...
        if retention > 0 and key == "daily":
            ...
            stale_emotions = [
                ...
                or (item.expires_at and item.expires_at < datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
            ]
            ...
            stale_rhythm_logs = [
                ...
                if item.status != "active" or self._older_than(item.date or item.updated_at, cutoff)
            ]
            ...
            stale_long_term = [
                ...
                if item.status != "active" or self._older_than(item.expires_at or "", cutoff)
            ]
            for memory_id in stale_long_term:
                self.long_term_memories.pop(memory_id, None)
            stale_memory_ids = set(stale_long_term)
            stale_clusters = [
                key for key, item in self.memory_episode_clusters.items()
                if item.status != "active" or self._older_than(item.last_date or item.updated_at, cutoff)
            ]
            for cluster_id in stale_clusters:
                self.memory_episode_clusters.pop(cluster_id, None)
            stale_conflicts = [
                key for key, item in self.memory_conflicts.items()
                if item.status != "open" or item.memory_id in stale_memory_ids or item.related_memory_id in stale_memory_ids
            ]
            ...
            stale_links = [
                ...
                if item.memory_id in stale_memory_ids or item.decision_id in stale_decisions
            ]
