...
def _install_stubs():
    ...
    modules["aiohttp"].ClientError = type("ClientError", (Exception,), {})
    modules["aiohttp"].ClientTimeout = lambda *args, **kwargs: types.SimpleNamespace(
    ...
    class _Node:
        ...
        def __init__(self, content, uin="0", name="", **kwargs):
            self.content = list(content or [])
            self.uin = str(uin or "0")
    ...
    class _At:
        ...
        def __init__(self, user_id="", target_user_id="", qq="", **kwargs):
            self.user_id = str(user_id or target_user_id or qq or "")
            self.qq = self.user_id
            self.target_user_id = self.user_id
            self.target_user_nickname = str(
                kwargs.get("target_user_nickname") or kwargs.get("name") or ""
            )

    class _Reply:
        type = "reply"

        def __init__(self, message_id="", target_message_id="", id="", **kwargs):
            self.message_id = str(message_id or target_message_id or id or "")
            self.id = self.message_id
            self.target_message_id = self.message_id
            self.target_message_sender_id = str(
                kwargs.get("target_message_sender_id") or kwargs.get("sender_id") or ""
            )
            self.target_message_sender_nickname = str(
                kwargs.get("target_message_sender_nickname")
                or kwargs.get("sender_name")
                or ""
            )
            self.target_message_sender_cardname = str(
                kwargs.get("target_message_sender_cardname") or ""
            )
            self.target_message_content = str(
                kwargs.get("target_message_content") or kwargs.get("content") or ""
            )
    ...
    class _Record:
        ...
        def __eq__(self, other):
            return (
                ...
                and other.get("type") == "record"
                and other.get("file") == self.file
            )
    ...
    class _HtmlRenderer:
        def __init__(self):
            self.calls = []
            self.result = "https://example.com/t2i.png"

        async def render_t2i(
            self, text, use_network=True, return_url=False, template_name=None
        ):
            self.calls.append(
                {
                    "text": text,
    ...
    modules["astrbot.api.event"].filter = types.SimpleNamespace(
        ...
        on_agent_done=lambda *args, **kwargs: lambda func: func,
        on_using_llm_tool=lambda *args, **kwargs: lambda func: func,
        on_llm_tool_respond=lambda *args, **kwargs: lambda func: func,
    ...
    modules["astrbot.api.message_components"].Node = _Node
    modules["astrbot.api.message_components"].Nodes = _Nodes
...
class ConversationManager:
    ...
    async def new_conversation(
        self, session_id, platform_id=None, content=None, title=None, persona_id=None
    ):
        self.calls.append(
            ("new_conversation", session_id, platform_id, content, title, persona_id)
        )
    ...
    async def update_conversation(
        ...
        self.current_ids[session_id] = (
            conversation_id or self.current_ids.get(session_id) or "current"
        )
...
class Context:
    def __init__(
        ...
        async def insert(
            ...
            self.message_history_manager.records.setdefault(
                (platform_id, user_id), []
            ).append(record)
            return record
        ...
        async def get(platform_id, user_id, page=1, page_size=200):
            return list(
                self.message_history_manager.records.get((platform_id, user_id), [])
            )
...
class DataManager:
    def __init__(self):
        ...
        self.emotion_arcs = {}
        self.next_emotion_arc_id = 1
        self.physiological_rhythm_logs = {}
        self.next_physiological_rhythm_log_id = 1
        self.physiological_rhythm_trend_revision = 0
        self.physiological_rhythm_trend_cache = {}
        self.reply_effects = {}
        self.next_reply_effect_id = 1
        self.life_decisions = {}
        self.next_life_decision_id = 1
        ...
        self.next_long_term_memory_id = 1
        self.memory_episode_clusters = {}
        self.next_memory_episode_cluster_id = 1
        self.memory_entities = {}
        self.next_memory_entity_id = 1
        self.memory_conflicts = {}
        self.next_memory_conflict_id = 1
    ...
    async def get_context_snapshot(
        self, *, max_summaries, experience_scope="", session_id=""
    ):
    ...
    async def get_future_memo_days(self, after_date, limit=8):
        max_rows = max(0, int(limit or 0))
        if max_rows <= 0:
            return []
        days = []
        for date_str in sorted(self.days):
            day = self.days[date_str]
            if date_str > after_date and str(getattr(day, "memo", "") or "").strip():
                days.append(day)
    ...
    async def get_week_plan(self, week_id):
        return self.week_plans.get(week_id)
    ...
    async def save_commitment(self, commitment):
        item = (
            commitment
            if isinstance(commitment, CommitmentRecord)
            else CommitmentRecord.from_value(commitment)
        )
        if not item:
    ...
    async def get_due_commitments(self, date_str, include_scheduled=False):
        ...
        return [
            ...
            and (
                item.trigger_date == date_str
                or (
                    not item.trigger_date
                    and item.time_window in {"next_chat", "next_time"}
                )
            )
        ]
    ...
    async def add_events(self, date_str, events):
        for event in events or []:
            item = (
                ...
                else EventRecord.from_value(event, date=date_str)
            )
    ...
    async def touch_places(self, date_str, places, source="daily"):
        for place in places or []:
            place = (
                place
                if isinstance(place, PlaceRecord)
                else PlaceRecord.from_value(place)
            )
            if not place:
                continue
            name = place.name
    ...
    async def get_recent_places(self, limit=10):
        ...
        values.sort(
            key=lambda item: (str(item.last_seen), int(item.visits)), reverse=True
        )
        return values[:limit] if limit > 0 else values
    ...
    async def touch_relationship(
        ...
        if subjective_tags:
            profile.subjective_tags = [
                self._text(item) for item in subjective_tags if self._text(item)
            ]
        profile.relationship_story = (
            self._text(relationship_story) or profile.relationship_story
        )
        ...
        if note:
            profile.notes.append(
                RelationshipNote(date=date_str, content=self._text(note), source=source)
            )
    ...
    async def touch_relationship_contact(
        ...
        *,
        ...
        contact_key = (
            key,
            str(contact_type or "unknown").strip(),
        ...
        if profile:
            profile.contacts = [
                item
                for item in self.relationship_contacts.values()
                if item.profile_id == key
            ]

    async def mark_relationship_contact_unreachable(
        self, target_scope, reason, *, contact_type="friend"
    ):
            ...
            if contact.target_scope == scope and contact.contact_type == kind:
                ...
                if profile:
                    profile.contacts = [
                        item
                        for item in self.relationship_contacts.values()
                        if item.profile_id == contact.profile_id
                    ]

    async def get_reachable_relationship_contacts(
        self, profile_id, *, contact_type="friend"
    ):
        key = str(profile_id or "").strip()
        kind = str(contact_type or "friend").strip()
        contacts = [
            ...
            for item in self.relationship_contacts.values()
            if item.profile_id == key
            and item.contact_type == kind
            and item.is_reachable
        ]
        contacts.sort(key=lambda item: str(item.last_seen), reverse=True)
        return contacts

    async def add_relationship_point(
        self, profile_id, content, date_str="", source="memory", weight=1.0
    ):
        key = str(profile_id or "").strip()
        ...
        if not profile:
            profile = RelationshipRecord(
                id=key, name=key, first_seen=date_str, last_seen=date_str, source=source
            )
            self.relationships[key] = profile
        ...
        profile.memory_points.append(
            RelationshipPoint(
                date=date_str, content=text, source=source, weight=float(weight or 0.0)
            )
        )
    ...
    async def get_recent_relationships(self, limit=8):
        ...
        values.sort(
            key=lambda item: (str(item.last_seen), int(item.interactions)), reverse=True
        )
        return values[:limit] if limit > 0 else values
    ...
    async def revise_relationship_profile(
        ...
        *,
        ...
        for point in relationship_points or []:
            ...
            profile.memory_points.append(
                RelationshipPoint(
                    date=date_str, content=text, source=source, weight=1.0
                )
            )
        profile.memory_points = profile.memory_points[-20:]
    ...
    async def save_chat_summary(self, summary):
        item = (
            summary
            if isinstance(summary, ChatSummaryRecord)
            else ChatSummaryRecord.from_value(summary)
        )
        if not item:
    ...
    async def get_recent_chat_summaries(self, limit=8):
        values = sorted(
            self.chat_summaries.values(), key=lambda item: item.id, reverse=True
        )
        return values[:limit] if limit > 0 else values
    ...
    async def save_group_environment(self, environment):
        item = (
            environment
            if isinstance(environment, GroupEnvironmentRecord)
            else GroupEnvironmentRecord.from_value(environment)
        )
        if not item:
    ...
    async def save_message_visibility(self, visibility):
        item = (
            visibility
            if isinstance(visibility, MessageVisibilityRecord)
            else MessageVisibilityRecord.from_value(visibility)
        )
        if not item:
    ...
    async def get_message_visibility_records(self, limit=20):
        values = sorted(
            self.message_visibility.values(), key=lambda item: item.id, reverse=True
        )
        return values[:limit] if limit > 0 else values
    ...
    async def save_action_decision(self, decision):
        item = (
            decision
            if isinstance(decision, ActionDecisionRecord)
            else ActionDecisionRecord.from_value(decision)
        )
        if not item:
    ...
    async def get_action_decision_records(self, limit=20):
        values = sorted(
            self.action_decisions.values(), key=lambda item: item.id, reverse=True
        )
        return values[:limit] if limit > 0 else values
    ...
    async def save_daily_review(self, review):
        item = (
            review
            if isinstance(review, DailyReviewRecord)
            else DailyReviewRecord.from_value(review)
        )
        if not item:
    ...
    async def get_recent_daily_reviews(self, limit=7):
        values = sorted(
            self.daily_reviews.values(), key=lambda item: item.date, reverse=True
        )
        return values[:limit] if limit > 0 else values
    ...
    async def upsert_preferences(self, preferences, date_str=""):
        ...
        for pref in preferences or []:
            item = (
                pref
                if isinstance(pref, PreferenceRecord)
                else PreferenceRecord.from_value(pref, date=date_str)
            )
            if not item:
                continue
    ...
    async def get_preferences(self, limit=20, category=""):
        ...
        values.sort(
            key=lambda item: (item.weight, item.last_seen, item.id), reverse=True
        )
        return values[:limit] if limit > 0 else values
    ...
    async def add_life_event(self, event):
        item = (
            event
            if isinstance(event, LifeEventRecord)
            else LifeEventRecord.from_value(event)
        )
        if not item:
            return None
        if not item.id:
    ...
    async def close_stale_life_events(self, before_date):
        expired = 0
        for item in self.life_events.values():
            if item.status == "open" and item.date and item.date < str(before_date):
                item.status = "expired"
                expired += 1
        return expired
    ...
    async def save_life_episode(self, episode):
        item = (
            episode
            if isinstance(episode, LifeEpisodeRecord)
            else LifeEpisodeRecord.from_value(episode)
        )
        if not item:
    ...
    async def save_memory_evidence(self, evidence):
        item = (
            evidence
            if isinstance(evidence, MemoryEvidenceRecord)
            else MemoryEvidenceRecord.from_value(evidence)
        )
        if not item:
            return None
        item.summary = self._text(item.summary)
        ...
        if self._is_life_decision_evidence(item):
            self._compact_life_decision_evidence(item.id)
        return item

    @staticmethod
    def _is_life_decision_evidence(item):
        return (
            str(getattr(item, "target_type", "") or "").strip() == "life_decision"
            and str(getattr(item, "evidence_type", "") or "").strip() == "decision"
        )

    def _compact_life_decision_evidence(self, keep_id):
        count = 0
        for evidence_id, item in list(self.memory_evidence.items()):
            if item.id != keep_id and self._is_life_decision_evidence(item):
    ...
    async def add_behavior_feedback(self, feedback):
        item = (
            feedback
            if isinstance(feedback, BehaviorFeedbackRecord)
            else BehaviorFeedbackRecord.from_value(feedback)
        )
        if not item:
            return None
        for existing in sorted(
            self.behavior_feedback.values(), key=lambda entry: entry.id, reverse=True
        ):
            if (
    ...
    async def save_emotion_arc(self, arc):
        item = (
            arc
            if isinstance(arc, EmotionArcRecord)
            else EmotionArcRecord.from_value(arc)
        )
        if not item:
            return None
        now_text = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        if not item.id:
            item.id = self.next_emotion_arc_id
            self.next_emotion_arc_id += 1
        ...
        self.emotion_arcs[item.id] = item
        self._compact_emotion_arcs(item.scope, item.label, item.id)
        return item

    @staticmethod
    def _parse_emotion_time(value):
    ...
    def _decayed_emotion_arc(self, item):
        ...
        if expires_at <= now:
            copied.intensity = 0
            copied.status = "expired"
            return copied
        span = max((expires_at - updated_at).total_seconds(), 1.0)
        remaining = max((expires_at - now).total_seconds(), 0.0)
        copied.intensity = max(
            0, min(round(item.intensity * min(1.0, remaining / span)), item.intensity)
        )
        return copied

    def _compact_emotion_arcs(self, scope, label, keep_id):
        scope = str(scope or "")
        label = str(label or "")
        if not label or not keep_id:
            return 0
        count = 0
        now_text = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        for item in self.emotion_arcs.values():
            if item.id == keep_id or item.status != "active":
                continue
            if item.scope == scope and item.label == label:
    ...
    async def get_emotion_arcs(
        self, limit=20, *, scope="", date="", active_only=True, include_global=True
    ):
        values = list(self.emotion_arcs.values())
        if scope:
            values = [
                item
                for item in values
                if item.scope == scope or (include_global and not item.scope)
            ]
        if date:
            values = [item for item in values if item.date == date]
        if active_only:
            values = [item for item in values if item.status == "active"]
        values = [self._decayed_emotion_arc(item) for item in values]
        if active_only:
            values = [
                item
                for item in values
                if item.status == "active" and item.intensity > 0
            ]
        values.sort(key=lambda item: (item.updated_at, item.id), reverse=True)
        return values[:limit] if limit > 0 else values

    def _compact_single_active_rhythm_logs(self):
        values = [
            item
            for item in self.physiological_rhythm_logs.values()
            if item.status == "active"
            and item.lifecycle_kind in {"short_term", "sustained"}
        ]
        values.sort(
            key=lambda item: (item.date, item.updated_at, item.id), reverse=True
        )
        seen = set()
        compacted = 0
        for item in values:
            if item.lifecycle_kind in seen:
                item.status = "superseded"
                item.weight = 0.0
                item.updated_at = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                compacted += 1
            else:
                seen.add(item.lifecycle_kind)
        return compacted

    async def save_physiological_rhythm_log(self, record):
        item = (
            record
            if isinstance(record, PhysiologicalRhythmLogRecord)
            else PhysiologicalRhythmLogRecord.from_value(record)
        )
        if not item:
            return None
        now_text = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        if not item.id:
            item.id = self.next_physiological_rhythm_log_id
            self.next_physiological_rhythm_log_id += 1
        if not item.created_at:
            item.created_at = now_text
        if not item.updated_at:
            item.updated_at = now_text
        if item.lifecycle_kind not in {"transient", "short_term", "sustained"}:
            item.lifecycle_kind = "transient"
        if item.lifecycle_kind == "transient" and (
            item.body_intensity >= 35
            or item.social_battery <= 40
            or item.recovery_actions
            or item.optional_cycle_enabled
        ):
            item.lifecycle_kind = "short_term"
        item.weight = {"transient": 0.6, "short_term": 1.0, "sustained": 1.5}.get(
            item.lifecycle_kind, 0.6
        )
        self.physiological_rhythm_logs[item.id] = item
        self._compact_single_active_rhythm_logs()
        self.physiological_rhythm_trend_revision += 1
        self.physiological_rhythm_trend_cache = {}
        return item

    async def get_physiological_rhythm_logs(
        self, limit=20, *, date="", active_only=True
    ):
        values = list(self.physiological_rhythm_logs.values())
        if date:
            values = [item for item in values if item.date == date]
        if active_only:
            values = [item for item in values if item.status == "active"]
        ...
        if active_only:
            ...
            visible = []
            for item in values:
                if item.lifecycle_kind in {"short_term", "sustained"}:
                    if item.lifecycle_kind in seen:
                        continue
                    seen.add(item.lifecycle_kind)
                visible.append(item)
    ...
    async def get_physiological_rhythm_trend(self, days=7, *, limit=20):
        days = max(int(days or 7), 1)
        limit = max(int(limit or 20), 1)
        cache_key = f"{datetime.datetime.now().date().isoformat()}:{days}:{limit}:{self.physiological_rhythm_trend_revision}"
        if cache_key in self.physiological_rhythm_trend_cache:
            return copy.deepcopy(self.physiological_rhythm_trend_cache[cache_key])
        values = await self.get_physiological_rhythm_logs(limit=limit, active_only=True)
        if not values:
            result = {
                "days": days,
                "logs": [],
            ...
            self.physiological_rhythm_trend_cache[cache_key] = copy.deepcopy(result)
            return result
        avg_body = round(sum(item.body_intensity for item in values) / len(values), 1)
        avg_social = round(sum(item.social_battery for item in values) / len(values), 1)
        high_body_days = len(
            {item.date for item in values if item.body_intensity >= 35}
        )
        low_social_days = len(
            {item.date for item in values if item.social_battery <= 40}
        )
        parts = [
        ...
        result = {
            "days": days,
            "logs": [item.as_dict() for item in values[:limit]],
            ...
            "summary": "；".join(parts),
            "updated_at": max(
                (item.updated_at for item in values if item.updated_at), default=""
            ),
            "trend_cache_key": cache_key,
        }
        self.physiological_rhythm_trend_cache[cache_key] = copy.deepcopy(result)
    ...
    async def save_reply_effect(self, effect):
        item = (
            effect
            if isinstance(effect, ReplyEffectRecord)
            else ReplyEffectRecord.from_value(effect)
        )
        if not item:
            return None
        if not item.id:
    ...
    async def expire_stale_reply_effects(
        self, max_age_seconds, *, evidence="闲时续话后一段时间内没有新的可见回应"
    ):
        now = datetime.datetime.now()
        expired = 0
    ...
    async def save_life_decision(self, decision):
        item = (
            decision
            if isinstance(decision, LifeDecisionRecord)
            else LifeDecisionRecord.from_value(decision)
        )
        if not item:
            return None
        if not item.id:
            item.id = self.next_life_decision_id
            self.next_life_decision_id += 1
        if not item.created_at:
            item.created_at = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        self.life_decisions[item.id] = item
        query = " ".join(
            part
            for part in (
                item.decision,
        ...
        if query:
            memories = await self.search_long_term_memories(query, limit=3)
            for memory in memories:
                await self.link_decision_memories(
                    item.id,
                    [memory],
    ...
    async def get_life_decisions(self, limit=20, *, kind="", date=""):
        values = list(self.life_decisions.values())
        if kind:
    ...
    async def update_life_decision_outcome(self, decision_id, outcome):
        item = self.life_decisions.get(int(decision_id or 0))
    ...
    async def get_memory_vectors(self, target_type, target_ids):
        return {
            target_id: dict(self.memory_vectors[(target_type, target_id)])
            for target_id in target_ids
            if (target_type, target_id) in self.memory_vectors
        }

    async def upsert_memory_vectors(self, items):
        count = 0
        for item in items or []:
            ...
            if not target_type or not target_id:
                continue
            self.memory_vectors[(target_type, target_id)] = dict(item)
            count += 1
        return count
    ...
    async def save_memory_correction(self, correction):
        item = (
            correction
            if isinstance(correction, MemoryCorrectionRecord)
            else MemoryCorrectionRecord.from_value(correction)
        )
        if not item:
            return None
        if not item.id:
    ...
    async def get_memory_corrections(
        self, limit=20, *, target_type="", target_id="", unapplied_only=False
    ):
    ...
    async def upsert_expression_profile(self, profile):
        item = (
            profile
            if isinstance(profile, ExpressionProfileRecord)
            else ExpressionProfileRecord.from_value(profile)
        )
        if not item:
            return None
    ...
    async def save_expression_review(self, review):
        item = (
            review
            if isinstance(review, ExpressionReviewRecord)
            else ExpressionReviewRecord.from_value(review)
        )
        if not item:
            return None
        if not item.id:
    ...
    async def upsert_behavior_pattern(self, pattern):
        item = (
            pattern
            if isinstance(pattern, BehaviorPatternRecord)
            else BehaviorPatternRecord.from_value(pattern)
        )
        if not item:
            return None
        ...
        if existing:
            existing.suggested_action = (
                item.suggested_action or existing.suggested_action
            )
    ...
    async def upsert_behavior_scene(self, scene):
        item = (
            scene
            if isinstance(scene, BehaviorSceneRecord)
            else BehaviorSceneRecord.from_value(scene)
        )
        if not item:
            return None
        existing = next(
    ...
    async def upsert_session_mid_summary(self, summary):
        item = (
            summary
            if isinstance(summary, SessionMidSummaryRecord)
            else SessionMidSummaryRecord.from_value(summary)
        )
        if not item:
            return None
    ...
    async def get_session_mid_summaries(self, limit=20, *, session_id=""):
        ...
        values.sort(
            key=lambda item: (item.message_count, item.updated_at), reverse=True
        )
        return values[:limit] if limit > 0 else values
    ...
    async def get_temporary_expression_states(
        self, limit=20, *, scope="", active_only=True
    ):
        ...
        values.sort(
            key=lambda item: (item.intensity, item.updated_at, item.id), reverse=True
        )
        return values[:limit] if limit > 0 else values
    ...
    async def upsert_focus_slot(self, slot):
        item = (
            slot
            if isinstance(slot, FocusSlotRecord)
            else FocusSlotRecord.from_value(slot)
        )
        if not item:
            return None
        existing = next(
    ...
    async def upsert_focus_target(self, target):
        item = (
            target
            if isinstance(target, FocusTargetRecord)
            else FocusTargetRecord.from_value(target)
        )
        if not item:
            return None
        existing = next(
    ...
    async def get_focus_targets(
        self, limit=20, enabled_only=True, include_expired=False
    ):
    ...
    async def save_expression_intent(self, intent):
        item = (
            intent
            if isinstance(intent, ExpressionIntentRecord)
            else ExpressionIntentRecord.from_value(intent)
        )
        if not item:
            return None
        if not item.id:
    ...
    async def upsert_emoji_asset(self, asset):
        item = (
            asset
            if isinstance(asset, EmojiAssetRecord)
            else EmojiAssetRecord.from_value(asset)
        )
        if not item:
            return None
        existing = next(
        ...
        if existing:
            ...
            if existing.status in {
                ...
                "disabled",
                "rejected",
            } and item.status in {"pending", "reviewing"}:
    ...
    async def get_emoji_asset_by_hash(self, file_hash):
        return next(
            (
                item
                for item in self.emoji_assets.values()
                if item.file_hash == file_hash
            ),
            None,
        )

    async def get_emoji_assets(self, limit=20, *, status="", sendable_only=False):
        values = list(self.emoji_assets.values())
        if status:
            values = [item for item in values if item.status == status]
        if sendable_only:
            values = [item for item in values if getattr(item, "sendable", True)]
    ...
    def _prune_video_insights(self, ttl_seconds=7200, max_items=60):
        now = time.time()
        ttl_seconds = max(60, int(ttl_seconds or 7200))
        max_items = max(8, int(max_items or 60))
        kept = {
            key: item
            for key, item in self.video_insights.items()
            if now - float(item.get("updated_at") or 0) <= ttl_seconds
        }
        ...
        deleted = len(self.video_insights) - len(kept)
        self.video_insights = kept
        return max(deleted, 0)

    async def upsert_video_insight(self, item, *, ttl_seconds=7200, max_items=60):
        if not isinstance(item, dict):
            return None
        clip = item.get("clip")
        if not isinstance(clip, dict):
            return None
        cache_key = self._text(item.get("cache_key") or item.get("key"))
        if not cache_key:
            return None
        ...
        payload.setdefault("frame_notes", [])
        payload.setdefault("transcript", "")
        payload.setdefault("transcript_source", "")
        payload.setdefault("note", "")
        payload.setdefault("note_source", "")
        payload.setdefault("metadata", {})
        self.video_insights[cache_key] = payload
        self._prune_video_insights(ttl_seconds, max_items)
        return copy.deepcopy(self.video_insights.get(cache_key))

    async def get_video_insight(self, cache_key, *, ttl_seconds=7200, max_items=60):
        self._prune_video_insights(ttl_seconds, max_items)
        value = self.video_insights.get(self._text(cache_key))
        return copy.deepcopy(value) if value else None

    async def get_recent_video_insights(
        self, scope="", *, limit=3, ttl_seconds=7200, max_items=60
    ):
        self._prune_video_insights(ttl_seconds, max_items)
        scope = self._text(scope)
        values = list(self.video_insights.values())
        if scope:
            values = [
                item
                for item in values
                if self._text((item.get("clip") or {}).get("scope")) == scope
            ]
        values.sort(key=lambda item: float(item.get("updated_at") or 0), reverse=True)
        return copy.deepcopy(values[: max(1, int(limit or 3))])

    async def delete_video_insights_for_message(self, scope, message_id):
        scope = self._text(scope)
        message_id = self._text(message_id)
        before = len(self.video_insights)
        self.video_insights = {
            key: item
            for key, item in self.video_insights.items()
            if not (
                self._text((item.get("clip") or {}).get("scope")) == scope
                and self._text((item.get("clip") or {}).get("message_id")) == message_id
            )
        }
        return before - len(self.video_insights)

    async def upsert_life_term(self, term):
        item = (
            term
            if isinstance(term, LifeTermRecord)
            else LifeTermRecord.from_value(term)
        )
        if not item:
            return None
    ...
    async def set_memory_boundary(self, boundary):
        item = (
            boundary
            if isinstance(boundary, MemoryBoundaryRecord)
            else MemoryBoundaryRecord.from_value(boundary)
        )
        if not item or MemoryBoundaryRecord.is_same_scope(
            item.source_scope, item.target_scope
        ):
            return None
        existing = next(
            (
                ...
                if current.source_scope == item.source_scope
                and current.target_scope == item.target_scope
            ),
            None,
        )
        if existing:
    ...
    async def get_memory_boundaries(self, limit=20, enabled_only=True):
        ...
        values = [
            item
            for item in values
            if not MemoryBoundaryRecord.is_same_scope(
                item.source_scope, item.target_scope
            )
        ]
        if enabled_only:
    ...
    async def get_memory_maintenance(self, limit=20):
        values = sorted(
            self.memory_maintenance.values(),
            key=lambda item: (item.date, item.id),
            reverse=True,
        )
        return values[:limit] if limit > 0 else values

    async def upsert_long_term_memory(self, memory):
        item = (
            memory
            if isinstance(memory, LongTermMemoryRecord)
            else LongTermMemoryRecord.from_value(memory)
        )
        if not item:
            return None
        existing = next(
            (
                current
                for current in self.long_term_memories.values()
                if current.scope == item.scope
                and current.category == item.category
                and current.content == item.content
                and current.source_table == item.source_table
        ...
        if existing:
            ...
            item.id = self.next_long_term_memory_id
            self.next_long_term_memory_id += 1
        self.long_term_memories[item.id] = item
        await self._maintain_long_term_memory_quality(item)
        return item

    async def upsert_long_term_memories(self, memories):
        saved = []
        for memory in memories:
            item = await self.upsert_long_term_memory(memory)
            if item:
                saved.append(item)
        return saved

    def _memory_lifecycle_kind(self, category):
        category = str(category or "").strip().lower()
        if category == "short_term" or category.startswith("focus"):
            return "short_term"
        if category == "correction" or "correction" in category:
            return "correction"
        if category.startswith("preference") or category in {
            "relationship",
            "place",
            "fact",
        }:
    ...
    async def _maintain_long_term_memory_quality(self, item):
        if (
            self._memory_lifecycle_kind(item.category) == "short_term"
            and not item.expires_at
        ):
            item.expires_at = (
                datetime.datetime.now().date() + datetime.timedelta(days=7)
            ).strftime("%Y-%m-%d")
            item.weight = max(float(item.weight or 1.0), 1.2)
        elif self._memory_lifecycle_kind(item.category) in {"long_term", "correction"}:
            item.weight = max(
                float(item.weight or 1.0),
                2.0
                if self._memory_lifecycle_kind(item.category) == "correction"
                else 1.5,
            )
        await self._upsert_memory_entity(item)
        await self._upsert_memory_cluster(item)
        await self._record_memory_conflicts(item)

    async def _upsert_memory_entity(self, item):
        entity_type = (
            "preference"
            if str(item.category).startswith("preference")
            else {
                "relationship": "person",
                "place": "place",
                "event": "event",
            ...
            }.get(str(item.category).split(":", 1)[0], "topic")
        )
        name = (
            item.title
            or str(item.content).replace("，", " ").replace("。", " ").split()[0]
        )[:80]
        existing = next(
            (
                current
                for current in self.memory_entities.values()
                if current.scope == item.scope
        ...
        if existing:
            ...
            existing.confidence = max(existing.confidence, item.confidence)
            existing.last_seen = item.date or existing.last_seen
            return existing
        entity = MemoryEntityRecord(
            id=self.next_memory_entity_id,
            scope=item.scope,
    ...
    async def _upsert_memory_cluster(self, item):
        if self._memory_lifecycle_kind(item.category) not in {"episode", "short_term"}:
            return None
        category = str(item.category or "life").split(":", 1)[0] or "life"
        title = (item.title or item.content[:60])[:120]
        existing = next(
            (
                current
                for current in self.memory_episode_clusters.values()
                if current.scope == item.scope
        ...
        if existing:
            existing.summary = item.content or existing.summary
            existing.memory_count += 1
            existing.weight = max(existing.weight, item.weight)
            existing.last_date = max(existing.last_date, item.date)
            return existing
        cluster = MemoryEpisodeClusterRecord(
            id=self.next_memory_episode_cluster_id,
            scope=item.scope,
            title=title,
        ...
        self.next_memory_episode_cluster_id += 1
        self.memory_episode_clusters[cluster.id] = cluster
        return cluster

    async def _record_memory_conflicts(self, item):
        if self._memory_lifecycle_kind(item.category) not in {
            "short_term",
            "correction",
        }:
            return []
        candidates = [
            other
            for other in self.long_term_memories.values()
            if other.id != item.id
            and other.status == "active"
            and other.scope == item.scope
            and (
                str(other.category).startswith("preference:")
                or other.category in {"relationship", "place"}
            )
        ][:3]
        conflicts = []
        for other in candidates:
            conflict = MemoryConflictRecord(
                id=self.next_memory_conflict_id,
                scope=item.scope,
                memory_id=item.id,
                related_memory_id=other.id,
                conflict_type="override"
                if self._memory_lifecycle_kind(item.category) == "correction"
                else "temporary_tension",
                summary=f"{item.content} / {other.content}"[:300],
                resolution="优先按时间范围和来源强度判断，不直接覆盖长期记忆。",
            )
            self.next_memory_conflict_id += 1
            self.memory_conflicts[conflict.id] = conflict
            conflicts.append(conflict)
        return conflicts

    def _long_term_scope_match(self, item, scopes):
        clean = {
            str(scope or "").strip()
            for scope in (scopes or [])
            if str(scope or "").strip()
        }
        return not clean or not item.scope or item.scope in clean

    def _long_term_category_match(self, item, categories):
        clean = {
            str(category or "").strip()
            for category in (categories or [])
            if str(category or "").strip()
        }
        return not clean or item.category in clean

    async def search_long_term_memories(
        self, query, scopes=None, categories=None, limit=5
    ):
        normalized = (
            str(query or "")
            .replace("，", " ")
        ...
        if not tokens and str(query or "").strip():
            tokens = [str(query or "").strip()]
        values = []
        for item in self.long_term_memories.values():
            if (
                item.status != "active"
                or not self._long_term_scope_match(item, scopes)
                or not self._long_term_category_match(item, categories)
            ):
                continue
            haystack = f"{item.title} {item.category} {item.content}"
            score = sum(1 for token in tokens if token in haystack)
            if score:
                cloned = LongTermMemoryRecord.from_value(
                    {**item.as_dict(), "score": float(score)}
                )
                values.append(cloned)
        values.sort(
            key=lambda item: (item.score, item.weight, item.updated_at, item.id),
            reverse=True,
        )
        return values[:limit] if limit > 0 else values

    async def list_recent_long_term_memories(
        self, scopes=None, categories=None, limit=8
    ):
        values = [
            item
            for item in self.long_term_memories.values()
            if item.status == "active"
            and self._long_term_scope_match(item, scopes)
            and self._long_term_category_match(item, categories)
        ]
        values.sort(
            key=lambda item: (item.weight, item.updated_at, item.id), reverse=True
        )
        return values[:limit] if limit > 0 else values

    async def get_memory_episode_clusters(self, scopes=None, limit=8):
        values = [
            item
            for item in self.memory_episode_clusters.values()
            if item.status == "active" and self._long_term_scope_match(item, scopes)
        ]
        values.sort(
            key=lambda item: (item.weight, item.last_date, item.id), reverse=True
        )
        return values[:limit] if limit > 0 else values

    async def get_memory_entities(self, scopes=None, entity_type="", limit=12):
        values = [
            item
            for item in self.memory_entities.values()
            if item.status == "active"
            and self._long_term_scope_match(item, scopes)
    ...
    async def get_memory_conflicts(self, scopes=None, status="open", limit=8):
        values = [
            item
            for item in self.memory_conflicts.values()
            if self._long_term_scope_match(item, scopes)
    ...
    async def link_decision_memories(
        self, decision_id, memories, influence="", weight=1.0
    ):
        links = []
        for memory in memories:
            memory_id = (
                ...
                else int(getattr(memory, "id", 0) or (memory or {}).get("id") or 0)
            )
            if not memory_id:
                continue
            existing = next(
                (
                    current
                    for current in self.memory_decision_links.values()
                    if current.decision_id == int(decision_id)
                    and current.memory_id == int(memory_id)
                ),
                None,
            )
            if existing:
                existing.influence = influence or existing.influence
                existing.weight = max(existing.weight, float(weight or 1.0))
                links.append(existing)
                continue
            link = MemoryDecisionLinkRecord(
                id=self.next_memory_decision_link_id,
            ...
            self.next_memory_decision_link_id += 1
            self.memory_decision_links[link.id] = link
            links.append(link)
        return links

    async def get_memory_decision_links(self, decision_id, limit=8):
        values = [
            item
            for item in self.memory_decision_links.values()
            if item.decision_id == int(decision_id)
        ]
        values.sort(key=lambda item: (item.weight, item.id), reverse=True)
        return values[:limit] if limit > 0 else values

    async def get_memory_decision_sources(self, decision_id, limit=5):
        result = []
        for link in await self.get_memory_decision_links(decision_id, limit=limit):
            memory = self.long_term_memories.get(link.memory_id)
            if not memory or memory.status != "active":
                continue
            data = memory.as_dict()
            data.update(link.as_dict())
            data["summary"] = link.influence or memory.title or memory.content[:80]
            result.append(data)
        return result
        ...
        checks = [
            {
                "key": "life_episodes",
                "label": "生活片段",
                "ok": bool(self.life_episodes),
                "count": len(self.life_episodes),
            },
            {
                "key": "memory_evidence",
                "label": "证据链",
                "ok": bool(self.memory_evidence),
                "count": len(self.memory_evidence),
            },
            {
                "key": "focus_targets",
                "label": "关注目标",
                "ok": bool(self.focus_targets),
                "count": len(self.focus_targets),
            },
            {
                "key": "behavior_feedback",
                "label": "行为反馈",
                "ok": bool(self.behavior_feedback),
                "count": len(self.behavior_feedback),
            },
            {
                "key": "emotion_arcs",
                "label": "情绪脉络",
                "ok": bool(self.emotion_arcs),
                "count": len(self.emotion_arcs),
            },
            {
                "key": "physiological_rhythm_logs",
                "label": "生理节律",
                "ok": bool(self.physiological_rhythm_logs),
                "count": len(self.physiological_rhythm_logs),
            },
            {
                "key": "life_terms",
                "label": "语言",
                "ok": bool(self.life_terms),
                "count": len(self.life_terms),
            },
            {
                "key": "memory_boundaries",
                "label": "记忆边界",
                "ok": True,
                "count": len(self.memory_boundaries),
            },
            {
                "key": "expression",
                "label": "表达习惯",
                "ok": bool(self.expression_profiles),
                "count": len(self.expression_profiles),
            },
            {
                "key": "patterns",
                "label": "行为模式",
                "ok": bool(self.behavior_patterns),
                "count": len(self.behavior_patterns),
            },
            {
                "key": "reply_effects",
                "label": "回复效果",
                "ok": bool(self.reply_effects),
                "count": len(self.reply_effects),
            },
            {
                "key": "scenes",
                "label": "行为场景",
                "ok": bool(self.behavior_scenes),
                "count": len(self.behavior_scenes),
            },
        ]
    ...
    def _table_counts(self):
        relationship_notes = sum(
            len(item.notes) for item in self.relationships.values()
        )
        relationship_points = sum(
            len(item.memory_points) for item in self.relationships.values()
        )
        review_points = sum(
            len(item.memory_points) for item in self.daily_reviews.values()
        )
        review_prefs = sum(
            len(item.preference_points) for item in self.daily_reviews.values()
        )
        return {
            "days": len(self.days),
            "timelines": sum(len(day.timeline) for day in self.days.values()),
            "outfit_history": sum(
                len(day.outfit_history) for day in self.days.values()
            ),
            ...
            "day_event_people": sum(
                len(event.people)
                for day in self.days.values()
                for event in day.new_events
            ),
            "week_plans": len(self.week_plans),
            "week_goals": sum(len(plan.goals) for plan in self.week_plans.values()),
            "week_hints": sum(
                len(plan.daily_hints) for plan in self.week_plans.values()
            ),
            "week_suggestions": sum(
                len(items)
                for plan in self.week_plans.values()
                for items in plan.suggested_activities.values()
            ),
            "commitments": len(self.commitments),
            "commitment_people": sum(
                len(item.people) for item in self.commitments.values()
            ),
            "day_commitments": sum(
                len(values) for values in self.day_commitments.values()
            ),
            ...
            "chat_summary_people": sum(
                len(item.people) for item in self.chat_summaries.values()
            ),
            ...
            "life_episode_people": sum(
                len(item.related_people) for item in self.life_episodes.values()
            ),
            "life_episode_places": sum(
                len(item.related_places) for item in self.life_episodes.values()
            ),
            "memory_evidence": len(self.memory_evidence),
            "behavior_feedback": len(self.behavior_feedback),
            "emotion_arcs": len(self.emotion_arcs),
            "physiological_rhythm_logs": len(self.physiological_rhythm_logs),
            "reply_effects": len(self.reply_effects),
            "life_decisions": len(self.life_decisions),
            ...
            "video_insight_details": sum(
                len(item.get("details") or []) for item in self.video_insights.values()
            ),
            "video_insight_frames": sum(
                len(item.get("frame_notes") or [])
                for item in self.video_insights.values()
            ),
            ...
            "memory_episode_clusters": len(self.memory_episode_clusters),
            "memory_episode_cluster_items": len(self.memory_episode_clusters),
            "memory_entities": len(self.memory_entities),
            "memory_entity_links": len(self.memory_entities),
            "memory_conflicts": len(self.memory_conflicts),
            "memory_decision_links": len(self.memory_decision_links),
    ...
    async def get_storage_categories(self, policy=None):
        ...
        for category in STORAGE_CATEGORIES.values():
            tables = [
                {"name": table, "rows": counts.get(table, 0)}
            ...
            groups = [
                {
                    ...
                    "label": group.label,
                    "total_rows": sum(
                        rows_by_table.get(table, 0) for table in group.tables
                    ),
                    "tables": [
    ...
    async def clear_storage_category(self, category_key):
        ...
        before_rows = next(
            (item["total_rows"] for item in before if item["key"] == key), 0
        )
        if key == "daily":
            self.days.clear()
            ...
            self.physiological_rhythm_logs.clear()
            self.physiological_rhythm_trend_revision += 1
            self.physiological_rhythm_trend_cache = {}
    ...
    async def cleanup_storage_category(self, category_key, keep_days=None):
        ...
        if not category:
            raise ValueError("存储分类不存在")
        retention = (
            self._storage_keep_days(category)
            if keep_days is None
            else max(int(keep_days), 0)
        )
        cutoff = (
        ...
        if retention > 0 and key == "daily":
            ...
            stale = [
                date for date in self.daily_reviews if self._older_than(date, cutoff)
            ]
            deleted = len(stale)
            for date in stale:
            ...
            old_events = [
                item for item in self.events if self._older_than(item.date, cutoff)
            ]
            self.events = [
                item for item in self.events if not self._older_than(item.date, cutoff)
            ]
            stale_life_events = [
                event_id
                ...
                and item.status
                in {"done", "closed", "cancelled", "expired", "resolved"}
            ]
            ...
            stale_summaries = [
                key
                for key, item in self.chat_summaries.items()
                if self._older_than(item.date, cutoff)
            ]
            ...
            stale_sessions = [
                key
                for key, item in self.session_mid_summaries.items()
            ...
            stale_episodes = [
                ...
                and item.status
                in {"done", "closed", "cancelled", "expired", "resolved", "corrected"}
            ]
            for episode_id in stale_episodes:
                self.life_episodes.pop(episode_id, None)
            stale_evidence = [
                key
                for key, item in self.memory_evidence.items()
                if self._older_than(item.date, cutoff)
            ]
            ...
            stale_feedback = [
                key
                for key, item in self.behavior_feedback.items()
                if self._older_than(item.date, cutoff)
            ]
            ...
            stale_emotions = [
                key
                for key, item in self.emotion_arcs.items()
                if item.status != "active"
                or self._older_than(item.date or item.updated_at, cutoff)
                or (
                    item.expires_at
                    and item.expires_at
            ...
            for arc_id in stale_emotions:
                self.emotion_arcs.pop(arc_id, None)
            stale_rhythm_logs = [
                key
                for key, item in self.physiological_rhythm_logs.items()
                if item.status != "active"
                or self._older_than(item.date or item.updated_at, cutoff)
            ]
            for rhythm_id in stale_rhythm_logs:
                self.physiological_rhythm_logs.pop(rhythm_id, None)
            stale_reply_effects = [
                key
                for key, item in self.reply_effects.items()
                if self._older_than(item.updated_at, cutoff)
            ]
            ...
            stale_decisions = [
                key
                for key, item in self.life_decisions.items()
                if self._older_than(item.date or item.created_at, cutoff)
            ]
            for decision_id in stale_decisions:
                self.life_decisions.pop(decision_id, None)
            stale_corrections = [
                key
            ...
            stale_patterns = [
                key
                for key, item in self.behavior_patterns.items()
                if self._older_than(item.last_seen, cutoff)
            ]
            for pattern_id in stale_patterns:
                self.behavior_patterns.pop(pattern_id, None)
            stale_scenes = [
                key
                for key, item in self.behavior_scenes.items()
                if self._older_than(item.last_seen, cutoff)
            ]
            ...
            stale_intents = [
                key
                for key, item in self.expression_intents.items()
                if self._older_than(item.created_at, cutoff)
            ]
            for intent_id in stale_intents:
                self.expression_intents.pop(intent_id, None)
            stale_maintenance = [
                key
                for key, item in self.memory_maintenance.items()
                if self._older_than(item.date, cutoff)
            ]
            ...
            stale_decision_links = [
                key
                for key, item in self.memory_decision_links.items()
            ...
            stale_reviews = [
                key
                for key, item in self.expression_reviews.items()
                if self._older_than(item.created_at, cutoff)
            ]
            ...
            stale_emoji = [
                key
                for key, item in self.emoji_assets.items()
                if item.status == "disabled"
                and self._older_than(item.updated_at, cutoff)
            ]
            ...
            stale_long_term = [
                ...
                if item.status != "active"
                or self._older_than(item.expires_at or "", cutoff)
            ]
            for memory_id in stale_long_term:
                self.long_term_memories.pop(memory_id, None)
            stale_memory_ids = set(stale_long_term)
            stale_clusters = [
                key
                for key, item in self.memory_episode_clusters.items()
                if item.status != "active"
                or self._older_than(item.last_date or item.updated_at, cutoff)
            ]
            for cluster_id in stale_clusters:
                self.memory_episode_clusters.pop(cluster_id, None)
            stale_conflicts = [
                key
                for key, item in self.memory_conflicts.items()
                if item.status != "open"
                or item.memory_id in stale_memory_ids
                or item.related_memory_id in stale_memory_ids
            ]
            for conflict_id in stale_conflicts:
                self.memory_conflicts.pop(conflict_id, None)
            stale_links = [
                key
                for key, item in self.memory_decision_links.items()
                if item.memory_id in stale_memory_ids
                or item.decision_id in stale_decisions
            ]
            ...
            stale_links = [
                date for date in self.day_commitments if self._older_than(date, cutoff)
            ]
            ...
            stale_commitments = [
                ...
                if item.status in {"done", "cancelled", "expired"}
                and self._older_than(item.completed_at or item.trigger_date, cutoff)
            ]
    ...
    async def cleanup_by_storage_policy(self, policy=None):
        ...
        for category in STORAGE_CATEGORIES.values():
            ...
            if keep_days > 0 and category.auto_cleanup:
                results.append(
                    await self.cleanup_storage_category(category.key, keep_days)
                )
        return {
            "results": results,
...
class Event:
    ...
    def chain_result(self, chain):
        return types.SimpleNamespace(
            chain=list(chain or []), result_content_type="LLM_RESULT"
        )
...
def make_composer(
    ...
    archive = DataManager()
    if seed_week_plan:
        archive.week_plans[get_week_id()] = WeekPlanRecord(
            week_id=get_week_id(),
            theme="自主生活周",
            goals=["根据近期状态自然安排"],
            daily_hints={},
