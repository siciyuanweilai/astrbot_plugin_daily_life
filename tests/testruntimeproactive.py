...
class RuntimeProactiveTest(ResponseGateRuntimeMixin, unittest.TestCase):
    ...
    def test_proactive_raw_group_switch_wins_over_stale_parsed_config(self):
        ...
        runtime.config = LifeSettings.from_dict(
            {"proactive_config": {"group_enabled": True, "private_enabled": True}}
        )
        runtime.raw_config = {
            "proactive_config": {
                "group_enabled": False,
        ...
        self.assertFalse(runtime._proactive_chat_enabled(True))
        self.assertTrue(runtime._proactive_chat_enabled(False))
        self.assertTrue(runtime._proactive_idle_enabled())

    def test_proactive_prunes_candidates_for_disabled_scope(self):
        runtime = DailyLifeRuntime.__new__(DailyLifeRuntime)
        runtime.config = LifeSettings.from_dict(
            {
                "proactive_config": {
    ...
    def test_proactive_rechecks_switch_before_sending(self):
        runtime = DailyLifeRuntime.__new__(DailyLifeRuntime)
        runtime.config = LifeSettings.from_dict(
            {"proactive_config": {"group_enabled": True}}
        )
        runtime.raw_config = {"proactive_config": {"group_enabled": True}}
        candidate = {
            "target_scope": "aiocqhttp:GroupMessage:20001",
            "message": "刚才那个话题",
            "message_id": "m-proactive-switch",
            "sender_id": "10001",
            "sender_name": "测试用户",
        ...
        runtime._proactive_idle_candidates = {"20001": candidate}
        sent = []

        async def evaluate(event, now=None):
            runtime.raw_config["proactive_config"]["group_enabled"] = False
            return {"should_reply": True, "decision": "reply", "reply_text": "接一句"}
        ...
        asyncio.run(
            runtime._evaluate_idle_candidate(
                "20001", candidate, datetime.datetime(2026, 7, 16, 22, 0)
            )
        )
    ...
    def test_response_gate_group_switch_bypasses_group_gate(self):
        ...
        self.assertEqual(decision["reason"], "未启用群聊随心回复")

    def test_response_gate_raw_group_switch_wins_over_stale_parsed_config(self):
        runtime = self._response_gate_runtime(
            {"response_gate_config": {"group_enabled": True}}
        )
        runtime.raw_config = {
            "response_gate_config": {
                "group_enabled": False,
        ...
        event = Event(
            unified_msg_origin="aiocqhttp:GroupMessage:20001",
            group_id="20001",
            message_id="m-group-raw-disabled",
        )
    ...
    def test_response_gate_uses_message_type_when_group_id_is_unavailable(self):
        runtime = self._response_gate_runtime(
            {
                "response_gate_config": {
                    "group_enabled": False,
        ...
        event = Event(
            unified_msg_origin="aiocqhttp:FriendMessage:20001",
            message_id="m-group-message-type",
        )
    ...
    def test_response_gate_private_switch_bypasses_private_gate(self):
        runtime = self._response_gate_runtime(
            {
                "response_gate_config": {
        ...
        event = Event(
            ...
            sender_id="10001",
            message_id="m-private-disabled",
        )
        event.message_str = "私聊普通聊一句"

        decision = asyncio.run(runtime.apply_response_gate_for_event(event))

        self.assertEqual(decision["action"], "reply")
        self.assertFalse(event.call_llm)
        self.assertEqual(decision["reason"], "未启用私聊随心回复")
    ...
    def test_response_gate_private_observe_records_user_context(self):
        ...
        self.assertEqual(
            history[-1]["content"][0], {"type": "text", "text": "我刚到家了"}
        )
    ...
    def test_response_gate_private_observe_records_user_image_context(self):
        ...
        self.assertIn(
            {"type": "text", "text": f"[Image Attachment: path {image_path}]"}, content
        )
        ...
        self.assertEqual(
            inserted.content["message"][0], {"type": "plain", "text": "怎么样？"}
        )
        self.assertEqual(inserted.content["message"][1]["type"], "image")
        self.assertEqual(
            inserted.content["message"][1]["filename"], "media_image_abc.png"
        )
        self.assertTrue(
            inserted.content["message"][1]["embedded_url"].startswith(
    ...
    def test_response_gate_private_message_can_reply(self):
        runtime = self._response_gate_runtime(
            {"response_gate_config": {"private_talk_frequency": 1.0}}
        )
        event = Event(
            unified_msg_origin="aiocqhttp:FriendMessage:10001",
            sender_id="10001",
    ...
    def test_response_gate_state_score_tracks_life_willingness(self):
        runtime = self._response_gate_runtime(
            {"response_gate_config": {"private_talk_frequency": 0.5}}
        )
        event = Event(
            unified_msg_origin="aiocqhttp:FriendMessage:10001",
            sender_id="10001",
        ...
        warm_score, _ = runtime._response_gate_score(
            event, warm_state, pending_count=1, now=now
        )
        cold_score, _ = runtime._response_gate_score(
            event, cold_state, pending_count=1, now=now
        )
    ...
    def test_response_gate_state_score_uses_physiological_rhythm(self):
        ...
        high_reasons = []
        high_delta = runtime._response_gate_state_delta(
            high_rhythm_state, False, high_reasons
        )
    ...
    def test_response_gate_private_question_has_structural_priority(self):
        runtime = self._response_gate_runtime(
            {"response_gate_config": {"private_talk_frequency": 0.35}}
        )
        now = datetime.datetime(2026, 5, 24, 12, 0)
        statement = Event(unified_msg_origin="aiocqhttp:FriendMessage:10001")
        statement.message_str = "然后继续"
        question = Event(unified_msg_origin="aiocqhttp:FriendMessage:10001")
        question.message_str = "然后干嘛？"

        statement_score, _ = runtime._response_gate_score(
            statement, None, pending_count=1, now=now
        )
        question_reasons = []
        question_score, question_reasons = runtime._response_gate_score(
            question, None, pending_count=1, now=now
        )

        self.assertGreater(question_score, statement_score)
        self.assertIn("私聊里有直接提问", question_reasons)

    def test_response_gate_attention_lease_reduces_other_chat_interruptions(self):
        runtime = self._response_gate_runtime({})
        now = datetime.datetime(2026, 5, 24, 12, 0)
        runtime._response_gate_note_attention(
            "private:10001", now, replied=True, priority=90
        )
        same_reasons = []
        other_reasons = []

        same = runtime._response_gate_attention_delta(
            "private:10001", now, same_reasons, is_group=False
        )
        other = runtime._response_gate_attention_delta(
            "group:20001", now, other_reasons, is_group=True
        )

        self.assertGreater(same, 0)
        self.assertLessEqual(other, -0.19)
        self.assertIn("注意力刚在别的会话里", other_reasons)
    ...
    def test_response_gate_backoff_observes_repeated_group_noise(self):
        ...
        first_decision = asyncio.run(
            runtime.evaluate_response_gate(
                first, now=datetime.datetime(2026, 5, 24, 12, 0)
            )
        )
        second_decision = asyncio.run(
            runtime.evaluate_response_gate(
                second, now=datetime.datetime(2026, 5, 24, 12, 0, 10)
            )
        )
    ...
    def test_response_gate_schema_is_configurable(self):
        ...
        self.assertFalse(config.response_gate.group_enabled)
        self.assertTrue(config.response_gate.private_enabled)
...
class RuntimeProactiveAsyncTest(
    ...
    async def test_regular_reply_effect_registers_watch_before_background_save(self):
        ...
        def schedule(coro, **kwargs):
            queued.append((coro, kwargs))
            return True

        runtime._schedule_background_task = schedule
        event = Event(message_id="m-regular-reply")

        scheduled = await runtime.note_regular_reply_effect(event)
        ...
        effects = await runtime.archive.get_reply_effects(limit=10)
        self.assertEqual(len(effects), 1)
        self.assertEqual(effects[0].source, "regular_reply")
    ...
    async def test_proactive_voice_sends_voice_without_duplicate_text(self):
        ...
        self._assert_last_assistant_history(
            runtime, "aiocqhttp:FriendMessage:10001", "我困啦"
        )
    ...
    async def test_proactive_voice_failure_falls_back_to_text(self):
        ...
        runtime.media = types.SimpleNamespace(
            voice=types.SimpleNamespace(synthesize=fail_voice)
        )
        runtime.archive = DataManager()
        runtime._apply_proactive_send_timing = lambda payload: async_return(None)
        runtime._send_proactive_emoji_if_needed = lambda scope, payload: async_return(
    ...
    async def test_directed_detection_accepts_configured_bot_aliases(self):
        runtime = DailyLifeRuntime.__new__(DailyLifeRuntime)
        runtime.config = LifeSettings.from_dict(
            {"bot_identity_aliases": ["小助手", "SweetBot"]}
        )
    ...
    async def test_evaluate_proactive_reply_can_generate_short_reply(self):
        ...
        await runtime.archive.save_day(
            DayRecord(
                ...
                timeline=[
                    TimelineItem(
                        time="19:20", activity="在群里看大家聊看展", status="轻松"
                    )
                ],
        ...
        life_decision_evidence = await runtime.archive.get_memory_evidence(
    ...
    async def test_proactive_reply_absorbs_used_short_term_focus(self):
        ...
        result = await runtime.evaluate_proactive_reply(
            event, now=datetime.datetime(2026, 5, 24, 21, 30)
        )
    ...
    async def test_proactive_prompt_includes_recent_conversation_context(self):
        ...
        await runtime.evaluate_proactive_reply(
    ...
    async def test_proactive_prompt_includes_air_state_and_target_candidates(self):
        ...
        runtime.note_proactive_activity(
            second, now=datetime.datetime(2026, 5, 24, 12, 2)
        )
        await runtime.evaluate_idle_proactive_candidates(
            now=datetime.datetime(2026, 5, 24, 12, 8)
        )
    ...
    async def test_proactive_reply_uses_local_candidate_target_when_model_omits_target(
        self,
    ):
        runtime, provider = self._make_proactive_runtime(
            [
        ...
        await runtime.archive.save_day(
            DayRecord(
                date="2026-05-24", state=LifeState(social=80, attention_openness=80)
            )
        )
        event = Event(
            sender_name="小夏",
        ...
        result = await runtime.evaluate_proactive_reply(
            event, now=datetime.datetime(2026, 5, 24, 12, 8)
        )
    ...
    async def test_proactive_readiness_allows_model_when_feedback_is_warm(self):
        ...
        await runtime.archive.save_day(
            DayRecord(
                date="2026-05-24", state=LifeState(social=72, attention_openness=70)
            )
        )
    ...
    async def test_evaluate_proactive_reply_respects_cooldown(self):
        ...
        runtime.composer = type(
            ...
            (),
            {
                ...
                "_call_llm_text": lambda self, provider, prompt, session_id, empty_retries=0, primary_provider_id="": (
                    async_return("{}")
                ),
        ...
        runtime._proactive_last_reply_at = {
            "10001": datetime.datetime(2026, 5, 24, 12, 0)
        }
        event = Event(
            sender_name="阿林",
            sender_id="u1",
        ...
        result = await runtime.evaluate_proactive_reply(
            event, now=datetime.datetime(2026, 5, 24, 12, 5)
        )
    ...
    async def test_evaluate_proactive_reply_supports_private_message(self):
        ...
        self.assertLess(
            provider.prompts[0].index("JSON 输出要求"),
    ...
    async def test_note_proactive_activity_skips_command_and_stopped_events(self):
        ...
        runtime.note_proactive_activity(
            command_event, now=datetime.datetime(2026, 5, 24, 12, 0)
        )
        runtime.note_proactive_activity(
            stopped_event, now=datetime.datetime(2026, 5, 24, 12, 0)
        )
    ...
    async def test_idle_proactive_candidate_sends_after_silence(self):
        ...
        await runtime.evaluate_idle_proactive_candidates(
            now=datetime.datetime(2026, 5, 24, 12, 10)
        )
        ...
        await runtime.evaluate_idle_proactive_candidates(
            now=datetime.datetime(2026, 5, 24, 12, 21)
        )
    ...
    async def test_idle_proactive_wait_keeps_candidate_and_delays_retry(self):
        ...
        await runtime.evaluate_idle_proactive_candidates(
            now=datetime.datetime(2026, 5, 24, 12, 6)
        )
        self.assertIn("10001", runtime._proactive_idle_candidates)
        self.assertEqual(runtime.context.sent_messages, [])

        await runtime.evaluate_idle_proactive_candidates(
            now=datetime.datetime(2026, 5, 24, 12, 9)
        )
        self.assertEqual(len(provider.prompts), 1)
        self.assertIn("10001", runtime._proactive_idle_candidates)

        await runtime.evaluate_idle_proactive_candidates(
            now=datetime.datetime(2026, 5, 24, 12, 17)
        )
        self.assertEqual(len(runtime.context.sent_messages), 1)
        self.assertEqual(
            runtime.context.sent_messages[0][1].items, ["下午场确实舒服点"]
        )
        self.assertEqual(runtime._proactive_idle_candidates, {})
    ...
    async def test_idle_proactive_observe_keeps_candidate_and_holds_retry(self):
        ...
        await runtime.evaluate_idle_proactive_candidates(
            now=base_time + datetime.timedelta(minutes=2)
        )

        self.assertEqual(len(provider.prompts), 1)
        self.assertIn("10001", runtime._proactive_idle_candidates)
        hold_until = runtime._proactive_idle_candidates["10001"].get(
            "observe_hold_until"
        )
        ...
        await runtime.evaluate_idle_proactive_candidates(
            now=base_time + datetime.timedelta(minutes=4)
        )
        ...
        await runtime.evaluate_idle_proactive_candidates(
            now=base_time + datetime.timedelta(minutes=8)
        )

        self.assertEqual(len(provider.prompts), 2)
        self.assertEqual(len(runtime.context.sent_messages), 1)
        self.assertEqual(
            runtime.context.sent_messages[0][1].items, ["then we can keep it simple"]
        )
        self.assertEqual(runtime._proactive_idle_candidates, {})
    ...
    async def test_proactive_observe_backoff_can_be_bypassed_by_new_messages(self):
        ...
        runtime.note_proactive_activity(
            event, now=base_time + datetime.timedelta(minutes=3)
        )
        await runtime.evaluate_idle_proactive_candidates(
        ...
        runtime.note_proactive_activity(
            event, now=base_time + datetime.timedelta(minutes=6)
        )
        await runtime.evaluate_idle_proactive_candidates(
            now=base_time + datetime.timedelta(minutes=8)
        )
        ...
        for offset in range(7, 10):
            ...
            runtime.note_proactive_activity(
                event, now=base_time + datetime.timedelta(minutes=offset)
            )
        await runtime.evaluate_idle_proactive_candidates(
            now=base_time + datetime.timedelta(minutes=11)
        )
    ...
    async def test_proactive_prompt_marks_full_timeline_as_background(self):
        ...
        runtime.note_proactive_activity(
            event, now=datetime.datetime(2026, 5, 24, 13, 40)
        )

        await runtime.evaluate_idle_proactive_candidates(
            now=datetime.datetime(2026, 5, 24, 13, 42)
        )
        ...
        self.assertLess(
            prompt.index("此刻活动："), prompt.index("全天日程背景（连续性参考）：")
        )
    ...
    async def test_proactive_reply_effect_records_positive_feedback(self):
        ...
        async def classify(*args, **kwargs):
            return {
                ...
                "continuity": 84,
                "friction": 0,
                "reason": "用户明确认可并继续当前互动",
            }

        runtime._classify_reply_effect = classify

        await runtime._observe_proactive_reply_effect(
            event, datetime.datetime(2026, 5, 24, 12, 5)
        )
    ...
    async def test_proactive_reply_effect_cools_air_only_on_confident_negative(self):
        runtime, _ = self._make_proactive_runtime([])
        key = "10001"
        event = Event(
            sender_id="u1",
            unified_msg_origin="aiocqhttp:GroupMessage:10001",
            group_id="10001",
            message_id="m-negative",
        )
        event.message_str = "这次接话让我不舒服"
        effect = await runtime.archive.save_reply_effect(
            ReplyEffectRecord(
        ...
        runtime._proactive_feedback_watch[key] = {
            "sent_at": datetime.datetime(2026, 5, 24, 12, 0),
            "target_scope": event.unified_msg_origin,
            "reply_text": "上一条闲时续话",
            "reply_effect_id": effect.id,
            "source": "proactive_reply",
        }
        ...
        state = runtime._proactive_air_state[key]
        self.assertEqual(state["last_effect"], "negative")
        self.assertGreater(state["silence_inertia"], 0)
        self.assertIsNotNone(state["backoff_until"])
        effects = await runtime.archive.get_reply_effects(10)
        self.assertEqual(effects[0].outcome, "negative")
    ...
    async def test_response_gate_relationship_delta_uses_familiarity(self):
        ...
        reasons: list[str] = []
        familiar_delta = await runtime._response_gate_relationship_delta(
            event, now, reasons
        )
    ...
    async def test_response_gate_semantic_wait_uses_accumulated_messages(self):
        ...
        scope = "aiocqhttp:FriendMessage:10001"
        now = datetime.datetime(2026, 5, 24, 12, 0)
        runtime._response_gate_mark_wait(
            scope,
            now,
            message_id="m-first",
            message_text="我先说前半句",
        ...
        event.message_str = "再补上后半句"
        wait_state = runtime._response_gate_wait_state(scope, now)
    ...
    async def test_stale_reply_effect_settlement_invokes_archive_expirer(self):
        ...
        async def expire(max_age_seconds):
            calls.append(max_age_seconds)
            return 4

        runtime.archive.expire_stale_reply_effects = expire

        settled = await runtime._settle_stale_reply_effects()

        self.assertEqual(settled, 4)
        self.assertEqual(calls, [runtime._AIR_FEEDBACK_WINDOW_SECONDS])

    async def test_embedding_ranking_is_semantic_and_reuses_candidate_vectors(self):
        ...
        runtime.context.get_all_embedding_providers = lambda: [embedding]
        runtime._schedule_background_task = None
        groups = {
            "memories": [
                ("memory:far", "不相关内容", "far"),
                ("memory:near", "语义贴近内容", "near"),
            ]
        }
    ...
    async def test_embedding_provider_selection_prefers_configured_provider(self):
        ...
        runtime, _ = self._make_proactive_runtime([])
        default = EmbeddingProvider("embedding-default")
        selected = EmbeddingProvider("embedding-selected")
        runtime.context.get_all_embedding_providers = lambda: [default, selected]
        runtime.config.memory.embedding_provider = "embedding-selected"

        self.assertIs(runtime._meaning_embedding_provider(), selected)

        runtime.config.memory.embedding_provider = "embedding-missing"
        self.assertIs(runtime._meaning_embedding_provider(), default)

        runtime.config.memory.embedding_provider = ""
        self.assertIs(runtime._meaning_embedding_provider(), default)
    ...
    async def test_response_gate_continues_after_proactive_reply_is_accepted(self):
        ...
        runtime.note_proactive_activity(
            accepted, now=datetime.datetime(2026, 5, 24, 12, 1)
        )
        self.assertEqual(runtime._response_gate_continuation_store(), {})
        ...
        self.assertEqual(third_decision["action"], "reply")
        self.assertTrue(third_decision["forced"])
        ...
        fourth.message_str = "fourth"

        fourth_decision = await runtime.evaluate_response_gate(
            fourth,
            now=datetime.datetime(2026, 5, 24, 12, 4),
        )

        self.assertEqual(fourth_decision["action"], "observe")
        self.assertFalse(fourth.call_llm)
        ...
        runtime._response_gate_mark_continuation(
            scope, live_now, reason="live continuation"
        )
    ...
    async def test_response_gate_does_not_continue_after_cold_proactive_ack(self):
        ...
        runtime.note_proactive_activity(
            event, now=datetime.datetime(2026, 5, 24, 12, 1)
        )
    ...
    async def test_response_gate_batch_pressure_and_presence_penalty(self):
        ...
        runtime._response_gate_first_seen_at[scope] = now - datetime.timedelta(
            seconds=120
        )

        single_score, _ = runtime._response_gate_score(
            event, LifeState(), pending_count=1, now=now
        )
        batch_score, batch_reasons = runtime._response_gate_score(
            event, LifeState(), pending_count=3, now=now
        )
        ...
        penalized_score, penalty_reasons = runtime._response_gate_score(
            event, LifeState(), pending_count=3, now=now
        )
    ...
    async def test_proactive_silence_inertia_and_send_timing(self):
        ...
        self.assertEqual(
            runtime._proactive_send_delay_seconds(
                {"send_timing": {"delay_seconds": 30}}
            ),
            12.0,
        )
        self.assertEqual(
            runtime._proactive_send_delay_seconds(
                {"send_timing": {"delay_seconds": "3.5"}}
            ),
            3.5,
        )
        self.assertEqual(runtime._proactive_send_delay_seconds({}), 0.0)

        with patch("core.runtime.delay.random.uniform", return_value=1.25):
            self.assertEqual(
                runtime._proactive_send_delay_seconds(
                    {"reply_text": "我听见了，不过现在真的快睡着了"}
                ),
                1.25,
            )

    async def test_proactive_send_keeps_single_message_when_reply_segmentation_disabled(
        self,
    ):
        runtime, _ = self._make_proactive_runtime([])
        ...
        self.assertEqual(len(runtime.context.sent_messages), 1)
        self.assertEqual(
            runtime.context.sent_messages[0][0], "aiocqhttp:FriendMessage:10001"
        )
        self.assertEqual(
    ...
    async def test_proactive_send_has_no_segment_delay_when_reply_segmentation_disabled(
        self,
    ):
        runtime, _ = self._make_proactive_runtime([])
        sleeps = []
        ...
        with (
            patch("core.runtime.delay.random.uniform", return_value=0.92),
            patch(
    ...
    async def test_proactive_send_does_not_use_framework_segmentation_when_master_disabled(
        self,
    ):
        runtime, _ = self._make_proactive_runtime(
            [],
    ...
    async def test_private_revisit_keeps_single_message_when_reply_segmentation_disabled(
        self,
    ):
        runtime, provider = self._make_proactive_runtime(
            [
    ...
    async def test_emoji_selection_uses_stable_candidate_order_for_model(self):
        ...
        self.assertIsNotNone(emoji)
        self.assertEqual(emoji.file_hash, "curious-8")
        self.assertEqual(len(provider.prompts), 1)
        self.assertIn('"id": 8', provider.prompts[0])
        self.assertNotIn('"id": 9', provider.prompts[0])
    ...
    async def test_emoji_selection_uses_recent_expression_intents(self):
        ...
        emoji = await runtime._select_emoji_asset_for_intent(
            {"emotion": "有点想接话", "send_emoji": True}, scope=scope
        )
    ...
    async def test_private_candidate_is_removed_after_normal_reply(self):
        ...
        self.assertIn(
            "aiocqhttp:FriendMessage:10001", runtime._proactive_idle_candidates
        )

        runtime.note_proactive_bot_reply(
            event, now=datetime.datetime(2026, 5, 24, 12, 1)
        )
    ...
    async def test_private_revisit_sends_message_to_private_target(self):
        ...
        self.assertEqual(
            inserted.content["message"],
            [{"type": "plain", "text": "刚想起你上次说的那个展，后来有新进展吗？"}],
        )
        self.assertEqual(
            inserted.content["text"], "刚想起你上次说的那个展，后来有新进展吗？"
        )
        ...
        self.assertNotIn("current_role=当前角色", provider.prompts[0])
        self.assertIn("最终可见输出只按要求的格式返回", provider.system_prompts[0])
        self.assertNotIn("隐藏推理", provider.system_prompts[0])
        ...
        self.assertNotIn("自然打字等待", provider.prompts[0])
            ...
            async def search_memory(self, payload):
                ...
                return type(
                    ...
                    (),
                    {
                        ...
                        "data": {
                            "memory_detail_list": [
                                {"memory_value": "旧水果记忆：我切好水果喊对方出来吃"}
                            ]
                        },
                        "error": "",
                    },
    ...
    async def test_private_revisit_prompt_marks_saved_pronouns_as_non_gender_evidence(
        self,
    ):
        runtime, provider = self._make_proactive_runtime(
            [
    ...
    async def test_private_revisit_marks_friend_contact_unreachable_after_not_friend_error(
        self,
    ):
        runtime, provider = self._make_proactive_runtime(
            [
        ...
        contacts = await runtime.archive.get_reachable_relationship_contacts(
            "10001", contact_type="friend"
        )
