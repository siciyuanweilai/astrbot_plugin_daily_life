...
from core.presets import DEFAULT_CATALOG_POOLS
...
class LifeArchiveSqliteTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_storage_categories_report_counts_and_clear_category(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            await archive.save_custom_catalog_item(
                CatalogItemRecord(category="daily_themes", text="轻松整理日")
            )
            await archive.touch_relationship(
                "u1",
                name="阿林",
            ...
            self.assertGreater(categories["workshop"]["total_rows"], 0)
    ...
    async def test_archive_schema_has_no_json_payload_columns(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for table in ("days", "week_plans", "events", "relationships", "custom_week_templates"):
                columns = {
                    row["name"]
    ...
    async def test_builtin_item_states_persist_in_sqlite(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            db_path = f"{tmpdir}/daily_life.db"
            item_id = builtin_entry_id(DEFAULT_CATALOG_POOLS["daily_themes"][0])
            archive = LifeArchive(db_path)

            ok = await archive.set_builtin_item_enabled("catalog", item_id, False, scope="daily_themes")
            archive.close()

            reopened = LifeArchive(db_path)
            states = await reopened.get_builtin_item_states("catalog", "daily_themes")

            self.assertTrue(ok)
            self.assertIn(item_id, states)
    ...
    async def test_custom_week_template_persists_as_structured_rows(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            archive = LifeArchive(f"{tmpdir}/daily_life.db")
            await archive.save_custom_week_template(
                WeekTemplateRecord(
                    ...
                    weight=0.3,
                    goals=["早睡", "低强度运动"],
                    daily_hints={"monday": "慢慢启动", "sunday": "整理状态"},
                    suggested_activities={"weekday": ["散步"], "weekend": ["整理房间"]},
                    tags=["恢复"],
                )
            )
            await archive.set_custom_week_template_enabled("light_recovery", False)
            await archive.set_custom_week_template_weight("light_recovery", 0.5)
            archive.close()

            reopened = LifeArchive(f"{tmpdir}/daily_life.db")
            active = await reopened.get_custom_week_templates()
            all_templates = await reopened.get_custom_week_templates(include_disabled=True)

            self.assertNotIn("light_recovery", active)
            template = all_templates["light_recovery"]
            self.assertEqual(template.name, "轻恢复周")
            self.assertEqual(template.weight, 0.5)
            self.assertEqual(template.goals, ["早睡", "低强度运动"])
            self.assertEqual(template.daily_hints["sunday"], "整理状态")
            self.assertEqual(template.suggested_activities["weekend"], ["整理房间"])
            self.assertEqual(template.tags, ["恢复"])
            reopened.close()
