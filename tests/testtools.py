...
class LifeToolsTest(unittest.TestCase):
    ...
    def test_home_scene_does_not_force_sleep_style_pool(self):
        ...
        self.assertEqual(style_pool_for_scene_category("public"), "outfit_styles")

    def test_outfit_style_pool_preserves_actual_clothes_across_home_arrival(self):
        self.assertEqual(
            resolve_outfit_style_pool(
                "home",
                decision="keep",
                requested="sleep_styles",
                current="outfit_styles",
            ),
            "outfit_styles",
        )
        self.assertEqual(
        ...
        self.assertEqual(
            resolve_outfit_style_pool("home", decision="sleepwear"),
            "sleep_styles",
        )
        self.assertEqual(
            resolve_outfit_style_pool("home", decision="outdoor"),
            "outfit_styles",
        )
    ...
    def test_future_outfit_timing_ignores_generic_sleepwear_category(self):
        issue = future_outfit_timing_issue(
            "奶白色云朵绒两件套睡衣，头发用抓夹松松盘起",
            [
                {
                    "time": "21:00",
                    "activity": "洗完澡换上另一套更轻薄的奶白色纯棉短袖睡衣",
                    "status": "困倦",
                }
            ],
    ...
    def test_future_outfit_timing_ignores_bare_category_change(self):
        issue = future_outfit_timing_issue(
            "奶油色睡衣，披着薄毯坐在床边",
            [{"time": "21:00", "activity": "洗完澡换上另一套睡衣", "status": "困倦"}],
            current_minutes=2 * 60,
        )
    ...
    def test_week_plan_daily_helpers_support_date_and_weekday_keys(self):
        ...
        self.assertEqual(
            resolve_daily_hint(plan, datetime.datetime(2026, 5, 24)), "完整日期提示"
        )
        self.assertEqual(
            resolve_daily_suggested(plan, datetime.datetime(2026, 5, 24)), "约下午茶"
        )
        self.assertEqual(
            resolve_daily_hint(plan, datetime.datetime(2026, 5, 25)), "月日提示"
        )
        self.assertEqual(
            resolve_daily_suggested(plan, datetime.datetime(2026, 5, 25)), "整理书桌"
        )
        self.assertEqual(
            resolve_daily_hint(plan, datetime.datetime(2026, 5, 31)), "周末提示"
        )

    def test_world_normalizers_use_current_field_names(self):
        self.assertEqual(
            normalize_place_names(
                [{"name": "常去咖啡店"}, {"place": "旧地点字段"}, "字符串地点"]
            ),
            ["常去咖啡店", "字符串地点"],
        )
        self.assertEqual(
