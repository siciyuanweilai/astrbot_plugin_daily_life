...
class RuntimeMemoryTest(unittest.TestCase):
    def test_snapshot_awareness_never_falls_back_to_another_scope(self):
        runtime = DailyLifeRuntime.__new__(DailyLifeRuntime)
        runtime._event_session_id = lambda _event: "session-current"
        runtime._event_group_meta = lambda _event: ("group-current", "当前群")
        current = SimpleNamespace(
            session_id="session-current", group_id="group-current"
        )
        other = SimpleNamespace(session_id="session-other", group_id="group-other")

        scoped = runtime._scope_snapshot_items(
            {
                "environments": [other, current],
                "decisions": [other],
                "visibility": [other],
            },
            object(),
    ...
    def test_hidden_world_memory_includes_pronoun_boundary(self):
        ...
        from core.facts import person_fact_context_from_relationships
        ...
        person_context = person_fact_context_from_relationships(
            relationships=[
                type(
                    "Relationship",
                    (),
                    {"id": "10001", "name": "小林", "persona_hint": ""},
                )()
            ]
...
class RuntimeMemoryAsyncTest(RuntimeAsyncHelperMixin, unittest.IsolatedAsyncioTestCase):
    ...
    async def test_relationship_calibration_rewrites_conflicting_saved_profile(self):
        ...
        self.assertIn(
            "对方在人设中的线索：我是女生。阿林是我的男死党", provider.prompts[0]
        )

    async def test_relationship_calibration_keeps_profile_when_llm_finds_no_conflict(
        self,
    ):
        provider = Provider(
    ...
    async def test_private_memory_awareness_does_not_save_group_environment(self):
        ...
        self.assertEqual(
            (await runtime.archive.get_recent_message_visibility(10))[0].reason,
            "私聊消息",
        )
        self.assertEqual(
            (await runtime.archive.get_recent_action_decisions(10))[0].reason,
            "只是私聊",
        )
    ...
    async def test_memory_awareness_keeps_llm_text_without_cleanup(self):
        ...
        evidence = (
            await runtime.archive.get_memory_evidence(
                target_type="relationship", limit=10
            )
        )[0]
