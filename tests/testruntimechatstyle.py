...
class RuntimeChatStyleTest(unittest.TestCase):
    def test_inline_markdown_cleanup_unwraps_chat_emphasis_only(self):
        text = (
            "**5 月 8 日已经复服了**。"
            "还有 *补充*、_斜体_、__重点__ 和 ~~旧说法~~。"
            "file_name、a_b、2 * 3 * 4 保持不变。"
        )

        cleaned = DailyLifeRuntime._chat_style_clean_inline_markdown(text)
    ...
    def test_inline_markdown_cleanup_preserves_code_links_and_unclosed_markers(self):
        text = (
            "这是 **重点**，`**代码**` 不动，"
            "[**链接标题**](https://example.com/a_(b)) 不动，未闭合 **原样。"
        )
    ...
    def test_plain_chat_cleanup_runs_before_segmentation(self):
        runtime = DailyLifeRuntime.__new__(DailyLifeRuntime)
        runtime.config = LifeSettings.from_dict({"chat_style_config": {}})
        runtime.context = Context(
            Provider([]), config={"t2i": True, "t2i_word_threshold": 120}
        )
    ...
    def test_plain_chat_cleanup_skips_structural_media_and_disabled_expression(self):
        structural_runtime = DailyLifeRuntime.__new__(DailyLifeRuntime)
        structural_runtime.config = LifeSettings.from_dict({"chat_style_config": {}})
        structural_event = Event(unified_msg_origin="aiocqhttp:FriendMessage:10001")
        structural_text = "# 标题\n\n- **第一项**"
        structural_event.set_result(
            structural_event.chain_result([types.SimpleNamespace(text=structural_text)])
        )
        media_event = Event(unified_msg_origin="aiocqhttp:FriendMessage:10001")
        media_event.set_result(
            media_event.chain_result(
                [
                    types.SimpleNamespace(text="**图片说明**"),
                    {"type": "image", "url": "https://example.com/a.png"},
                ]
            )
        )
        disabled_runtime = DailyLifeRuntime.__new__(DailyLifeRuntime)
        disabled_runtime.config = LifeSettings.from_dict(
            {"chat_style_config": {"enabled": False}}
        )
        disabled_event = Event(unified_msg_origin="aiocqhttp:FriendMessage:10001")
        disabled_event.set_result(
            disabled_event.chain_result([types.SimpleNamespace(text="**保留原文**")])
        )

        self.assertFalse(
            structural_runtime.apply_chat_plain_text_cleanup_before_send(
                structural_event
            )
        )
        self.assertFalse(
            structural_runtime.apply_chat_plain_text_cleanup_before_send(media_event)
        )
        self.assertFalse(
            disabled_runtime.apply_chat_plain_text_cleanup_before_send(disabled_event)
        )
        self.assertEqual(structural_event.get_result().chain[0].text, structural_text)
        self.assertEqual(media_event.get_result().chain[0].text, "**图片说明**")
        self.assertEqual(disabled_event.get_result().chain[0].text, "**保留原文**")

    def test_plain_chat_cleanup_skips_text_to_image_candidate(self):
        runtime = DailyLifeRuntime.__new__(DailyLifeRuntime)
        runtime.config = LifeSettings.from_dict({"chat_style_config": {}})
        runtime.context = Context(
    ...
    def test_chat_style_master_switch_disables_the_entire_expression_pipeline(self):
        ...
        runtime.config = LifeSettings.from_dict(
            {
                ...
                "chat_style_config": {
                    "enabled": False,
                    "casual_short_prompt": "short replies",
                    "casual_max_chars": 10,
        ...
        event.set_result(
            types.SimpleNamespace(chain=[types.SimpleNamespace(text=original)])
        )

        self.assertFalse(runtime._chat_style_enabled())
        self.assertFalse(runtime._semantic_segment_enabled())
        self.assertEqual(runtime.build_hidden_chat_style_hint(), "")
        self.assertEqual(
            asyncio.run(runtime.build_chat_style_injection_context(event)), ""
        )
        self.assertEqual(runtime._chat_style_limit_for_event(event), 0)
        self.assertEqual(
            runtime._chat_style_limit_for_scope(event.unified_msg_origin), 0
        )
        self.assertEqual(runtime._proactive_reply_limit_for_prompt(), 80)
        self.assertEqual(
            runtime._format_proactive_expression_style(event.unified_msg_origin), ""
        )
        self.assertEqual(
            runtime._chat_style_natural_segment_delay_seconds("hello", "world"),
            0.0,
        )
        self.assertFalse(runtime.apply_chat_style_before_send(event))
        self.assertEqual(event.get_result().chain[0].text, original)
    ...
    def test_normalize_proactive_reply_text_keeps_complete_sentence_sequence(self):
        runtime = DailyLifeRuntime.__new__(DailyLifeRuntime)
        runtime.config = LifeSettings.from_dict(
            {"chat_style_config": {"proactive_max_chars": 20}}
        )
    ...
    def test_normalize_proactive_reply_text_does_not_cut_at_first_sentence(self):
        runtime = DailyLifeRuntime.__new__(DailyLifeRuntime)
        runtime.config = LifeSettings.from_dict(
            {"chat_style_config": {"proactive_max_chars": 15}}
        )
    ...
    def test_chat_style_injection_uses_semantic_expression_hint(self):
        runtime = DailyLifeRuntime.__new__(DailyLifeRuntime)
        ...
        self.assertIn("[HiddenChatStyle]", text)
        self.assertIn("普通聊天使用纯文本，不使用 Markdown", text)
        self.assertNotIn("[HiddenFactCheckSearch]", text)
        self.assertFalse(hasattr(event, "_daily_life_fact_check_used"))

    def test_semantic_segment_injection_uses_scene_length_hint(self):
        runtime = DailyLifeRuntime.__new__(DailyLifeRuntime)
        runtime.context = Context(Provider([]))
        runtime.config = LifeSettings.from_dict(
            {
    ...
    def test_chat_style_before_send_keeps_full_private_reply(self):
        ...
        setattr(
            event,
            "_daily_life_chat_style_decision",
            {"kind": "casual", "scope": "private"},
        )
    ...
    def test_chat_style_before_send_segments_plain_fact_reply_without_type_classification(
        self,
    ):
        runtime = DailyLifeRuntime.__new__(DailyLifeRuntime)
        runtime.config = LifeSettings.from_dict(
    ...
    def test_chat_style_before_send_uses_builtin_sentence_boundaries(self):
        runtime = DailyLifeRuntime.__new__(DailyLifeRuntime)
        runtime.config = LifeSettings.from_dict(
            {"chat_style_config": {"private_casual_max_chars": 120}}
        )
        event = Event(unified_msg_origin="aiocqhttp:FriendMessage:10001")
        ...
        self.assertTrue(changed)
        segments = [item.text for item in event.get_result().chain]
        self.assertEqual(segments[0], "还醒着。")
        self.assertEqual("".join(segments), reply)

    def test_chat_style_before_send_respects_explicit_line_breaks_for_short_private_reply(
        self,
    ):
        runtime = DailyLifeRuntime.__new__(DailyLifeRuntime)
        runtime.config = LifeSettings.from_dict({"chat_style_config": {}})
        ...
        self.assertTrue(changed)
        segments = [item.text for item in event.get_result().chain]
        self.assertEqual(segments[0], "我听见了。")
        self.assertEqual("".join(segments), reply.replace("\n", ""))
    ...
    def test_chat_style_before_send_segments_by_soft_pauses(self):
        runtime = DailyLifeRuntime.__new__(DailyLifeRuntime)
        runtime.config = LifeSettings.from_dict(
            {"chat_style_config": {"private_casual_max_chars": 80}}
        )
        event = Event(unified_msg_origin="aiocqhttp:FriendMessage:10001")
        event.message_str = "还醒着吗"
    ...
    def test_chat_style_before_send_splits_short_lead_soft_pause(self):
        ...
        event.set_result(
            types.SimpleNamespace(
                chain=[types.SimpleNamespace(text="你呢，今天不出去浪")]
            )
        )
        setattr(
            event,
        ...
        self.assertFalse(changed)
        self.assertEqual(event.get_result().chain[0].text, "你呢，今天不出去浪")
    ...
    def test_chat_style_before_send_limits_natural_segments(self):
        ...
        runtime.config = LifeSettings.from_dict(
            {
                "chat_style_config": {
                    "casual_max_chars": 120,
                    "private_casual_max_chars": 120,
                }
            }
        )
        event = Event(unified_msg_origin="aiocqhttp:FriendMessage:10001")
    ...
    def test_chat_style_before_send_allows_five_segments_for_long_break_rich_reply(
        self,
    ):
        runtime = DailyLifeRuntime.__new__(DailyLifeRuntime)
        runtime.config = LifeSettings.from_dict({"chat_style_config": {}})
    ...
    def test_chat_style_before_send_prefers_two_segments_for_medium_three_beat_reply(
        self,
    ):
        runtime = DailyLifeRuntime.__new__(DailyLifeRuntime)
        runtime.config = LifeSettings.from_dict({"chat_style_config": {}})
    ...
    def test_chat_style_before_send_splits_short_reaction_three_complete_sentences(
        self,
    ):
        runtime = DailyLifeRuntime.__new__(DailyLifeRuntime)
        runtime.config = LifeSettings.from_dict({"chat_style_config": {}})
    ...
    def test_chat_style_before_send_merges_tiny_reaction_then_splits_bridge_clause(
        self,
    ):
        runtime = DailyLifeRuntime.__new__(DailyLifeRuntime)
        runtime.config = LifeSettings.from_dict({"chat_style_config": {}})
    ...
    def test_chat_style_before_send_splits_reaction_and_soft_condition_tail(self):
        ...
        reply = (
            "你想得美。我只是在考虑，你要是再不回来，属于你的那瓶酸奶就要被我喝掉了。"
        )
        event.set_result(
            types.SimpleNamespace(chain=[types.SimpleNamespace(text=reply)])
        )
    ...
    def test_chat_style_before_send_does_not_split_inside_quotes(self):
        ...
        self.assertFalse(
            any(
                "“垃圾袋扎好了" in segment and "速去”" not in segment
    ...
    def test_chat_style_natural_segments_keep_english_space(self):
        runtime = DailyLifeRuntime.__new__(DailyLifeRuntime)
        runtime.config = LifeSettings.from_dict(
            {"chat_style_config": {"private_casual_max_chars": 90}}
        )
        event = Event(unified_msg_origin="aiocqhttp:FriendMessage:10001")
        ...
        self.assertIn("prompt 先别改太满", event.get_result().chain[0].text)
    ...
    def test_chat_style_natural_segments_keep_protected_spans_intact(self):
        ...
        self.assertFalse(
            any("`a" in segment and "b`" not in segment for segment in segments)
        )
        self.assertFalse(
            any(
                "https://example.com/a" in segment and url not in segment
                for segment in segments
            )
        )
    ...
    def test_chat_style_before_send_records_segment_plan(self):
        ...
        event.message_str = "听见了吗"
        reply = (
            "我听见了。不过现在真的快睡着了，你也别熬太狠。明天还要带我去买草莓蛋糕呢。"
        )
        event.set_result(
            types.SimpleNamespace(chain=[types.SimpleNamespace(text=reply)])
        )
        ...
        self.assertIn(pending[0].reason, {"完整句停顿", "语义软停顿", "短气口停顿"})
    ...
    def test_chat_style_before_send_segments_do_not_share_component_state(self):
        ...
        reply = (
            "我都闭眼了……眼皮重得像挂了秤砣。你赶紧数羊去，再吵我真装听不见了，晚安。"
        )
    ...
    def test_chat_style_before_send_keeps_multi_text_casual_reply(self):
        runtime = DailyLifeRuntime.__new__(DailyLifeRuntime)
        runtime.config = LifeSettings.from_dict(
            {"chat_style_config": {"private_casual_max_chars": 40}}
        )
        event = Event(unified_msg_origin="aiocqhttp:FriendMessage:10001")
        event.message_str = "想吃什么"
        event.set_result(
    ...
    def test_natural_segments_clean_punctuation_under_t2i_threshold(self):
        runtime = DailyLifeRuntime.__new__(DailyLifeRuntime)
        runtime.context = Context(
            Provider([]),
            config={"t2i": False, "t2i_word_threshold": 120},
        )
        runtime.config = LifeSettings.from_dict(
            {"chat_style_config": {"private_casual_max_chars": 30}}
        )
        event = Event(unified_msg_origin="aiocqhttp:FriendMessage:10001")
        event.message_str = "在干嘛呢"
        reply = "先去喝绿豆沙啊，正餐回头再看。附近那家云吞面行不行？"
        event.set_result(
            types.SimpleNamespace(chain=[types.SimpleNamespace(text=reply)])
        )
    ...
    def test_chat_style_before_send_uses_plugin_segments_when_astrbot_segment_enabled(
        self,
    ):
        runtime = DailyLifeRuntime.__new__(DailyLifeRuntime)
        runtime.context = Context(
        ...
        runtime.config = LifeSettings.from_dict(
            {"chat_style_config": {"private_casual_max_chars": 20}}
        )
        event = Event(unified_msg_origin="aiocqhttp:FriendMessage:10001")
        event.message_str = "详细说说"
        reply = (
    ...
    def test_chat_style_before_send_refines_multi_text_comma_chain(self):
        ...
        event.set_result(
            types.SimpleNamespace(
                chain=[
                    types.SimpleNamespace(
                        text="那你躺吧，说话算话啊，晚上番茄鸡蛋面的宵夜全交给你了。"
                    ),
    ...
    def test_chat_style_before_send_does_not_segment_media_chain(self):
        ...
        text_item = types.SimpleNamespace(
            text="还没，刚摸黑躺下。突然发个贴贴，怎么，良心发现想请我吃宵夜？"
        )
    ...
    def test_chat_style_segmented_reply_sends_separate_messages(self):
        ...
        self.assertEqual(
            "".join(message.chain[0].text for message in event.sent_messages), reply
        )
    ...
    def test_chat_style_segmented_reply_uses_configured_delay(self):
        ...
        with (
            patch("core.runtime.delay.random.uniform", return_value=0.65),
            patch(
        ...
        self.assertTrue(sent)
        self.assertEqual(sleeps, [0.65] * (len(event.sent_messages) - 1))
        self.assertGreaterEqual(len(event.sent_messages), 2)
    ...
    def test_chat_style_segment_delay_follows_segment_length(self):
        ...
        with patch("core.runtime.delay.random.uniform", fake_uniform):
            lead_delay = runtime._chat_style_natural_segment_delay_seconds(
                "你呢", "今天不出去浪"
            )
            short_delay = runtime._chat_style_natural_segment_delay_seconds(
                "嗯。", "好。"
            )
        ...
        self.assertTrue(all(1.5 <= low <= high <= 3.5 for low, high in ranges))
        self.assertEqual(
            (lead_delay, short_delay, middle_delay, long_delay),
            tuple(low for low, _ in ranges),
        )
    ...
    def test_chat_style_initial_typing_delay_uses_text_weight(self):
        ...
        with patch("core.runtime.delay.random.uniform", fake_uniform):
            delay = runtime._chat_style_initial_typing_delay_seconds(
                "我听见了，不过现在真的快睡着了"
            )
