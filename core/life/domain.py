...
_POI_SEARCH_PROVENANCE_TTL_SECONDS = 15 * 60
_POI_SEARCH_PROVENANCE_MAX_SCOPES = 256
...
class LifeDomainService(DailyLocationPlanningMixin, DailyLocationAuditMixin):
    ...
    @staticmethod
    def _tool_scope_key(scope: str) -> str:
        return str(scope or "").strip() or "__local__"

    def _prune_poi_search_provenance(self, now: float | None = None) -> None:
        current = time.monotonic() if now is None else float(now)
        for key, record in list(self._poi_search_provenance.items()):
            if float(record.get("expires_at") or 0) <= current:
                self._poi_search_provenance.pop(key, None)
        overflow = len(self._poi_search_provenance) - _POI_SEARCH_PROVENANCE_MAX_SCOPES
        if overflow > 0:
            oldest = sorted(
                self._poi_search_provenance,
                key=lambda key: float(
                    self._poi_search_provenance[key].get("created_at") or 0
                ),
            )
            for key in oldest[:overflow]:
                self._poi_search_provenance.pop(key, None)

    def _remember_poi_search(self, scope: str, places: list[dict[str, Any]]) -> None:
        now = time.monotonic()
        self._prune_poi_search_provenance(now)
        poi_ids = {
            str(item.get("poi_id") or "").strip()
            for item in places
            if str(item.get("poi_id") or "").strip()
        }
        self._poi_search_provenance[self._tool_scope_key(scope)] = {
            "provider": self.map_provider,
            "poi_ids": poi_ids,
            "created_at": now,
            "expires_at": now + _POI_SEARCH_PROVENANCE_TTL_SECONDS,
        }

    def _poi_detail_authorized(self, scope: str, poi_id: str) -> bool:
        self._prune_poi_search_provenance()
        record = self._poi_search_provenance.get(self._tool_scope_key(scope))
        if not record or record.get("provider") != self.map_provider:
            return False
        return poi_id in set(record.get("poi_ids") or ())
    ...
    async def tool_place_search(
        ...
        *,
        ...
        self._remember_poi_search(scope, public_places)
        return {
            "ok": True,
            "query": query,
            "search_scope": (
    ...
    async def tool_place_detail(
        self, poi_id: str, *, scope: str = ""
    ) -> dict[str, Any]:
        """读取先前地点搜索返回的 POI 详情。"""
        ...
        if not self._poi_detail_authorized(scope, poi_id):
            return {
                "ok": False,
                "reason": "该地点编号不属于本会话最近一次地点搜索，请先重新搜索地点。",
            }
