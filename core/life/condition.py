...
def normalize_physiological_rhythm(
    ...
    *,
    ...
    actions = _list_texts(
        raw.get("recovery_actions") or prev.get("recovery_actions"), limit=5
    )
    social_default = _clamp_score(
        default_social_battery, int(defaults["social_battery"])
    )
    return {
        "energy_curve": _compact_text(
            raw.get("energy_curve")
            or prev.get("energy_curve")
            or defaults["energy_curve"],
            80,
        ),
        "body_condition": condition,
        "recovery_actions": actions,
        "social_battery": _clamp_score(
            raw.get("social_battery"),
            _clamp_score(prev.get("social_battery"), social_default),
        ),
        "attention_state": _compact_text(
            raw.get("attention_state")
            or prev.get("attention_state")
            or defaults["attention_state"],
            60,
        ),
        "optional_cycle": optional_cycle,
        "summary": _compact_text(
            raw.get("summary") or prev.get("summary") or defaults["summary"], 120
        ),
    }
...
def format_physiological_rhythm_prompt(value: Any) -> str:
    ...
    optional_cycle = (
        rhythm.get("optional_cycle")
        if isinstance(rhythm.get("optional_cycle"), dict)
        else {}
    )
    parts = [
    ...
    actions = (
        rhythm.get("recovery_actions")
        if isinstance(rhythm.get("recovery_actions"), list)
        else []
    )
    if actions:
...
def normalize_state(
    ...
    *,
    ...
    scores = {
        key: _clamp_score(
            raw.get(key), _clamp_score(prev.get(key), STATE_DEFAULTS[key])
        )
    ...
    return {
        **scores,
        "mood": _compact_text(
            raw.get("mood") or prev.get("mood") or STATE_DEFAULTS["mood"], 120
        ),
        ...
        "summary": _compact_text(
            raw.get("summary") or prev.get("summary") or STATE_DEFAULTS["summary"], 160
        ),
...
def state_is_stale(
    state: dict | None, now: datetime.datetime | None = None, minutes: int = 30
) -> bool:
    ...
    try:
        updated_at = datetime.datetime.strptime(
            str(state_data["updated_at"]), "%Y-%m-%d %H:%M"
        )
    except (TypeError, ValueError):
        return True
    now = now or life_now()
...
def format_state_prompt(state: dict | None) -> str:
    ...
    rhythm_text = format_physiological_rhythm_prompt(
        normalized.get("physiological_rhythm")
    )
    return (
