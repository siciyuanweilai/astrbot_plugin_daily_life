...
OUTFIT_CONTINUITY_RULES = (
    ...
    "outfit 只写此刻实际穿在身上或实际携带的组成；放在玄关、桌边、衣架上或只是为稍后出门备好的鞋、包、外套、伞和随身物品，"
    "属于外出备选，不是当前穿搭。scene_category 为 home 或 sleep 时，不得把外出鞋、随身包等备选组成写成仍穿着或携带；"
    "应在实际离家节点通过 change_outfit 加入，回家放下包、脱外层或换室内鞋同样属于局部穿搭变化。\n"
    ...
    "只有当前节点已经明确处于离家动作，才能把备选鞋包并入当前穿搭。\n"
    "keep 延续当前 outfit、style、hair_style、hair、makeup、nails；partial_change 只调整有事实依据且不适配的组成，主体衣物保持连续。"
