...
class LifeBackgroundComposer(
    ...
    async def _fallback_provider_for_call(self, primary_provider_id: str, reason: str = ""):
        provider, fallback_id = await self._system_default_provider(primary_provider_id)
        if not provider:
            return None, ""
        suffix = f"：{reason}" if reason else ""
        logger.info(f"[日常生活] 指定大语言模型本次调用不可用，临时使用当前默认模型（{fallback_id}）{suffix}")
    ...
    async def _call_llm_text(
        ...
        async def switch_to_fallback(reason: str) -> bool:
            nonlocal current_provider, current_provider_id
            if not primary_provider_id or current_provider_id != primary_provider_id:
                return False
            fallback_provider, fallback_id = await self._fallback_provider_for_call(primary_provider_id, reason)
            if not fallback_provider:
                return False
            current_provider = fallback_provider
            current_provider_id = fallback_id
            return True

        attempt = 0
        while attempt <= empty_retries:
            if attempt > 0 and attempt == empty_retries:
                await switch_to_fallback("达到空响应重试上限")
            try:
                ...
                err_text = str(exc)
                if "401" in err_text and await switch_to_fallback("401"):
                    continue
                if attempt < empty_retries:
                    logger.warning(f"[日常生活] 大语言模型调用异常（第 {attempt + 1} 次）：{exc}")
                    attempt += 1
                    continue
                if await switch_to_fallback("调用异常"):
                    continue
                logger.warning(f"[日常生活] 大语言模型调用异常（第 {attempt + 1} 次）：{exc}")
                return ""
            ...
            if attempt < empty_retries:
                ...
                attempt += 1
                continue
            if await switch_to_fallback("返回空响应"):
                continue
            return ""
        return ""
