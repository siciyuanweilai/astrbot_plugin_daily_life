...
class HiddenExperienceMixin:
    ...
    def _hidden_join(self, values: Any, *, limit: int, count: int) -> str:
        return "；".join(
            self._hidden_text(text, limit) for text in list(values or [])[:count]
        )

    @staticmethod
    def _hidden_lines(
        items: list[Any] | None, count: int, render: Callable[[Any], str]
    ) -> list[str]:
        return [line for item in list(items or [])[:count] if (line := render(item))]

    @staticmethod
    def _hidden_section(
        sections: list[str], title: str, lines: list[str], intro: str = ""
    ) -> None:
        if lines:
            body = "\n".join(lines)
    ...
    def _hidden_rhythm_line(self, item: Any) -> str:
        ...
        return (
            f"- {self._hidden_first(item, 'date', limit=20) or '近期'}：{body}"
            if body
            else ""
        )
    ...
    def _hidden_expression_review_line(self, item: Any) -> str:
        ...
        if not (risk or suggestion):
            return ""
        return f"- 表达审核{passed}: {risk or suggestion}" + (
    ...
    def _hidden_behavior_scene_line(self, item: Any) -> str:
        ...
        return f"- {scene}: {cues or '语义场景'}；倾向{action or '观察'}" + (
    ...
    def _hidden_life_sections(
        ...
        *,
        ...
        self._hidden_section(
            sections,
            "[HiddenLifeEpisodes]",
            self._hidden_lines(episodes, 3, self._hidden_episode_line),
        )
        self._hidden_section(
            sections,
            "[HiddenFocusTargets]",
            self._hidden_lines(focus_targets, 4, self._hidden_focus_target_line),
        )
    ...
    def _hidden_behavior_sections(
        ...
        *,
        ...
        self._hidden_section(
            sections,
            "[HiddenBehaviorFeedback]",
            self._hidden_lines(feedback, 3, self._hidden_behavior_feedback_line),
        )
        self._hidden_section(
            sections,
            "[HiddenBehaviorPatterns]",
            self._hidden_lines(
                behavior_patterns, 4, self._hidden_behavior_pattern_line
            ),
        )
        self._hidden_section(
            sections,
            "[HiddenBehaviorScenes]",
            self._hidden_lines(behavior_scenes, 4, self._hidden_behavior_scene_line),
        )

    def _hidden_emotion_sections(
        self, sections: list[str], *, emotion_arcs: list[Any] | None = None
    ) -> None:
        self._hidden_section(
            sections,
            "[HiddenEmotionArc]",
    ...
    def _hidden_rhythm_sections(
        ...
        *,
        ...
        if isinstance(physiological_rhythm_trend, dict):
            summary = self._hidden_text(
                physiological_rhythm_trend.get("summary", ""), 180
            )
            if summary:
    ...
    def _hidden_expression_sections(
        ...
        *,
        ...
        self._hidden_section(
            sections,
            "[HiddenExpressionReviews]",
            self._hidden_lines(
                expression_reviews, 3, self._hidden_expression_review_line
            ),
        )
        self._hidden_section(
            sections,
            ...
            self._hidden_lines(
                temporary_expression_states, 3, self._hidden_temporary_expression_line
            ),
        )
        self._hidden_section(
            sections,
            "[HiddenExpressionIntents]",
            self._hidden_lines(
                expression_intents, 3, self._hidden_expression_intent_line
            ),
        )
    ...
    def _hidden_memory_sections(
        ...
        *,
        ...
        self._hidden_section(
            sections,
            "[HiddenMemoryCorrections]",
            self._hidden_lines(
                memory_corrections, 3, self._hidden_memory_correction_line
            ),
        )
        self._hidden_section(
            sections,
            "[HiddenFocusSlots]",
            self._hidden_lines(focus_slots, 4, self._hidden_focus_slot_line),
        )
        self._hidden_section(
            sections,
            "[HiddenMidConversationSummary]",
            self._hidden_lines(mid_summaries, 3, self._hidden_mid_summary_line),
        )
        self._hidden_section(
            sections,
            "[HiddenSceneTerms]",
            self._hidden_lines(terms, 6, self._hidden_term_line),
        )
        self._hidden_section(
            sections,
    ...
    def _format_hidden_experience_context(
        ...
        sections: list[str] = []
        self._hidden_life_sections(
            sections, episodes=episodes, focus_targets=focus_targets
        )
        ...
        self._hidden_behavior_sections(
            sections,
            behavior_patterns=behavior_patterns,
            behavior_scenes=behavior_scenes,
        )
        self._hidden_memory_sections(
            sections, focus_slots=focus_slots, mid_summaries=mid_summaries
        )
        self._hidden_expression_sections(
            sections,
