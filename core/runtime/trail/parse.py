...
class HistoryParseMixin:
    ...
    def _history_text_from_value(value: Any) -> str:
        ...
        for attr in ("message_str", "text", "content", "raw_message"):
            text = HistoryParseMixin._history_text_from_value(
    ...
    def _history_user_item_matches(item: Any, expected: dict[str, Any]) -> bool:
        ...
        if item.get("content") == expected.get("content"):
            return True
        if HistoryParseMixin._history_content_has_media(
            item.get("content")
        ) or HistoryParseMixin._history_content_has_media(expected.get("content")):
            return False
    ...
    @staticmethod
    def _history_message_parts_for_user(
        text: str, media_label: str = ""
    ) -> list[dict[str, str]]:
        if text:
    ...
    async def _history_display_url(path: str) -> str:
        ...
        if not path or path.startswith(("http://", "https://", "data:")):
            return path
        local = await asyncio.to_thread(expand_path, path)
        if not await asyncio.to_thread(path_is_file, local):
            return path
        try:
    ...
    async def _platform_media_part_from_history_payload(
