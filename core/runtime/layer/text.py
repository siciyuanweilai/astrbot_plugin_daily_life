...
class LayerTextMixin:
    def build_hidden_chat_style_hint(self) -> str:
        ...
        try:
            ...
            casual_max_int = 0
        if casual_max_int > 0:
            lines.append(f"- 日常闲聊参考长度约 {casual_max_int} 字左右；认真问题按内容自然展开。")

        lines.append("- 轻松接话保持短气口；一句只放一个主要意思，能自然停住就停住。")
        lines.append("- 客观事实先看依据；信息会变化或拿不准时先核验，不能确认就坦白边界。")
    ...
    def _format_hidden_state_compact(self, state: Any) -> str:
        ...
        rhythm = state_dict.get("physiological_rhythm") if isinstance(state_dict.get("physiological_rhythm"), dict) else {}
        rhythm_text = self._hidden_text(format_physiological_rhythm_prompt(rhythm), 180) if rhythm else ""
