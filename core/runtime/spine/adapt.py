...
class SpineAdaptMixin:
    ...
    async def apply_config(self, next_config: dict[str, Any]) -> LifeSettings:
        ...
        async with self.generation_lock:
            ...
            previous_weather_client = getattr(self, "weather_client", None)
            previous_media = getattr(self, "media", None)
            self.rhythm.stop()
            self._bind_runtime(parsed)
            self._injection_snapshot_cache = {}
            self.rhythm.start()
            if previous_weather_client:
                try:
                    await previous_weather_client.close()
                except Exception as exc:
                    logger.warning(f"{LOG_PREFIX} 关闭原天气客户端失败：{exc}")
            if previous_media:
                try:
                    await previous_media.close()
                except Exception as exc:
