...
class LifeRhythmClock:
    ...
    def healthy(self) -> bool:
        return bool(self.scheduler.running and not self.last_error)
    ...
    def start(self):
        try:
            ...
            if (
                proactive.private_revisit_enabled
                and self.proactive_revisit_task
            ):
                interval = max(5, int(proactive.revisit_interval_minutes or 30))
                self.scheduler.add_job(
                    self.proactive_revisit_task,
                    "interval",
                    minutes=interval,
                    id="private_revisit_check",
                    replace_existing=True,
                )
            ...
            logger.error(f"[日常生活] 调度器初始化失败：{exc}")
            raise
