...
class VoiceSwitchGateMixin:
    ...
    def _prune_voice_switch_cadence(self, now: datetime.datetime | None = None) -> None:
        ...
        for key, item in list(store.items()):
            ...
            try:
                expired = (
                    now - last_seen
                ).total_seconds() > self._VOICE_SWITCH_CADENCE_TTL_SECONDS
            except TypeError:
                expired = True
            if expired:
                store.pop(key, None)
    ...
    def _voice_switch_cadence_snapshot(
        ...
        if isinstance(last_voice_at, datetime.datetime):
            try:
                ...
                seconds_since_voice = None
        text_after_voice = (
            int(item.get("text_after_voice") or 0) if isinstance(item, dict) else 0
        )
        consecutive_voice = (
            int(item.get("consecutive_voice") or 0) if isinstance(item, dict) else 0
        )
        chain_limit = (
            self._voice_switch_chain_limit(item)
            if isinstance(item, dict)
            else self._VOICE_SWITCH_MAX_CONSECUTIVE_VOICE
        )
        voice_chain_open = (
        ...
        return {
            "last_channel": str(item.get("last_channel") or "")
            if isinstance(item, dict)
            else "",
    ...
    def _mark_voice_switch_channel(
        self, event: Any, channel: str, now: datetime.datetime | None = None
    ) -> None:
        scope = self._voice_switch_scope_key(event)
        if not scope:
            return
        ...
        normalized = (
            "语音" if str(channel or "").lower() in {"voice", "语音"} else "文字"
        )
        item["last_seen"] = now
        if normalized == "语音":
    ...
    def _voice_switch_auto_gate(
        ...
        if probability < 100 and random.random() * 100 >= probability:
            return (
                False,
                "我这轮虽然可以说出来，但按当前聊天节奏更适合把语音留到更需要的片刻。",
            )
        return True, ""
