...
class RuntimeMediaCommonMixin:
    ...
    def _media_tool_failure_text(media_name: str, error: str) -> str:
        ...
        if detail and ("智能提取失败" in detail or "没有收到" in detail or "没有找到" in detail):
    ...
    async def _media_payload_from_item_async(self, item: Any) -> dict[str, str]:
        ...
        if callable(converter):
            try:
                ...
                logger.debug(f"{LOG_PREFIX} 媒体本地解析失败：{self._media_error_summary(exc)}")
        return payload
    ...
    async def _quote_image_reference_from_event_async(self, event: Any) -> str:
        for source in self._event_sources(event):
            for attr in ("quote", "reply", "reply_message"):
                reference = await self._quote_image_reference_from_value_async(getattr(source, attr, None))
                if reference:
                    return reference
            for item in self._event_message_items(source):
                kind = self._event_component_kind(item)
    ...
    async def _resolve_life_image_reference_async(self, event: Any, reference_image: str = "") -> str:
        ...
        if explicit:
            return explicit
        current = await self._image_reference_from_items_async(self._event_message_items(event))
        if current:
            return current
