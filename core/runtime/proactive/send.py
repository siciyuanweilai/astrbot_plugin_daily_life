        ...
        *,
        ...
        try:
            ...
            if await self._send_proactive_voice_if_enabled(
                ...
                return True
            if self._proactive_segmented_reply_enabled(target_scope):
                sent = await self._send_segmented_proactive_message(
                    target_scope,
                    reply_text,
                    source_event=source_event,
                    source_message_id=source_message_id,
                ...
                if not await self.send_message_if_not_recalled(
                    ...
                    return False
                self.note_structured_bot_message(target_scope, reply_text)
            await self._append_proactive_send_history(target_scope, reply_text)
    ...
    async def _send_proactive_voice_if_enabled(
        ...
        *,
        ...
        try:
            ...
            logger.debug(f"{LOG_PREFIX} 闲时消息语音发送失败，改用文字兜底：{exc}")
            await self._note_voice_expression_decision(
                scope=target_scope,
                channel="文字",
                source="闲时回复",
                reason=f"闲时消息语音发送失败，改用文字兜底：{exc}",
                result="改用文字",
                text=reply_text,
                emotion=emotion,
                emotion_category=emotion_category,
