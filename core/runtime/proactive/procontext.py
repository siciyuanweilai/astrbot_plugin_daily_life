...
class ProactiveContextMixin:
    ...
    def _format_proactive_expression_style(self, target_scope: str) -> str:
        ...
        if limit > 0:
            lines.append(f"- {scope_label}主动消息参考长度约 {limit} 字左右；不是硬截断，但后台主动发言要优先短气口。")
        lines.append(f"- 表达节奏：{style_prompt or '轻量、自然、少展开。'}")
        return "\n".join(lines)

    def _proactive_reply_style_reject_reason(self, target_scope: str, reply_text: str) -> str:
        limit = self._proactive_expression_limit_for_scope(target_scope)
        if limit <= 0:
            return ""
    ...
    @staticmethod
    def _format_context_message_content(message: dict[str, str], limit: int = 140) -> str:
