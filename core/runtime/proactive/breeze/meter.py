...
class AirMeterMixin:
    ...
    def _air_label(self, state: dict[str, Any] | None = None) -> str:
        ...
        if last_effect == "negative":
            return "上次闲时续话收到明确负向反馈，需要更克制"
    ...
    def _format_proactive_air_state(self, key: str, now: datetime.datetime) -> str:
        ...
        if isinstance(wait_until, datetime.datetime) and wait_until > now:
            parts.append(
                f"正在等待更多话题自然落点，约 {wait_until.strftime('%H:%M')} 后再看"
            )
    ...
    def _air_backoff_seconds(self, quiet_count: int) -> int:
        ...
        return min(
            self._AIR_BACKOFF_CAP_SECONDS,
            self._AIR_BACKOFF_BASE_SECONDS * (2**exponent),
        )
        ...
        *,
        ...
        if isinstance(backoff_until, datetime.datetime) and backoff_until > now:
            if pending_count >= self._AIR_BYPASS_PENDING_COUNT:
                ...
                logger.debug(
                    f"{LOG_PREFIX} 闲时回复退避被新消息打破：待看消息 {pending_count} 条"
                )
                return 0
    ...
    def _proactive_timing_delta(
        self, event: Any, now: datetime.datetime, reasons: list[str]
    ) -> float:
        delta = 0.0
        ...
        if isinstance(last_activity_at, datetime.datetime):
            ...
            idle_threshold = self._proactive_idle_seconds(event)
            if idle_seconds >= idle_threshold * 2:
                delta -= 0.04
    ...
    async def _proactive_state_score(
        self,
        now: datetime.datetime,
        *,
        is_group: bool,
        ...
        if not (callable(state_getter) and callable(state_delta)):
            return 0.0, False
        state = await state_getter(now)
        if state is None:
            return 0.0, False
        normalized = normalize_state(
            state.as_dict() if hasattr(state, "as_dict") else state
        )
        if (
            not message_can_interrupt(normalized, interrupt)
            and pending_count < self._AIR_BYPASS_PENDING_COUNT
        ):
            reasons.append("当前生活状态不适合被这类闲时消息打断")
            return 0.0, True
        return state_delta(state, is_group, reasons), False

    async def _proactive_context_score(
        self,
        event: Any,
        now: datetime.datetime,
        key: str,
    ...
    @staticmethod
    def _proactive_target_payload(
        interrupt: dict[str, Any], target: dict[str, Any]
    ) -> dict[str, Any]:
        return {
            ...
            "target_sender_name": target.get("sender_name", ""),
        }

    def _proactive_observe_result(
        self,
        score: float,
        reasons: list[str],
        interrupt: dict[str, Any],
        ...
        *,
        ...
        result = {
            ...
            "confidence": (
                ...
                else round(1.0 - normalized_score, 3)
            ),
            "reason": "；".join(reasons) or "本地状态判断更适合继续观察",
            "reply_text": "",
            "local_score": round(normalized_score, 3),
            "local_reasons": reasons,
            "can_interrupt": can_interrupt,
        }
        result.update(self._proactive_target_payload(interrupt, target))
        return result
    ...
    async def _proactive_readiness_check(
        ...
        *,
        ...
        score, interrupt = self._proactive_message_score(
            event,
            pending_count=pending_count,
            is_group=is_group,
            reasons=reasons,
        )
        score += self._proactive_timing_delta(event, now, reasons)

        if key:
            score += self._proactive_air_delta(key, reasons)
        state_score, blocked = await self._proactive_state_score(
            now,
            is_group=is_group,
            pending_count=pending_count,
            interrupt=interrupt,
        ...
        score += state_score
        if blocked:
            return self._proactive_observe_result(
                score,
                reasons,
                interrupt,
                local_target,
        ...
        score += await self._proactive_context_score(event, now, key, reasons)
        ...
        if score < threshold:
            return self._proactive_observe_result(
                score, reasons, interrupt, local_target, can_interrupt=True
            )
        result = {
        ...
        result.update(self._proactive_target_payload(interrupt, local_target))
        return result
    ...
    def _proactive_air_delta(self, key: str, reasons: list[str]) -> float:
        ...
        elif last_effect == "negative":
            delta -= 0.16
            reasons.append("上次闲时接话收到明确负向反馈")
    ...
    def _update_proactive_air_after_decision(
        ...
        *,
        ...
        state = self._get_proactive_air_state(key)
        decision = (
            str(payload.get("decision") or ("reply" if sent else "observe")).strip()
            or "observe"
        )
        ...
        state["silence_inertia"] = min(
            100,
            int(state.get("silence_inertia") or 0) + (16 if decision == "wait" else 12),
        )
        if decision == "wait":
            state["wait_until"] = now + datetime.timedelta(
                seconds=self._AIR_WAIT_SECONDS
            )
            state["backoff_until"] = None
            return
        ...
        state["backoff_until"] = (
            now + datetime.timedelta(seconds=seconds) if seconds > 0 else None
        )

    def _recent_messages_for_candidate(
    ...
    def _format_candidate_recent_messages(
        self, event: Any, fallback_sender_name: str
    ) -> str:
        raw_messages = getattr(event, "proactive_recent_messages", None)
        if not isinstance(raw_messages, list) or not raw_messages:
            raw_messages = [
        ...
        for index, item in enumerate(
            raw_messages[-self._AIR_MAX_RECENT_MESSAGES :], start=1
        ):
            content = "；".join(
                part.strip()
                for part in str(item.get("content") or "").splitlines()
                if part.strip()
            )
            if len(content) > 120:
            ...
            sender_name = (
                ...
                or fallback_sender_name
                or "对方"
            )
            seen_at = item.get("seen_at")
            time_text = (
                seen_at.strftime("%H:%M")
                if isinstance(seen_at, datetime.datetime)
                else ""
            )
            prefix = f"- {message_id}"
            if time_text:
