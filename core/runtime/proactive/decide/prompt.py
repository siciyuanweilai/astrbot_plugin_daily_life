...
class ProactivePromptMixin:
        ...
        *,
        ...
        if callable(builder):
            return await builder(
                persona=persona,
                explicit_instruction=message,
                relationships=relationships,
            )
        return person_fact_context_from_relationships(
            persona=persona,
            explicit_instruction=message,
            relationships=relationships,
        )
    ...
    async def _proactive_prompt_records(
        ...
        *,
        ...
        return {
            ...
            "candidate_messages": self._format_candidate_recent_messages(
                event, sender_name
            ),
            "air_state": self._format_proactive_air_state(scope_key, now)
            if scope_key
            else "暂无会话空气记录。",
        }
    ...
    def _proactive_readiness_text(readiness: dict[str, Any] | None) -> dict[str, Any]:
        ...
        local_reasons = (
            readiness.get("local_reasons")
            if isinstance(readiness.get("local_reasons"), list)
            else []
        )
        local_reason_text = (
            "；".join(str(item).strip() for item in local_reasons if str(item).strip())
            or "无特别偏向"
        )
        ...
        return {
            ...
            "interrupt_reason": str(
                readiness.get("interrupt_reason") or "无明显打断信号"
            ),
    ...
    async def _proactive_prompt_scene(
        ...
        state = normalize_state(
            day.state.as_dict() if day and getattr(day, "state", None) else {}
        )
        ...
        person_facts = await self._proactive_person_fact_context(
            persona=persona,
            message=message,
...
人物事实边界：
{scene["person_fact_context"].format_for_generation(include_rules=False) or "暂无额外明确人物事实。"}
...
{style_line}"""
    ...
    async def _build_proactive_prompt(
        ...
        return cache_friendly_prompt(
            self._proactive_prompt_fixed(), self._proactive_prompt_dynamic(scene)
        )
