...
class StructuredGlanceMixin:
    ...
    def structured_recent_history_messages(self, event_or_scope: Any, limit: int = 6) -> list[dict[str, str]]:
        scope = event_or_scope if isinstance(event_or_scope, str) else self._event_session_id(event_or_scope)
        scope = str(scope or "").strip()
        if not scope:
            return []
        messages = list(self._structured_scope_messages(scope))[-max(1, int(limit or 6)) :]
        return [self._structured_history_payload(item) for item in messages if item.content or item.media]
