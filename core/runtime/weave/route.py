    ...
    def _structured_resolve_talking_to(
        ...
        if message.is_bot:
            if message.reply_to_id:
                target = self._structured_find_message(
                    message.scope, message.reply_to_id
                )
        ...
        if (
            callable(mentions_bot_alias)
            and source_event is not None
            and mentions_bot_alias(source_event)
        ):
            message.talking_to_id = "bot"
            message.talking_to_name = "我"
            return
        ...
        if message.reply_to_id:
            ...
            if target:
                ...
                message.talking_to_name = (
                    "我" if target.is_bot else target.display_sender
                )
                return
        ...
        if message.reply_to_sender_id:
            ...
            message.talking_to_name = (
                message.reply_to_sender_name or message.reply_to_sender_id
            )
            return
