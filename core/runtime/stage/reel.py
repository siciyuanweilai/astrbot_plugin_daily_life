...
- appearance_profile 只提取当前人设明确写出的跨场景稳定身份外观，例如脸部轮廓、明确五官、瞳色、肤色、自然发色和稳定体态；排除临时发型（扎法、分缝、刘海、发尾整理）、服装、妆容、配饰、姿势、镜头和临时状态；没有明确内容时留空。该字段只供系统缓存，不代替本轮 body_presentation。
- 输出字段尽量短，最终会被拼成图片提示词。
{CORE_JSON_OUTPUT_RULES}
JSON 字段：
