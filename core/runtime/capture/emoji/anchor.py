...
class EmojiBaseMixin:
    ...
    EMOJI_ASSET_MIN_PLAIN_IMAGE_CONFIDENCE = 0.85
    EMOJI_ASSET_ALLOWED_REACTION_TYPES = {
        "emoji",
        "sticker",
        "reaction",
        "meme",
    }
    EMOJI_ASSET_INACTIVE_STATUSES = {
        "pending",
        "reviewing",
        "failed",
        "rejected",
    ...
    async def _emoji_ready_asset_count(self, *, exclude_hash: str = "") -> int:
        ...
        return sum(
            1
            for item in assets
            if str(getattr(item, "file_hash", "") or "").strip() != exclude_hash
        )

    async def _emoji_can_accept_ready_asset(self, *, exclude_hash: str = "") -> bool:
        return (
            self._emoji_replace_when_full()
            or await self._emoji_ready_asset_count(exclude_hash=exclude_hash)
            < self._emoji_max_ready_count()
        )
    ...
    def _emoji_asset_cache_dir(self, *, create: bool = True) -> Path | None:
        ...
        if not data_path:
            return None
        cache_dir = (
            Path(data_path).expanduser().resolve().parent / self.EMOJI_ASSET_DIR_NAME
        )
        if create:
    ...
    def _emoji_asset_type_from_item(cls, item: Any) -> str:
        values = cls._emoji_asset_meta_values(item)
        for kind in sorted(
            cls.EMOJI_ASSET_COLLECTIBLE_KINDS | cls.EMOJI_ASSET_REVIEWABLE_KINDS
        ):
            if kind in values:
                return kind
        if values & {"image", "picture"}:
    ...
    def _remember_visual_context_summary(self, fingerprint: str, summary: str) -> None:
    ...
    @classmethod
    def _emoji_asset_suffix(
        cls, source: str, data: bytes = b"", content_type: str = ""
    ) -> str:
    ...
    def _emoji_asset_filename(fingerprint: str, suffix: str) -> str:
        safe_hash = (
            "".join(ch for ch in str(fingerprint or "") if ch.isalnum())[:80]
            or uuid.uuid4().hex
        )
    ...
    def _resolve_cached_emoji_path(
        self, path_text: str, cache_dir: Path
    ) -> Path | None:
    ...
    def _apply_visual_context_summary(
        ...
        if not summary:
            return
        if fingerprint:
            self._remember_visual_context_summary(fingerprint, summary)
        self._apply_visual_context_summary_text(
            context_scope, context_message_key, summary
        )
