...
class EmojiCacheMixin:
    ...
    async def _cache_emoji_asset_path(self, payload: dict[str, str], fingerprint: str) -> Path | None:
        ...
        if source.startswith("data:image/") or source.startswith("base64://"):
            return await self._decode_emoji_asset_data(source, fingerprint, cache_dir)
        if source.startswith("file://"):
            source = self._local_path_from_file_uri(source)
        if source.startswith(("http://", "https://")):
            return await self._download_emoji_asset(source, fingerprint, cache_dir)
        return await self._copy_emoji_asset(source, fingerprint, cache_dir)
    ...
    async def _copy_emoji_asset(self, source: str, fingerprint: str, cache_dir: Path) -> Path | None:
        try:
            ...
            if not source_path.is_file():
                return None
            if not self._emoji_asset_size_allowed(source_path.stat().st_size):
                return None
    ...
    async def _decode_emoji_asset_data(self, source: str, fingerprint: str, cache_dir: Path) -> Path | None:
        try:
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.get(url) as response:
                    ...
                    data = await response.content.read(max_bytes + 1 if max_bytes > 0 else -1)
            if not data or not self._emoji_asset_size_allowed(len(data)):
                return None
