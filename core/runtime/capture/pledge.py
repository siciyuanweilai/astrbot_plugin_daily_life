        ...
        try:
            ...
            if callable(auditor):
                audit = await auditor(
                    ...
                    patterns=COMMITMENT_PERSON_TEXT_PATHS,
                    provider=provider,
                ...
                if audit.unresolved:
                    logger.warning(
                        f"{LOG_PREFIX} 承诺人物事实存在未解决冲突，本次不保存。"
                    )
                    return None
            ...
            if saved.trigger_date or saved.trigger_time or saved.time_window:
                details.append(
                    f"触发时间：{saved.trigger_date} {saved.trigger_time or saved.time_window}".strip()
                )
            if saved.people:
            ...
            logger.info(f"{LOG_PREFIX} 已记录聊天承诺：编号={saved.id}")
            return saved
