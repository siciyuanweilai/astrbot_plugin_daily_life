...
class CapturePledgeMixin:
    ...
    def _build_commitment_extract_prompt(
        ...
        persona_line = f"对方在人设中的线索：{persona_hint}\n" if persona_hint else ""
        if persona_hint:
            pronoun_boundary = "如果称谓与对方在人设中的线索冲突，必须按这条人设线索修正；不要沿用已保存印象里的冲突称谓，也不要把对方当成我。"
        else:
            pronoun_boundary = "本轮没有从当前人设提取到对方性别线索；旧印象或旧记忆里零散出现的他/她不能当作性别依据，隐藏推理和输出都用中性称呼。"
...
- 隐藏推理、理由和输出字段都必须遵守人物称谓与性别规则；有对方在人设中的线索时，以该线索为准，但不要把对方当成我。
- 这是极短内部裁定，不需要展开分析；如果服务端仍记录隐藏推理，只能保留一句第一人称内心判断。
- 隐藏推理只写我此刻的感受和判断；普通情绪表达不是未来约定。
"""
        dynamic = f"""说话人：{sender_name}
{persona_line}称谓校准：{pronoun_boundary}
