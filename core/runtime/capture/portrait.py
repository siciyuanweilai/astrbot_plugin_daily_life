...
class ImprintPortraitMixin:
    async def _apply_memory_correction(
        ...
        if target_type == "life_episode":
            ...
            return bool(
                episode_id
                and await self.archive.correct_life_episode(
                    episode_id, correction.correction
                )
            )
    ...
    async def _save_memory_targets(
        ...
        for target in targets[:8]:
            ...
            if target_persona_hint:
                target = await self._calibrate_memory_target_payload(
                    target, meta, target_persona_hint
                )
                target_persona_hint = (
                    self._str_payload(target.get("persona_hint")) or target_persona_hint
                )
            ...
            subjective_tags = [
                ...
                for item in self._list_payload(
                    target.get("subjective_tags") or target.get("tags")
                )
            ...
            if not any(
                [note, points, subjective_name, relationship_story, subjective_tags]
            ):
                continue
            await self.archive.touch_relationship(
                ...
                note=note,
                date_str=meta["date"],
                source=self._str_payload(target.get("source"), "chat_memory_target")
                or "chat_memory_target",
                platform=platform,
                user_id=user_id,
