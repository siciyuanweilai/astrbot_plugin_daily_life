...
def _file_sha256(path, chunk_size: int = 1024 * 1024) -> str:
...
class CaptureEventMixin:
    ...
    def _event_is_group_message(cls, event: Any) -> bool:
        ...
        if len(parts) >= 2 and parts[1].lower() == "groupmessage":
            return True

        message_type = cls._safe_event_call(event, "get_message_type").lower()
        if "group" in message_type and "message" in message_type:
            return True
        ...
        for source in cls._event_sources(event):
            for current in (source, getattr(source, "message_obj", None)):
                ...
                for attr in ("type", "message_type"):
                    value = str(getattr(current, attr, "") or "").lower()
                    if "group" in value and "message" in value:
                        return True
    ...
    def _event_platform_user(cls, event: Any) -> tuple[str, str]:
        ...
        if cls._event_is_group_message(event):
            return platform or cls._safe_event_call(
    ...
    def _event_group_meta(cls, event: Any) -> tuple[str, str]:
        ...
        if not group_id and cls._event_is_group_message(event):
            _, real_id = parse_unified_origin(
                cls._safe_event_attr(event, "unified_msg_origin")
            )
            group_id = real_id
    ...
    def _event_is_platform_directed(cls, event: Any) -> bool:
        return any(
            bool(getattr(source, "is_at_or_wake_command", False))
            for source in cls._event_sources(event)
        )
    ...
    def _event_bot_aliases(self) -> list[str]:
        ...
        aliases = (
            getattr(config, "bot_identity_aliases", []) if config is not None else []
        )
    ...
    def _event_text_contains_bot_alias(cls, text: str, alias: str) -> bool:
        ...
        while True:
            ...
            if not cls._event_ascii_token_char(
                before
            ) and not cls._event_ascii_token_char(after):
                return True
            start = index + 1
    ...
    def _event_mentions_bot_alias(self, event: Any) -> bool:
        ...
        if not text:
            return False
        return any(
            self._event_text_contains_bot_alias(text, alias)
            for alias in self._event_bot_aliases()
        )

    def _event_is_directed(self, event: Any) -> bool:
        return self._event_is_platform_directed(
            event
        ) or self._event_mentions_bot_alias(event)
    ...
    def _event_has_command_handler(cls, event: Any) -> bool:
        for source in cls._event_sources(event):
            ...
            handlers = (
                get_extra("activated_handlers", []) if callable(get_extra) else []
            )
        ...
        for source in cls._event_sources(event):
            ...
            for item in messages:
                ...
                if isinstance(item, dict):
                    type_name = (
                        str(item.get("type") or item.get("kind") or type_name)
                        .strip()
                        .lower()
                    )
    ...
    def _event_message_component_facts(cls, event: Any, text: str = "") -> str:
        text = str(
            text if text is not None else getattr(event, "message_str", "") or ""
        ).strip()
    ...
    def _message_media_payload(cls, item: Any) -> dict[str, str]:
        ...
        def collect_mapping(source: Any) -> None:
            ...
            for key in (
                ...
                if text:
                    values["file" if key == "file_" else key] = text
        ...
        def collect_object(source: Any) -> None:
            ...
            for key in (
                ...
                text = str(
                    direct.get(key) if key in direct else getattr(source, key, "") or ""
                ).strip()
                if text:
                    values["file" if key == "file_" else key] = text
            for key in ("data", "origin", "original", "raw", "source", "media"):
    ...
    async def _media_fingerprint(self, payload: dict[str, str]) -> str:
        explicit = (
            payload.get("hash")
            or payload.get("file_unique")
            or payload.get("file_unique_id")
        ...
        if local_path and not local_path.startswith(("http://", "https://")):
            try:
                path = await asyncio.to_thread(expand_path, local_path, resolve=True)
                if await asyncio.to_thread(path_is_file, path):
                    size = await asyncio.to_thread(lambda: path.stat().st_size)
                    if not self._emoji_asset_size_allowed(size):
                        return ""
                    return await asyncio.to_thread(_file_sha256, path)
        ...
        return (
            hashlib.sha256(marker.encode("utf-8", errors="ignore")).hexdigest()
            if marker
            else ""
        )
