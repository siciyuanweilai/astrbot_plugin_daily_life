...
class CaptureEventMixin:
    ...
    @classmethod
    def _event_is_directed(cls, event: Any) -> bool:
    ...
    def _message_media_payload(cls, item: Any) -> dict[str, str]:
        values: dict[str, str] = {}
        if isinstance(item, dict):
            for key in ("file", "path", "url", "image", "hash", "file_id"):
    ...
    async def _media_fingerprint(payload: dict[str, str]) -> str:
        explicit = (payload.get("hash") or payload.get("file_id") or "").strip()
        if explicit:
            return explicit[:80]
    ...
    @staticmethod
    def _voice_list_matches(values: list[str], candidates: set[str]) -> bool:
        normalized = {str(item or "").strip() for item in values or [] if str(item or "").strip()}
        return bool(normalized & candidates)

    def _voice_scope_candidates(self, scope_or_event: Any) -> tuple[bool, set[str]]:
        if isinstance(scope_or_event, str):
            scope = str(scope_or_event or "").strip()
            parts = scope.split(":")
            is_group = len(parts) >= 2 and parts[1].lower() == "groupmessage"
            candidates = {scope}
            if parts:
                candidates.add(parts[-1])
            return is_group, {item for item in candidates if item}

        scope = self._event_session_id(scope_or_event)
        candidates = {scope} if scope else set()
        group_id, _ = self._event_group_meta(scope_or_event)
        is_group = self._event_is_group_message(scope_or_event)
        if is_group and group_id:
            candidates.add(group_id)
        if not is_group:
            _, user_id = self._event_platform_user(scope_or_event)
            if user_id:
                candidates.add(user_id)
            sender_id = self._safe_event_call(scope_or_event, "get_sender_id")
            if sender_id:
    ...
    def _voice_allowed_for_scope(self, scope_or_event: Any) -> bool:
        ...
        if not voice_config or not voice_config.enabled:
            return False
        is_group, candidates = self._voice_scope_candidates(scope_or_event)
        whitelist = voice_config.group_whitelist if is_group else voice_config.private_whitelist
        blacklist = voice_config.group_blacklist if is_group else voice_config.private_blacklist
        if self._voice_list_matches(blacklist, candidates):
            return False
        if whitelist and not self._voice_list_matches(whitelist, candidates):
            return False
        return True
