...
class ChatStyleRuntimeMixin(ChatDelayMixin):
    ...
    def _build_chat_style_decision_hint(
        ...
        if limit > 0:
            lines.append(
                f"- {scope_label}轻闲聊参考长度约 {limit} 字左右；整轮达到主要意思就停，"
                "不要用多条消息绕过总长度；有正事时按内容自然展开。"
            )
        return "\n".join(lines)
    ...
    async def build_chat_style_injection_context(
        ...
        if self._semantic_segment_enabled():
            ...
            length_hint = (
                f"当前{scope_label}闲聊的单个分段参考长度约为 {limit} 字；"
                "轻闲聊整轮也以此作为总长度倾向，发送阶段按完整停顿收束，"
                "这里不是硬性截断；认真问题、事实解释和情绪支持不适用。"
                if limit > 0
            ...
            return (
                ...
                "普通闲聊优先使用短而完整的自然句，整轮只保留最必要的一个或几个意思；"
                "需要转换表达重心或追加独立意思时自然另起一条。"
                "达到主要意思就停，不追加没有必要的理由、安慰、反问或第二层铺陈；"
                "认真问题、事实解释和情绪支持按内容完整说清，不机械压缩。"
                "普通聊天使用纯文本，不使用 Markdown 加粗、斜体、删除线或标题格式。"
                + length_hint
            )
