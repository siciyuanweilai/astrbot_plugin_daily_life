...
class ResponseGateSenseMixin:
    def _response_gate_attention_delta(
        ...
        *,
        ...
        if not focus_key or not isinstance(updated_at, datetime.datetime):
            return 0.0
        lease_until = attention.get("lease_until")
        lease_active = isinstance(lease_until, datetime.datetime) and now < lease_until
        ...
        if focus_key == key:
            ...
            return delta
        if lease_active:
            delta = -0.2 * strength if is_group else -0.1 * strength
        else:
    ...
    def _response_gate_presence_delta(
        ...
        recent = [
            ...
            and (now - item).total_seconds() <= 5 * 60
        ]
    ...
    async def _response_gate_feedback_delta(
        ...
        effects = [
            item
            for item in effects
            if str(getattr(item, "source", "") or "") == "regular_reply"
        ]
        if not effects:
            return 0.0
        total = 0.0
    ...
    async def _response_gate_scene_delta(
        self, archive: Any, key: str, reasons: list[str]
    ) -> float:
        ...
        for item in scenes[:5]:
            confidence = max(
                0.0, min(float(getattr(item, "confidence", 0.0) or 0.0), 1.0)
            )
    ...
    async def _response_gate_episode_delta(
        self, archive: Any, event: Any, reasons: list[str]
    ) -> float:
        ...
        for item in episodes:
            related = {
                str(value or "").strip()
                for value in list(getattr(item, "related_people", []) or [])
            }
            if people & related:
                matched += 1
        if matched <= 0:
    ...
    def _response_gate_note_attention(
        self,
        key: str,
        now: datetime.datetime,
        *,
        ...
        if replied:
            ...
            attention["priority"] = max(
                int(attention.get("priority") or 0), max(0, min(int(priority), 100))
            )
            attention["lease_until"] = now + datetime.timedelta(
                seconds=180 if priority >= 80 else 90
            )
            return
