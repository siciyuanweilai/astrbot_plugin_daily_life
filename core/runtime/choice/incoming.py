...
class ResponseGateEventMixin:
    ...
    def _response_gate_force_reply_reason(self, event: Any) -> str:
        if self._event_has_quote(event):
            if self._event_is_group_message(event):
                ...
                structured = (
                    structured_getter(event) if callable(structured_getter) else None
                )
                talks_to_bot = getattr(
                    self, "_structured_talking_to_bot", lambda _: False
                )
                talks_to_other = getattr(
                    self, "_structured_talking_to_other_user", lambda _: False
                )
        ...
        if not self._event_is_group_message(event):
            return ""
        if self._response_gate_has_media(event) and self._response_gate_visible_text(
            event
        ):
    ...
    def mark_alias_directed_event_as_wake(self, event: Any) -> bool:
        if not self._event_is_group_message(event):
            return False
        if self._event_is_platform_directed(
            event
        ) or not self._event_mentions_bot_alias(event):
            return False
        is_stopped = getattr(event, "is_stopped", None)
        if callable(is_stopped) and is_stopped():
            return False
    ...
    def _response_gate_scope_key(self, event: Any) -> str:
        ...
        return (
            ...
            or self._safe_event_call(event, "get_sender_id")
        )
    ...
    def _response_gate_has_media(self, event: Any) -> bool:
        for item in self._event_message_items(event):
            ...
            if any(
                token in kind for token in ("image", "record", "voice", "video", "file")
            ):
                return True
        return False
