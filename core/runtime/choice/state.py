...
class ResponseGateStateMixin:
    ...
    def _init_response_gate_state(self) -> None:
        ...
        self._response_gate_waiting: dict[str, dict[str, Any]] = {}
        self._response_gate_attention: dict[str, Any] = {}
        self._observed_user_history_keys: set[str] = set()
        self._observed_user_history_order: deque[str] = deque()
        self._recalled_skip_logged: set[str] = set()
        self._recalled_skip_log_order: deque[str] = deque()
        self._response_gate_semantic_metrics: dict[str, int] = {
            "accepted": 0,
            "fallback": 0,
            "timeout": 0,
            "invalid": 0,
    ...
    def _response_gate_wait_state(
        self, key: str, now: datetime.datetime
    ) -> dict[str, Any] | None:
        store = getattr(self, "_response_gate_waiting", None)
        if not isinstance(store, dict):
            self._response_gate_waiting = {}
            return None
        item = store.get(key)
        if not isinstance(item, dict):
            return None
        until = item.get("until")
        if not isinstance(until, datetime.datetime) or now >= until:
            store.pop(key, None)
            return None
        return item

    def _response_gate_mark_wait(
        self,
        key: str,
        ...
        *,
        ...
        current = self._response_gate_wait_state(key, now)
        rounds = int(current.get("rounds") or 0) + 1 if current else 1
        messages = list(current.get("messages") or []) if current else []
    ...
    def _response_gate_mark_continuation(
        ...
        *,
        ...
        try:
            turn_count = int(
                turns if turns is not None else self._RESPONSE_GATE_CONTINUATION_TURNS
            )
        ...
        try:
            window_seconds = int(
                ...
                else self._RESPONSE_GATE_CONTINUATION_SECONDS
            )
        except (TypeError, ValueError):
    ...
    def _response_gate_continuation_reason(
        self, key: str, now: datetime.datetime
    ) -> str:
    ...
    def _response_gate_record_seen(self, key: str, now: datetime.datetime) -> int:
        ...
        if (
            not isinstance(previous_seen_at, datetime.datetime)
            or (now - previous_seen_at).total_seconds() > 180
        ):
            pending_count = 1
    ...
    def _response_gate_record_no_reply(self, key: str, now: datetime.datetime) -> None:
        ...
        self._response_gate_backoff_until[key] = now + datetime.timedelta(
            seconds=seconds
        )

    def _response_gate_in_backoff(
        self, key: str, now: datetime.datetime, pending_count: int
    ) -> bool:
        config = self.config.response_gate
