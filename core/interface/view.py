...
class PageViewMixin:
    ...
    async def _build_page_status(self) -> dict:
        ...
        physiological_rhythm_logs = await self.runtime.archive.get_physiological_rhythm_logs(limit=20)
        physiological_rhythm_trend = await self.runtime.archive.get_physiological_rhythm_trend(days=7, limit=20)
        ...
        long_term_memories = await self.runtime.archive.list_recent_long_term_memories(limit=12)
        memory_clusters = await self.runtime.archive.get_memory_episode_clusters(limit=8)
        ...
        return {
            ...
            "experience": {
                ...
                "emotion_arcs": [item.as_dict() for item in emotion_arcs],
                "physiological_rhythm_logs": [item.as_dict() for item in physiological_rhythm_logs],
    ...
    def _page_current_decision(target_date: str, life_decisions: list) -> dict:
        for item in life_decisions:
            if getattr(item, "kind", "") == "daily_plan" and getattr(item, "date", "") == target_date:
                return item.as_dict()
        for item in life_decisions:
    ...
    async def _page_memo_status(self, target_date: str, data, now: datetime.datetime) -> dict:
        ...
        try:
            ...
            target_day = now.date()
        if target_memo:
            return self._page_memo_payload(self._page_memo_items(target_date, "target", target_memo))
            ...
            items.append(
                {
                    ...
                    "scope": scope,
                    "text": line,
                    "display_text": f"{date_text} {line}" if scope == "future" else line,
                }
            )
        return items
    ...
    async def _page_observatory(
        ...
        if decision:
            ...
            decision["influence_sources"] = self._page_decision_sources(decision, evidence)
                ...
                decision["memory_sources"] = [
                    {
                        **item,
                        "summary": self._page_compact(
                            item.get("influence") or item.get("title") or item.get("content"),
                            80,
                        ),
                    }
    ...
    @staticmethod
    def _page_action_decisions(decisions: list, limit: int = PAGE_WORLD_RECORD_LIMIT) -> list:
        items = []
