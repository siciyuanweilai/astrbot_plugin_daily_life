...
class SocialCommandMixin:
    ...
    async def _commitments(self, event: Any, req: CommandRequest) -> AsyncIterator[Any]:
        ...
        if action in {"添加", "新增"}:
            ...
            if not content:
                ...
                return
            sender_name = await self.runtime.contact_resolver.resolve_event_sender(
                event
            )
        ...
        if action in {"完成", "取消"}:
            ...
            yield event.plain_result(
                "已更新承诺状态。" if ok else f"未找到承诺：{commitment_id}"
            )
            return
    ...
    async def _invite(self, event: Any, req: CommandRequest) -> AsyncIterator[Any]:
        ...
        alt_time = (
            str(decision.get("alternative_time") or "").strip()
