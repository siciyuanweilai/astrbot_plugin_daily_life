...
from ..config.vocab import CN_PERIOD_MAP, PERIOD_HOURS, TEMPLATE_CN_MAP
from ..labels import template_label
...
class OperateCommandMixin:
    async def _clear(self, event: Any, req: CommandRequest) -> AsyncIterator[Any]:
        ...
        yield event.plain_result("✅ 已清空全部日常生活背景数据。需要重新生成时使用 /生活 重置")

    async def _reset(self, event: Any, req: CommandRequest) -> AsyncIterator[Any]:
        if req.param1 in ["清空", "全部", "数据"]:
            yield event.plain_result("清空数据请使用 /生活 清空")
            return

        templates = await self.runtime.composer._get_week_templates()
        template_id = TEMPLATE_CN_MAP.get(req.param1)
        if template_id or req.param1 in templates or req.param1 == "随机":
            template_id = template_id or ("random" if req.param1 == "随机" else req.param1)
            yield event.plain_result(f"生成周计划（模板: {template_label(template_id) or template_id}）...")
            async with self.runtime.generation_lock:
                plan = await self.runtime.composer.generate_week_plan(template_id, req.param2)
            if plan:
                yield event.plain_result(f"✅ 已生成！\n🎯 {plan.theme}\n📌 {', '.join(plan.goals)}")
            else:
    ...
    async def _weather(self, event: Any, req: CommandRequest) -> AsyncIterator[Any]:
        ...
        if not query_city:
            yield event.plain_result("请指定城市：/生活 天气 北京")
            return
