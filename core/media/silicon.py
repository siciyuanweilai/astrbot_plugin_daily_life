...
class SiliconFlowVoiceService:
    ...
    async def synthesize(
        self, text: str, *, emotion: str = "", emotion_category: str = ""
    ) -> GeneratedVoice:
        ...
        if await asyncio.to_thread(lambda: path.exists() and path.stat().st_size > 0):
        ...
        for attempt in range(self.settings.max_retries + 1):
            try:
                ...
                async with session.post(
                    f"{self.settings.api_url}/audio/speech",
                    headers=headers,
                    json=payload,
                ) as response:
                    content_type = str(
                    ...
                    if 200 <= response.status < 300 and (
                        ...
                        speed_note = (
                            ""
                            if route["speed"] == DEFAULT_VOICE_SPEED
                            else f"，语速={route['speed']}"
                        )
                        logger.info(
    ...
    def _voice_route(
        self, emotion: str = "", emotion_category: str = ""
    ) -> dict[str, Any]:
