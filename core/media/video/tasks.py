...
VideoDownloader = Callable[
    [aiohttp.ClientSession, str, dict[str, str], str], Awaitable[str]
]
...
async def resolve_video_task(
    *,
    ...
    data: Any,
    id_label: str,
    poll_video_url: Callable[
        [aiohttp.ClientSession, dict[str, str], str, str], Awaitable[str]
    ],
) -> GeneratedVideo:
    ...
    if not request_id:
        raise RuntimeError(
            f"Grok 视频生成未返回{id_label}标识：{upstream_error_text(data)}"
        )
    ...
    *,
    ...
    while True:
        ...
        if status != last_logged_status or now - last_log_at >= 60:
            log_debug(
                f"{LOG_PREFIX} 等待视频生成任务：{request_id}，状态：{status_label(status)}"
            )
    ...
    try:
        async with session.get(
            ...
            if response.status >= 400:
                ...
                raise RuntimeError(
                    f"Grok 视频内容下载失败（HTTP {response.status}）：{detail[:300]}"
                )
            file = await asyncio.to_thread(tmp_path.open, "wb")
            try:
