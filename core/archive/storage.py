...
class StorageArchiveMixin:
    ...
    def _storage_category_stats_unlocked(
        self, category: StorageCategory, policy: Any = None
    ) -> dict:
        tables = [
        ...
        for table in tables:
            ...
            if cursor.rowcount and cursor.rowcount > 0:
                ...
                if table == "physiological_rhythm_logs":
                    invalidate = getattr(
                        self, "_invalidate_physiological_rhythm_trend_cache", None
                    )
    ...
    def _cleanup_world_unlocked(self, keep_days: int) -> int:
        ...
        return self._delete_statements_unlocked(
            (
                (
                    "DELETE FROM event_people WHERE event_id IN (SELECT id FROM events WHERE date <> '' AND date < ?)",
                    (cutoff,),
                ),
    ...
    def _cleanup_conversation_unlocked(self, keep_days: int) -> int:
        cutoff = self._cutoff_date(keep_days)
        deleted = self._delete_statements_unlocked(
            (
                (
                    """
                ...
                (
                    "DELETE FROM chat_summaries WHERE COALESCE(NULLIF(date, ''), created_at) < ?",
                    (cutoff,),
                ),
                (
                    "DELETE FROM group_environments WHERE COALESCE(NULLIF(date, ''), created_at) < ?",
                    (cutoff,),
                ),
                (
                    "DELETE FROM message_visibility WHERE COALESCE(NULLIF(date, ''), created_at) < ?",
                    (cutoff,),
                ),
                (
                    "DELETE FROM action_decisions WHERE COALESCE(NULLIF(date, ''), created_at) < ?",
                    (cutoff,),
                ),
        ...
        deleted += self._delete_statements_unlocked(
            (
                (
                    """
                    DELETE FROM chat_memory_batches
                    WHERE status IN ('completed', 'failed')
                      AND COALESCE(NULLIF(completed_at, ''), created_at) < ?
                      AND NOT EXISTS (
                          SELECT 1 FROM chat_memory_messages m
                          WHERE m.session_id = chat_memory_batches.session_id
                            AND m.id BETWEEN chat_memory_batches.first_row_id
                                         AND chat_memory_batches.last_row_id
                      )
                    """,
                    (cutoff,),
                ),
                (
                    """
                    DELETE FROM chat_memory_sessions
                    WHERE COALESCE(
                        NULLIF(last_completed_at, ''),
                        NULLIF(last_message_at, ''),
                        updated_at
                    ) < ?
                      AND NOT EXISTS (
                          SELECT 1 FROM chat_memory_messages m
                          WHERE m.session_id = chat_memory_sessions.session_id
                      )
                      AND NOT EXISTS (
                          SELECT 1 FROM chat_memory_batches b
                          WHERE b.session_id = chat_memory_sessions.session_id
                      )
                    """,
                    (cutoff,),
                ),
    ...
    def _cleanup_experience_unlocked(self, keep_days: int) -> int:
        ...
        return self._delete_statements_unlocked(
            (
                ...
                (
                    "DELETE FROM memory_evidence WHERE COALESCE(NULLIF(date, ''), created_at) < ?",
                    (cutoff,),
                ),
                (
                    "DELETE FROM behavior_feedback WHERE COALESCE(NULLIF(date, ''), created_at) < ?",
                    (cutoff,),
                ),
                (
                    """
                ...
                (
                    "DELETE FROM memory_decision_links WHERE decision_id IN (SELECT id FROM life_decisions WHERE COALESCE(NULLIF(date, ''), created_at) < ?)",
                    (cutoff,),
                ),
                (
                    "DELETE FROM life_decisions WHERE COALESCE(NULLIF(date, ''), created_at) < ?",
                    (cutoff,),
                ),
                (
                    "DELETE FROM memory_corrections WHERE applied = 1 AND updated_at < ?",
                    (cutoff,),
                ),
                (
                    "DELETE FROM behavior_patterns WHERE COALESCE(NULLIF(last_seen, ''), updated_at) < ?",
                    (cutoff,),
                ),
                (
                    "DELETE FROM behavior_scenes WHERE COALESCE(NULLIF(last_seen, ''), updated_at) < ?",
                    (cutoff,),
                ),
                (
                    "DELETE FROM focus_slots WHERE COALESCE(NULLIF(expires_at, ''), updated_at) < ?",
                    (cutoff,),
                ),
                (
                    "DELETE FROM focus_targets WHERE expires_at <> '' AND expires_at < ?",
                    (cutoff,),
                ),
                (
                    "DELETE FROM life_terms WHERE COALESCE(NULLIF(last_seen, ''), updated_at) < ?",
                    (cutoff,),
                ),
                (
                    "DELETE FROM memory_boundaries WHERE enabled = 0 AND updated_at < ?",
                    (cutoff,),
                ),
                (
                    "DELETE FROM memory_maintenance WHERE COALESCE(NULLIF(date, ''), created_at) < ?",
                    (cutoff,),
                ),
            )
        )
    ...
    def _cleanup_expression_unlocked(self, keep_days: int) -> int:
        ...
        return self._delete_statements_unlocked(
            (
                ...
                (
                    "DELETE FROM temporary_expression_states WHERE COALESCE(NULLIF(expires_at, ''), updated_at) < ?",
                    (cutoff,),
                ),
                ("DELETE FROM expression_intents WHERE created_at < ?", (cutoff,)),
                (
                    "DELETE FROM emoji_assets WHERE status = 'disabled' AND updated_at < ?",
                    (cutoff,),
                ),
    ...
    def _cleanup_media_unlocked(self, keep_days: int) -> int:
        ...
        return self._delete_statements_unlocked(
            (
                (
                    "DELETE FROM video_insight_frames WHERE insight_id IN (SELECT id FROM video_insights WHERE updated_at > 0 AND updated_at < ?)",
                    (cutoff_time,),
                ),
                (
                    "DELETE FROM video_insight_details WHERE insight_id IN (SELECT id FROM video_insights WHERE updated_at > 0 AND updated_at < ?)",
                    (cutoff_time,),
                ),
                (
    ...
    def _cleanup_planning_unlocked(self, keep_days: int) -> int:
        ...
        for sql, params in (
            (
                "DELETE FROM week_plans WHERE week_id <> '' AND week_id < ?",
                (cutoff_week,),
            ),
    ...
    async def cleanup_storage_category(
        self, category_key: str, keep_days: int | None = None
    ) -> dict:
        category = STORAGE_CATEGORIES.get(normalize_storage_category(category_key))
        if not category:
            raise ValueError("存储分类不存在")
