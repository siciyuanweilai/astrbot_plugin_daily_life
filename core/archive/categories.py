...
STORAGE_CATEGORIES: dict[str, StorageCategory] = {
    ...
    "conversation": StorageCategory(
        ...
        groups=(
            StorageTableGroup(
                "summaries",
                "聊天摘要",
                ("chat_summaries", "chat_summary_people", "session_mid_summaries"),
            ),
            StorageTableGroup(
                "group_awareness",
                "群聊感知",
                ("group_environments", "message_visibility", "action_decisions"),
            ),
        ),
    ),
    "experience": StorageCategory(
        ...
        groups=(
            ...
            StorageTableGroup(
                "signals",
                "证据与反馈",
                ("memory_evidence", "behavior_feedback", "reply_effects"),
            ),
            StorageTableGroup(
                "rhythm", "情绪节律", ("emotion_arcs", "physiological_rhythm_logs")
            ),
            StorageTableGroup(
                "decisions", "决策与修正", ("life_decisions", "memory_corrections")
            ),
            StorageTableGroup(
                "patterns", "行为模式", ("behavior_patterns", "behavior_scenes")
            ),
            StorageTableGroup(
                "focus",
                "关注边界",
    ...
    "expression": StorageCategory(
        ...
        groups=(
            ...
            StorageTableGroup(
                "emoji", "表情素材", ("expression_intents", "emoji_assets")
            ),
    ...
    "longterm": StorageCategory(
        ...
        groups=(
            StorageTableGroup(
                "memories", "记忆条目", ("long_term_memories", "memory_vectors")
            ),
            StorageTableGroup(
                "clusters",
                "片段聚类",
                ("memory_episode_clusters", "memory_episode_cluster_items"),
            ),
            StorageTableGroup(
                "entities", "实体索引", ("memory_entities", "memory_entity_links")
            ),
            StorageTableGroup(
                "quality", "冲突关联", ("memory_conflicts", "memory_decision_links")
            ),
        ),
    ),
...
def iter_owned_storage_tables() -> tuple[str, ...]:
    return tuple(
        table for category in STORAGE_CATEGORIES.values() for table in category.tables
    )


def validate_storage_categories(
    schema_tables: set[str], *, ignored_tables: set[str] | None = None
) -> None:
    ...
    cross_category_clear = sorted(
        f"{category.key}:{table}"
        for category in STORAGE_CATEGORIES.values()
        for table in set(category.clear_order) - set(category.tables)
    )
    if duplicate_tables or missing_tables or unknown_tables or cross_category_clear:
        problems = []
        if duplicate_tables:
        ...
        if cross_category_clear:
            problems.append(f"跨分类清理：{', '.join(cross_category_clear)}")
