...
class ContactNameResolver:
    ...
    def __init__(
        ...
        self._onebot_group_name_pending: dict[str, asyncio.Task[str]] = {}

    async def close(self) -> None:
        """取消可能持续到运行时重载之后的昵称查询。"""
        pending = [
            *self._onebot_name_pending.values(),
            *self._onebot_group_name_pending.values(),
        ]
        self._onebot_name_pending.clear()
        self._onebot_group_name_pending.clear()
        for task in pending:
            if not task.done():
                task.cancel()
        if pending:
    ...
    async def get_onebot_nickname(self, target_uid: str, event: Any = None) -> str:
        ...
        task = asyncio.create_task(
            self._fetch_onebot_nickname(cache_key, target_s, probe_id, event, bot)
        )
        self._onebot_name_pending[cache_key] = task
        task.add_done_callback(
            lambda done: self._forget_pending(
                self._onebot_name_pending, cache_key, done
            )
        )
    ...
    async def _fetch_onebot_nickname(
        ...
        try:
            ret = await call_bot_action(
                bot, "get_stranger_info", raise_missing=True, user_id=int(probe_id)
            )
    ...
    async def resolve_group_name(
        self, group_id: str, event: Any = None, target_umo: str = ""
    ) -> str:
        ...
        event_name = self._clean_group_name_candidate(
            self._safe_event_call(event, "get_group_name")
            or self._safe_event_attr(event, "group_name"),
            group_id_s,
        )
        if event_name:
            return event_name

        adapter_id, real_id = parse_unified_origin(
            target_umo or self._safe_event_attr(event, "unified_msg_origin")
        )
        ...
        task = asyncio.create_task(
            self._fetch_onebot_group_name(cache_key, group_id_s, probe_id, bot)
        )
        self._onebot_group_name_pending[cache_key] = task
        task.add_done_callback(
            lambda done: self._forget_pending(
                self._onebot_group_name_pending, cache_key, done
            )
        )
        return await asyncio.shield(task)
    ...
    async def _fetch_onebot_group_name(
        ...
        try:
            ret = await call_bot_action(
                bot, "get_group_info", raise_missing=True, group_id=int(probe_id)
            )
            if isinstance(ret, dict):
                for key in ("group_remark", "group_memo", "group_name"):
                    name = self._clean_group_name_candidate(ret.get(key), group_id)
                    if name:
    ...
    @classmethod
    def _cached_name(cls, cache: dict[str, str], key: str) -> str:
        value = str(cache.pop(key, "") or "").strip()
        if value:
            cache[key] = value
        return value

    @classmethod
    def _remember_name(cls, cache: dict[str, str], key: str, value: str) -> None:
        cache.pop(key, None)
        cache[key] = value
        while len(cache) > cls._NAME_CACHE_LIMIT:
            cache.pop(next(iter(cache)))

    @staticmethod
    def _forget_pending(
        pending: dict[str, asyncio.Task[str]],
        key: str,
    ...
    def _clean_nickname_candidate(
        self, nickname: str, target_uid: str, event: Any = None
    ) -> str:
