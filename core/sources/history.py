...
class SavedHistoryReader:
    ...
    def resolve_target_umo(self, target_id: str, is_group: bool) -> str:
        ...
        if adapter_id and real_id:
            if is_group and (
                adapter_id.strip().lower() == "weixin_oc"
                or self._is_weixin_session(real_id)
            ):
                return ""
            if (
                len(parts) >= 3
                and parts[1].lower() == "groupmessage"
                and self._is_weixin_session(real_id)
            ):
                return ""
            return target
    ...
    async def _fetch_platform_history(
        self, target_umo: str, max_count: int, hours: int
    ) -> List[Dict[str, str]]:
        ...
        try:
            ...
            messages = [
                msg
                for msg in (
                    self._normalize_platform_record(record) for record in records or []
                )
                if msg and self._within_cutoff(msg.get("timestamp", ""), cutoff)
            ]
            if messages:
                logger.debug(
                    f"{self.log_prefix} 已读取平台保存的聊天历史：{target_umo}（{len(messages)} 条消息）"
                )
    ...
    def _history_user_ids(self, adapter_id: str, real_id: str) -> List[str]:
        ...
        if str(adapter_id or "").lower().startswith("webchat") and ids[0].startswith(
            "webchat!"
        ):
            parts = ids[0].split("!", 2)
