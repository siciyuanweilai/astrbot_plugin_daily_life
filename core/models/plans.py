...
class WeekPlanRecord:
    ...
    def from_value(value: Any, week_id: str = "") -> "WeekPlanRecord":
        ...
        return WeekPlanRecord(
            ...
            goals=[
                str(goal).strip() for goal in raw.get("goals", []) if str(goal).strip()
            ]
            if isinstance(raw.get("goals", []), list)
            else [],
            daily_hints={str(key): str(value).strip() for key, value in hints.items()}
    ...
    def as_dict(self) -> dict[str, Any]:
        return {
            ...
            "suggested_activities": {
                key: list(value) for key, value in self.suggested_activities.items()
            },
