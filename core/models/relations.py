    ...
    def from_value(value: Any) -> RelationshipRecord | None:
        ...
        return RelationshipRecord(
            ...
            subjective_tags=[
                str(item).strip()
                for item in (
                    raw.get("subjective_tags", [])
                    if isinstance(raw.get("subjective_tags"), list)
                    else []
                )
                if str(item).strip()
            ],
            ...
            contacts=[
                contact
                for contact in (
                    RelationshipContactRecord.from_value(item) for item in contacts
                )
                if contact
            ],
        )
...
class GroupEnvironmentRecord:
    ...
    def from_value(value: Any) -> GroupEnvironmentRecord | None:
        ...
        return GroupEnvironmentRecord(
            ...
            understanding_confidence=optional_int(raw.get("understanding_confidence"))
            or 0,
            deep_analysis_needed=optional_bool(raw.get("deep_analysis_needed"))
            or False,
            summary=summary,
            created_at=str(raw.get("created_at") or "").strip(),
        )
...
class MessageVisibilityRecord:
    ...
    def from_value(value: Any) -> MessageVisibilityRecord | None:
        ...
        return MessageVisibilityRecord(
            ...
            psychological_freshness=optional_int(raw.get("psychological_freshness"))
            or 0,
