...
class BehaviorPatternRecord:
    ...
    def from_value(value: Any, *, date: str = "", source: str = "chat_memory") -> "BehaviorPatternRecord | None":
        ...
        suggested_action = _text(raw.get("suggested_action") or raw.get("action"), 80)
...
class BehaviorSceneRecord:
    ...
    def from_value(value: Any, *, date: str = "", source: str = "chat_memory") -> "BehaviorSceneRecord | None":
        ...
        if not isinstance(value, dict):
            return None
        raw = value
        scene = _text(raw.get("scene") or raw.get("label"), 120)
        cues = _texts(raw.get("cues") or raw.get("signals"), 8, 80)
        preferred_action = _text(raw.get("preferred_action") or raw.get("action"), 80)
        outcome_hint = _text(raw.get("outcome_hint") or raw.get("outcome"), 200)
...
class SessionMidSummaryRecord:
    ...
    def from_value(value: Any, *, source: str = "chat_memory") -> "SessionMidSummaryRecord | None":
        ...
        participants = _texts(raw.get("participants") or raw.get("people"), 12, 40)
        if not (session_id and (summary or topic or mood or participants)):
            return None
        return SessionMidSummaryRecord(
            session_id=session_id,
            scope_label=_text(raw.get("scope_label") or raw.get("label"), 120),
            summary=summary,
            topic=topic,
            mood=mood,
