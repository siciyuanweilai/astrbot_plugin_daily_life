...
class PreferenceRecord:
    ...
    def from_value(
        ...
        content = str(
            (raw.get("content") if isinstance(value, dict) else value) or ""
        ).strip()
        if not content:
            return None
...
class LifeEventRecord:
    ...
    def from_value(
        ...
        raw = value if isinstance(value, dict) else {}
        title = str(
            (raw.get("title") if isinstance(value, dict) else value) or ""
        ).strip()
        if not title:
            return None
...
class DailyReviewRecord:
    ...
    def from_value(value: Any, *, date: str = "") -> "DailyReviewRecord | None":
        ...
        return DailyReviewRecord(
            ...
            memory_points=[
                str(item).strip()[:240] for item in memory_points if str(item).strip()
            ],
            preference_points=[
                item
                for item in (
                    PreferenceRecord.from_value(
                        pref, date=date_text, source="daily_review"
                    )
            ...
            life_events=[
                item
                for item in (
                    LifeEventRecord.from_value(
                        event, date=date_text, source="daily_review"
                    )
