...
class EmotionArcRecord:
    ...
    def from_value(
        ...
        return EmotionArcRecord(
            ...
            label=label or "情绪波动",
            valence=EmotionArcRecord._score(
                raw.get("valence"), 0, lower=-100, upper=100
            ),
...
class PhysiologicalRhythmLogRecord:
    ...
    @staticmethod
    def _score(
        value: Any, default: int = 0, *, lower: int = 0, upper: int = 100
    ) -> int:
        number = optional_int(value)
        if number is None:
            number = default
    ...
    @staticmethod
    def _optional_cycle_enabled(
        raw: dict, label: str, source: str, intensity: int
    ) -> bool:
        return bool(optional_bool(raw.get("optional_cycle_enabled"))) and bool(
            label or source or intensity > 0
        )
    ...
    def from_value(
        ...
        *,
        ...
        optional_cycle_intensity = PhysiologicalRhythmLogRecord._score(
            raw.get("optional_cycle_intensity"), 0
        )
        ...
        return PhysiologicalRhythmLogRecord(
            ...
            social_battery=PhysiologicalRhythmLogRecord._score(
                raw.get("social_battery"), 50
            ),
            ...
            optional_cycle_intensity=optional_cycle_intensity
            if optional_cycle_enabled
            else 0,
            optional_cycle_source=optional_cycle_source
            if optional_cycle_enabled
            else "",
            summary=summary,
            lifecycle_kind=PhysiologicalRhythmLogRecord._lifecycle(
                raw.get("lifecycle_kind")
            ),
