...
class LifeTermRecord:
    ...
    def from_value(
        ...
        return LifeTermRecord(
            ...
            examples=_texts(raw.get("examples"), 6, 120),
...
class MemoryBoundaryRecord:
    ...
    def from_value(value: Any) -> "MemoryBoundaryRecord | None":
        if isinstance(value, MemoryBoundaryRecord):
            return (
                None
                if MemoryBoundaryRecord.is_same_scope(
                    value.source_scope, value.target_scope
                )
                else value
            )
        raw = value if isinstance(value, dict) else {}
        source_scope = _text(raw.get("source_scope"), 160)
        target_scope = _text(raw.get("target_scope"), 160)
        if not (source_scope and target_scope) or MemoryBoundaryRecord.is_same_scope(
