...
def as_reference_image_items(
    value: Any, *, max_count: int = 6
) -> list[dict[str, Any]]:
...
def as_friend_reference_profiles(value: Any) -> list[dict[str, Any]]:
    if not isinstance(value, list):
        return []
    result: list[dict[str, Any]] = []
    seen: set[str] = set()
    for raw in value:
        if not isinstance(raw, dict):
            continue
        profile_id = as_str(raw.get("profile_id", "")).strip()
        if not profile_id or profile_id in seen:
            continue
        display_name = as_str(raw.get("display_name", "")).strip() or profile_id
        reference_images = as_reference_image_items(
            raw.get("reference_images", []), max_count=3
        )
        seen.add(profile_id)
        item = {
