...
class LifeSettings:
    ...
    video_generation: VideoGenerationSettings = field(
    ...
    def from_dict(data: Any) -> LifeSettings:
        ...
        config.schedule_time = normalize_time(
            rhythm_conf.get("schedule_time", "07:00"), "07:00"
        )
        config.reference_history_days = as_int(
            rhythm_conf.get("history_days", 3), 3, 0, 30
        )
        ...
        config.history_hours = as_int(rhythm_conf.get("history_hours", 24), 24, 0, 168)
        config.history_max_count = as_int(
        ...
        config.image_generation = ImageGenerationSettings.from_dict(
            image_generation_conf
        )
        config.video_generation = VideoGenerationSettings.from_dict(
            video_generation_conf
        )
        config.video_generation.aspect_ratio = (
            config.image_generation.primary_aspect_ratio()
        )
        ...
        config.state_prompt = (
            as_str(
                prompt_conf.get("state_rules", DEFAULT_STATE_PROMPT),
                DEFAULT_STATE_PROMPT,
            ).strip()
            or DEFAULT_STATE_PROMPT
        )
        config.timeline_prompt = (
            as_str(
                prompt_conf.get("timeline_rules", DEFAULT_TIMELINE_PROMPT),
                DEFAULT_TIMELINE_PROMPT,
            ).strip()
            or DEFAULT_TIMELINE_PROMPT
        )
        config.world_prompt = (
            as_str(
                prompt_conf.get("world_rules", DEFAULT_WORLD_PROMPT),
                DEFAULT_WORLD_PROMPT,
            ).strip()
            or DEFAULT_WORLD_PROMPT
        )
        config.chat_prompt = (
            as_str(
                prompt_conf.get("chat_rules", DEFAULT_CHAT_PROMPT), DEFAULT_CHAT_PROMPT
            ).strip()
