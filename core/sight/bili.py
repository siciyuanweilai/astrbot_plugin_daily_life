...
async def resolve_bili_target(
    target: BiliTarget, *, timeout_seconds: int = 10
) -> BiliTarget | None:
    if not target:
        return None
    if target.bvid:
        ...
        if target.url == canonical_url:
            return target
        return BiliTarget(
            bvid=target.bvid, url=canonical_url, source=target.source, page=target.page
        )
...
async def fetch_bili_metadata(
    ...
    *,
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.get(
                ...
                if int(getattr(response, "status", 0) or 0) >= 400:
                    logger.debug(
                        f"{LOG_PREFIX} B站视频元数据请求失败：HTTP {getattr(response, 'status', '')}；{target.bvid}"
                    )
                    return None
                payload = await response.json(content_type=None)
    except Exception as exc:
        logger.debug(
            f"{LOG_PREFIX} B站视频元数据请求异常：{target.bvid}；{type(exc).__name__}: {exc}"
        )
        return None
    if isinstance(payload, dict) and payload.get("code") not in (None, 0):
        logger.debug(
            f"{LOG_PREFIX} B站视频元数据请求失败："
            f"状态码={payload.get('code')}；信息={payload.get('message') or payload.get('msg') or ''}；{target.bvid}"
        )
        return None
    data = (
        payload.get("data")
    ...
    if not data:
        logger.debug(
            f"{LOG_PREFIX} B站视频元数据请求失败：响应缺少数据字段；{target.bvid}"
        )
        return None
...
def target_from_value(
    value: Any, *, source: str = "event", depth: int = 0
) -> BiliTarget | None:
    ...
    for attr in ("raw_message", "message_str", "text", "content", "data", "url"):
        if hasattr(value, attr):
            target = target_from_value(
                getattr(value, attr, None), source=attr, depth=depth + 1
            )
            if target:
                return target
    return None
