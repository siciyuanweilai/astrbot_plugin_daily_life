...
class SightBrief:
    ...
    async def summarize(
        ...
        *,
        ...
        max_chars = max(
            500, int(getattr(settings, "max_transcript_chars", 8000) or 8000)
        )
        transcript = " ".join(str(transcript or "").split())[:max_chars]
        ...
        checkpoint_key = self._checkpoint_key(
            clip, transcript, frame_notes, metadata, provider_id=provider_id
        )
        ...
        fusion_transcript = (
            ...
            else _excerpt(transcript, TRANSCRIPT_EXCERPT_LIMIT)
        )
        ...
        details = _clean_list(
            audio_details + _clean_list(payload.get("details"), limit=5), limit=8
        )
    ...
    def _checkpoint_key(
        ...
        *,
        ...
        payload = {
            ...
            "transcript": " ".join(str(transcript or "").split())[:30000],
    ...
    def _prompt(
        ...
        *,
        ...
        frame_text = (
            "\n".join(f"- {item}" for item in frame_notes[:12])
            or "（没有时间线画面描述）"
        )
