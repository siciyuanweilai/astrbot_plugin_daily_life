...
class BiliCookieJar:
    ...
    def save(self, cookies: Mapping[str, str]) -> bool:
        clean = {
            str(key): str(value) for key, value in dict(cookies or {}).items() if value
        }
    ...
    def ensure_cookiefile(self) -> Path | None:
        ...
        try:
            ...
            logger.debug(f"{LOG_PREFIX} B站登录凭据文件准备失败：{exc}")
            return None
    ...
    def _write_json(self, data: Mapping[str, str]) -> None:
        self._atomic_write(
            self._path, json.dumps(dict(data), ensure_ascii=False, indent=2)
        )
