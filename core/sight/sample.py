...
def _looks_like_media_file(path: Path) -> bool:
    try:
        ...
        return False
    return (len(header) >= 12 and header[4:8] == b"ftyp") or header.startswith(
        (b"\x1aE\xdf\xa3", b"FLV", b"RIFF", b"\x00\x00\x01\xba")
    )
    ...
    *,
    ...
    cached_ready = await asyncio.to_thread(
        lambda: (
            ...
            and (has_video_suffix or _looks_like_media_file(target))
        )
    )
    if cached_ready:
        return target, ""
    ...
    try:
        async with aiohttp.ClientSession(timeout=timeout) as session:
            ...
            async with session.get(source, headers=request_headers) as response:
                ...
                if (
                    not has_video_suffix
                    and content_type
                    and not _looks_like_media_content_type(content_type)
                ):
...
async def resolve_sample_source(
    ...
    *,
    ...
    if "bilibili.com" in lowered or "b23.tv" in lowered:
        cookiefile = await asyncio.to_thread(
            lambda: BiliCookieJar(cache_dir).ensure_cookiefile()
        )
...
async def _download_bili_direct_video(
    ...
    *,
    ...
    if not resolved or not resolved.bvid:
        ...
        return None
    cookies = await asyncio.to_thread(lambda: BiliCookieJar(cache_dir).get())
    if cookiefile and await asyncio.to_thread(cookiefile.is_file):
        try:
            text = await asyncio.to_thread(
                cookiefile.read_text, encoding="utf-8", errors="ignore"
            )
        except OSError:
            text = ""
    ...
    metadata = await fetch_bili_metadata(
        resolved, timeout_seconds=10, cookies=cookies or None
    )
    cid = 0
    if metadata and metadata.cid:
        cid = metadata.cid
    if not cid:
        cid = await _bili_pagelist_cid(
            resolved.bvid, resolved.page or 1, resolved.canonical_url, cookies
        )
    if not cid:
        _log_video_prepare_failed("B站视频", "没有获取到视频 cid")
        return None
    direct = await _bili_playurl_direct_url(
        resolved.bvid, cid, resolved.canonical_url, cookies
    )
    if not direct:
...
async def _bili_pagelist_cid(
    bvid: str, page: int, referer: str, cookies: dict[str, str]
) -> int:
    ...
    async with aiohttp.ClientSession(
        timeout=timeout, headers=browser_headers(cookies, referer=referer)
    ) as session:
...
async def _bili_playurl_direct_url(
    bvid: str, cid: int, referer: str, cookies: dict[str, str]
) -> str | None:
    timeout = aiohttp.ClientTimeout(total=10)
    async with aiohttp.ClientSession(
        timeout=timeout, headers=browser_headers(cookies, referer=referer)
    ...
    if dash:
        ...
        if best:
            return (
                str(best.get("baseUrl") or best.get("base_url") or "").strip() or None
            )
    return None
...
def select_frame_seconds(duration: float, max_frames: int) -> list[float]:
    ...
    step = duration / count
    seconds = [
        min(duration - 0.2, max(0.2, step * (index + 0.5))) for index in range(count)
    ]
    result: list[float] = []
...
def _candidate_frame_count(duration: float, max_frames: int) -> int:
    limit = max(1, int(max_frames or 8))
    if duration <= max(limit * 2, 6):
        return limit
    return min(24, limit + min(limit, 4))


def _frame_visual_signature(path: Path) -> tuple[int, ...]:
    try:
        from PIL import Image

        with Image.open(path) as image:
            pixels = image.convert("L").resize((16, 16)).getdata()
...
def select_distinct_frames(
    frames: list[SightFrame], max_frames: int
) -> list[SightFrame]:
    limit = max(1, int(max_frames or 8))
    if len(frames) <= 1:
        return list(frames)

    signatures = [_frame_visual_signature(frame.path) for frame in frames]
    distinct: list[int] = [0]
    for index in range(1, len(frames)):
        if _signature_distance(signatures[distinct[-1]], signatures[index]) >= 0.045:
            distinct.append(index)
    if len(distinct) > 1 and distinct[-1] != len(frames) - 1:
        last = len(frames) - 1
        if _signature_distance(signatures[distinct[-1]], signatures[last]) >= 0.025:
            distinct.append(last)

    if len(distinct) <= limit:
        return [frames[index] for index in distinct]

    selected = {distinct[0], distinct[-1]}
    while len(selected) < limit:
        ...
        for index in distinct:
            if index in selected:
                continue
            visual = min(
                _signature_distance(signatures[index], signatures[item])
                for item in selected
            )
            temporal = min(abs(index - item) for item in selected) / max(
                1, len(frames) - 1
            )
            score = visual + math.sqrt(temporal) * 0.08
            if score > best_score:
                best_index = index
                best_score = score
        if best_index < 0:
...
async def extract_video_frames(
    source_path: Path, cache_dir: Path, max_frames: int = 8
) -> list[SightFrame]:
    if not source_path:
        return []
    ...
    duration = (
        await asyncio.to_thread(_video_duration, ffprobe, source_path)
        if ffprobe
        else 0.0
    )
    frames: list[SightFrame] = []
    seen_hashes: set[str] = set()
    candidate_count = _candidate_frame_count(duration, max_frames)
    for index, second in enumerate(
        select_frame_seconds(duration, candidate_count), start=1
    ):
        label = _format_second(second)
        target = frame_dir / f"frame_{index:02d}_{_safe_label(second)}.jpg"
        ok = await asyncio.to_thread(
            _extract_frame, ffmpeg, source_path, target, second
        )
        if not ok:
            continue
    ...
    for frame in frames:
        if frame.path not in selected_paths:
            await asyncio.to_thread(frame.path.unlink, missing_ok=True)
    return selected
