...
async def audio_outline_checkpointed(
    ...
    *,
    ...
    if runtime is None or not checkpoint_key:
        return await brief._audio_outline(
            provider, call_llm, composer, transcript, metadata, provider_id=provider_id
        )
...
async def _audio_outline_direct_checkpointed(
    ...
    *,
    ...
    result = await brief._audio_outline(
        provider, call_llm, composer, transcript, metadata, provider_id=provider_id
    )
    try:
...
def _checkpoint_matches(
    payload: dict[str, Any], checkpoint_key: str, provider_id: str
) -> bool:
    return (
