    ...
    *,
    ...
    if result and result.has_text and cache_dir is not None:
        await asyncio.to_thread(
            ...
            audio_path,
            result,
            raw_payload=transcriber.last_payload,
        )
    return result
...
def _existing_audio(target_dir: Path, fingerprint: str) -> Path | None:
    for path in sorted(target_dir.glob(f"{fingerprint}.*")):
        if (
            path.suffix.lower() in AUDIO_SUFFIXES
            and path.is_file()
...
def _downloaded_audio_path(
    info: dict[str, Any], target_dir: Path, fingerprint: str
) -> Path | None:
...
def _download_audio_with_ytdlp_sync(
    ...
    try:
        ...
        return None
    return _downloaded_audio_path(
        info if isinstance(info, dict) else {}, target_dir, fingerprint
    )


async def download_audio_with_ytdlp(
    source: str, cache_dir: Path, *, cookiefile: Path | None = None
) -> Path | None:
    ...
    return await asyncio.to_thread(
        _download_audio_with_ytdlp_sync, source, target_dir, fingerprint, cookiefile
    )
...
class BcutTranscriber:
    ...
    async def transcribe(self, audio_path: Path) -> TranscriptResult | None:
        ...
        async with aiohttp.ClientSession(
            ...
            task_id = await self._create_task(
                session, str(committed.get("download_url") or "")
            )
    ...
    async def _create_upload(
        ...
        size = await asyncio.to_thread(lambda: audio_path.stat().st_size)
    ...
    async def _upload_chunks(
        self, session: aiohttp.ClientSession, audio_path: Path, upload: dict[str, Any]
    ) -> None:
    ...
    async def _commit_upload(
        self, session: aiohttp.ClientSession, upload: dict[str, Any]
    ) -> dict[str, Any]:
        return await self._post_json(
            session,
    ...
    async def _create_task(
        self, session: aiohttp.ClientSession, download_url: str
    ) -> str:
        if not download_url:
            raise AudioTranscriptError("音频转写任务缺少下载地址")
        data = await self._post_json(
            session, f"{API_BASE}/task", {"resource": download_url, "model_id": "8"}
        )
    ...
    async def _await_result(
        self, session: aiohttp.ClientSession, task_id: str, deadline: float
    ) -> dict[str, Any]:
        while time.monotonic() < deadline:
            async with session.get(
                f"{API_BASE}/task/result", params={"model_id": 7, "task_id": task_id}
            ) as response:
                if int(response.status or 0) >= 400:
                    raise AudioTranscriptError(
                        f"音频转写查询失败：HTTP {response.status}"
                    )
                payload = await response.json(content_type=None)
            if payload.get("code") != 0:
                raise AudioTranscriptError(
                    str(payload.get("message") or "音频转写查询失败")
                )
    ...
    async def _post_json(
        self, session: aiohttp.ClientSession, url: str, body: dict[str, Any]
    ) -> dict[str, Any]:
        async with session.post(
            url, data=json.dumps(body), headers=_headers()
        ) as response:
            if int(response.status or 0) >= 400:
        ...
        if payload.get("code") != 0:
            raise AudioTranscriptError(
                str(payload.get("message") or "音频转写请求失败")
            )
        data = payload.get("data")
        return data if isinstance(data, dict) else {}


def _transcript_from_bcut(
