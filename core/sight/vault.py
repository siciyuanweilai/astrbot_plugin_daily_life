...
class SightVault:
    def __init__(
        self, archive: Any, *, ttl_seconds: int = 2 * 60 * 60, max_items: int = 60
    ):
    ...
    async def upsert(self, insight: SightInsight) -> SightInsight:
        ...
        clip_metadata = (
            insight.clip.metadata if isinstance(insight.clip.metadata, dict) else {}
        )
        for key in ("content_fingerprint", "platform_id", "file_size"):
            if clip_metadata.get(key) not in ("", None):
                metadata[key] = clip_metadata[key]
        payload["metadata"] = metadata
        payload["cache_key"] = insight.key
        saved = await saver(
            payload, ttl_seconds=self.ttl_seconds, max_items=self.max_items
        )
        return (
            SightInsight.from_dict(saved) or insight
    ...
    async def remove_message(self, scope: str, message_id: str) -> int:
        ...
        if not callable(remover):
            return 0
        return int(
            await remover(str(scope or "").strip(), str(message_id or "").strip()) or 0
        )
