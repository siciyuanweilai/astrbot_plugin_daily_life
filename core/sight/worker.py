...
def plain_funasr_text(result: Any) -> str:
    ...
    for item in items:
        text = str(
            item.get("text", "") if isinstance(item, dict) else item or ""
        ).strip()
        if text:
            parts.append(text)
...
def timestamped_segments(result: Any) -> list[dict[str, Any]]:
    ...
    for item in items:
        ...
        timestamps = [
            _timestamp_pair(value) for value in list(item.get("timestamp") or [])
        ]
...
def _chunk_tokens(
    tokens: list[str], timestamps: list[tuple[int, int]]
) -> list[dict[str, Any]]:
    segments: list[dict[str, Any]] = []
    current: list[str] = []
    start_ms = 0
    ...
    for token, timestamp in zip(tokens, timestamps):
        ...
        end_ms = timestamp[1]
        if (
            end_ms - start_ms >= SEGMENT_TARGET_MS
            and len(current) >= SEGMENT_MIN_TOKENS
        ):
...
def main() -> int:
    ...
    output.write_text(
        json.dumps(transcribe(args), ensure_ascii=False), encoding="utf-8"
    )
    return 0
