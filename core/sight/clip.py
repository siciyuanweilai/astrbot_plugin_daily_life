...
class SightClip:
    ...
    def key(self) -> str:
        content_fingerprint = str(
            self.metadata.get("content_fingerprint") or ""
        ).strip()
        platform_id = str(self.metadata.get("platform_id") or "").strip()
        file_size = str(self.metadata.get("file_size") or "").strip()
        provisional_identity = (
            f"{self.name}|{file_size}" if self.name and file_size else ""
        )
        identity = str(
            content_fingerprint
            or platform_id
