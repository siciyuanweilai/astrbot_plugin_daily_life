...
def component_data(item: Any) -> dict[str, Any]:
    ...
    try:
        ...
        direct = {}
    data: dict[str, Any] = dict(direct) if isinstance(direct, dict) else {}
    ...
    for key in (
        ...
        if value not in (None, ""):
            data[key] = value
    if not data.get("file") and data.get("file_"):
...
def payload_from_item(item: Any) -> dict[str, str]:
    ...
    for key in ("file", "path", "url", "src", "video", "video_url"):
        raw = data.get(key)
        source = source_from_value(raw) if isinstance(raw, dict) else clean_source(raw)
        if source:
            values[key] = source
        if key == "file" and isinstance(raw, dict):
            for nested_key in (
                "id",
                "file_id",
                "fileid",
                ...
                text = str(raw.get(nested_key) or "").strip()
                if not text:
                    continue
                normalized_key = {
                    "id": "file_id",
                ...
                values.setdefault(normalized_key, text)
    for key in (
        "file_id",
        "fileid",
        "id",
...
def clip_from_item(
    ...
    *,
    ...
    name = (
        payload.get("name") or payload.get("file_name") or payload.get("filename") or ""
    )
    file_size = payload.get("file_size") or payload.get("size") or ""
...
def clips_from_value(
    ...
    *,
    ...
    if isinstance(value, str):
        return clips_from_cq_text(
            value, scope=scope, message_id=message_id, origin=origin, text=text
        )

    direct = clip_from_item(
        value, scope=scope, message_id=message_id, origin=origin, text=text
    )
    clips = [direct] if direct else []
    data = component_data(value)
    nested_message_id = str(
        data.get("target_message_id") or data.get("message_id") or message_id or ""
    ).strip()
    for key in (
        "chain",
        "message",
...
def clips_from_cq_text(
    ...
    *,
    ...
    for match in re.finditer(r"\[CQ:([^,\]]+)(?:,([^\]]*))?\]", raw_message or ""):
        ...
        clip = clip_from_item(
            data, scope=scope, message_id=message_id, origin=origin, text=text
        )
        if clip:
            clips.append(clip)
    return dedupe_clips(clips)
...
def explicit_clip(
    value: str, *, scope: str, message_id: str, text: str = ""
) -> SightClip | None:
    source = clean_source(value)
    if not source:
        return None
    return SightClip(
        scope=scope, message_id=message_id, source=source, origin="explicit", text=text
    )
