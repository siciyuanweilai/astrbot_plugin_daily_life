...
async def transcribe_local(
    ...
    *,
    ...
    return await transcribe_local_audio(
        audio_path, cache_dir, config=config, max_chars=max_chars
    )
...
async def transcribe_local_audio(
    ...
    *,
    ...
    payload = await asyncio.to_thread(_read_json, output_path)
...
async def ensure_wav(audio_path: Path, cache_dir: Path) -> Path:
    ...
    if not ok:
        raise AudioTranscriptError("本地语音识别音频转换失败")
    return target


def transcript_from_payload(
    payload: dict[str, Any], *, max_chars: int = 8000
) -> TranscriptResult | None:
    pieces: list[str] = []
    ...
    async with _prepare_lock:
        ...
        if not await asyncio.to_thread(_modules_ready):
            raise AudioTranscriptError(
                "本地语音识别依赖未安装，请在框架管理面板进入“更多 → 平台日志”，"
                "点击“安装 Pip 库”，输入 funasr>=1 modelscope>=1 torch>=2 "
                "torchaudio>=2 完成安装后重启框架"
            )
        await asyncio.to_thread(
            _ensure_models, models_dir, max(300, int(config.timeout_seconds or 900))
        )
...
async def _run_worker(
    input_path: Path, output_path: Path, cache_dir: Path, config: LocalAsrConfig
) -> None:
    ...
    async with semaphore:
        ...
        if result.returncode != 0:
            ...
            raise AudioTranscriptError(
                f"本地语音识别转写失败：{detail or result.returncode}"
            )
        if not await asyncio.to_thread(output_path.is_file):
            raise AudioTranscriptError("本地语音识别没有生成转写结果")
        elapsed = time.monotonic() - started
        logger.debug(f"{LOG_PREFIX} 本地语音识别转写耗时：{elapsed:.1f} 秒")
...
def _models_ready(models_dir: Path) -> bool:
    return all(
        _looks_like_model(models_dir / alias) for alias in MODEL_ALIASES.values()
    )


def _download_model(
    repo: str, target: Path, cache_dir: Path, timeout_seconds: int
) -> None:
    ...
    logger.info(f"{LOG_PREFIX} 本地语音识别模型准备开始：{repo}")
    result = subprocess.run(
    ...
    if result.returncode != 0 and not downloaded:
        raise AudioTranscriptError(
            f"本地语音识别模型下载失败：{_last_lines(result.stderr or result.stdout)}"
        )
    if not downloaded or not _looks_like_model(downloaded):
        raise AudioTranscriptError(f"本地语音识别模型校验失败：{repo}")
    if target.exists():
...
def _downloaded_model_path(stdout: str, cache_dir: Path, repo: str) -> Path | None:
    for line in reversed(
        [item.strip() for item in str(stdout or "").splitlines() if item.strip()]
    ):
        path = Path(line)
...
def _looks_like_model(path: Path) -> bool:
    return path.is_dir() and any(
        (path / name).exists()
        for name in ("configuration.json", "config.yaml", "model.pt")
    )
...
def _read_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise AudioTranscriptError(f"本地语音识别结果读取失败：{exc}") from exc
