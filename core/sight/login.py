...
from .auth import (
    browser_headers,
    enrich_bili_cookies,
    parse_login_url_cookies,
    remember_response_cookies,
)
...
class BiliLoginService:
        ...
        try:
            async with aiohttp.ClientSession(timeout=self.timeout) as session:
                ...
                async with session.get(
                    QR_GENERATE, headers=self._headers()
                ) as response:
    ...
    async def poll(self, key: str) -> BiliLoginResult:
        try:
            async with aiohttp.ClientSession(
                ...
                async with session.get(
                    QR_POLL, params={"qrcode_key": key}, headers=self._headers()
                ) as response:
                    if int(response.status or 0) != 200:
    ...
    async def run_until_complete(
        self, key: str, *, total_timeout: float = 180
    ) -> BiliLoginResult:
        elapsed = 0.0
        delay = 2.0
        while elapsed < total_timeout:
            ...
            if result.status in {
                ...
                BiliLoginStatus.EXPIRED,
                BiliLoginStatus.ERROR,
            }:
                return result
            ...
            delay = (
                1.5
                if result.status == BiliLoginStatus.SCANNED
                else min(5.0, delay * 1.2)
            )
