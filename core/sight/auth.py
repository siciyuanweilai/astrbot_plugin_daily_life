...
def cookie_header(cookies: dict[str, str] | None = None) -> str:
    return "; ".join(
        f"{key}={value}" for key, value in dict(cookies or {}).items() if key and value
    )
...
async def enrich_bili_cookies(
    ...
    *,
    ...
    async with aiohttp.ClientSession(
        timeout=timeout, headers=browser_headers(values)
    ) as session:
...
async def _touch_home(session: aiohttp.ClientSession, cookies: dict[str, str]) -> None:
    try:
        async with session.get(
            BILI_HOME_URL, headers=browser_headers(cookies)
        ) as response:
