...
TRANSCRIPT_CAPTURE_CHARS = 500000
...
class SightReader:
    def __init__(self, runtime: Any):
        ...
        self.audio_transcript_mode = str(
            getattr(settings, "audio_transcript_mode", "local") or "local"
        )
        self.max_chars = max(TRANSCRIPT_CAPTURE_CHARS, int(max_chars or 8000))
        self.local_asr_config = LocalAsrConfig(
            batch_size_s=LOCAL_ASR_BATCH_SIZE_SECONDS,
            timeout_seconds=max(
                120, int(getattr(settings, "local_asr_timeout_seconds", 900) or 900)
            ),
        )
    ...
    async def read(self, _event: Any, clip: SightClip) -> SightTextResult:
        ...
        return self._text_result_from_transcript(
    ...
    def _text_result_from_transcript(
        ...
        if base_metadata:
            ...
            merged.update(
                {
                    key: value
                    for key, value in dict(transcript.metadata or {}).items()
    ...
    async def _read_transcript(
        ...
        *,
        ...
        for route in self._transcript_routes():
            result = await self._read_route(
                route, source, errors=errors, log_stage=log_stage
            )
            if result:
                return result
        return None
    ...
    async def _read_route(
        ...
        *,
        ...
        label = "本地语音识别" if route == "local" else "必剪"
        timeout_seconds = (
            self.local_asr_config.timeout_seconds
            if route == "local"
            else self.audio_timeout_seconds
        )
        try:
            if log_stage:
    ...
    async def _read_prepared_transcript(
        ...
        *,
        ...
        for route in self._transcript_routes():
            result = await self._read_prepared_route(
                route, audio_path, errors=errors, log_stage=log_stage
            )
            if result:
                return result
        return None
    ...
    async def _call_prepared_route(
        self, route: str, audio_path: Path
    ) -> TranscriptResult | None:
        if route == "local":
